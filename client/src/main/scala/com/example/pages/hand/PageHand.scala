...
object PageHandInternal {
   ...
   *
   */
  case class State( currentcontract: Contract,
    ...
    def withScoring() = {
      val rc = if (contractTricks match {
        ...
        case _ =>
          ...
          tricks.isDefined &&
          (!allowHonors || honors.isDefined && (honors.getOrElse(100)==0 || honorsPlayer.isDefined) &&
                                               (honors.getOrElse(0) != 100 || contractSuit != NoTrump)
              )

      }) {
        val cc = if (contractTricks.get.tricks == 0) {
          currentcontract.copy( contractTricks=0,
          ...
          currentcontract.copy( contractTricks=contractTricks.get,
...
//                  "old State "+this)
    ...
    def nextInput() = {
      ...
      PageHandNextInput.values.find { x => x match {
        ...
        case InputContractDoubled => contractDoubled.isEmpty && gotContractTricks
        case InputContractBy => declarer.isEmpty && gotContractTricks
        case InputHonors => allowHonors && honors.isEmpty
        case InputHonorsPlayer => allowHonors && honors.getOrElse(0) != 0 && honorsPlayer.isEmpty
        case InputResultMadeOrDown => gotContractTricks && madeOrDown.isEmpty
        case InputResultTricks => gotContractTricks && (tricks.isEmpty || madeOrDown.isEmpty ||
                                  !BridgeHand.getTricksRange(madeOrDown.get, contractTricks.get).contains(tricks.get))
        case InputAll => true
        }
      }.get
    }
  ...
  object PageHandNextInput extends MyEnumeration {
      ...
      val InputContractTricks, InputContractSuit, InputContractDoubled, InputContractBy,
          InputHonors, InputHonorsPlayer,
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def renderHand(props: Props,state: State) = {
      ...
      def sectionHeader = {
        val (cbHonors, cbHonorsP, curHonors, curHonorsP) = if (state.allowHonors) {
          ( Some( setHonors _), Some(setHonorsPlayer _), state.honors, state.honorsPlayer )
        } else {
          ( None, None, None, None )
        }
        SectionHeader(contract.scoringSystem,
                      state.declarer,
                      contract.north,
                      contract.south,
                      contract.east,
      ...
      def sectionResults = {
        ...
        SectionResult(state.madeOrDown,
                      state.tricks,
                      state.contractTricks,
                      state.contractSuit,
      ...
      <.div( handStyles.pageHand,
          <.span( ^.id:="VerifySectionHeader","Bridge Scorer:"),
  ...
  val component = ScalaComponent.builder[Props]("PageHand")
                            .initialStateFromProps { props =>
                              if (props.newhand) State( props.contract,
                                          ...
                                          props.callbackWithHonors.isDefined,
                                          props.honors,
                                          props.honorsPlayer).withScoring()
                            }
