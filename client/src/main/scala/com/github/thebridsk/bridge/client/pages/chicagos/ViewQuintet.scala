...
object ViewQuintetInternal {
  ...
  val logger: Logger = Logger("bridge.ViewQuintet")
  ...
  private[chicagos] val playerName = ScalaComponent
    .builder[(String, String)]("PlayerName")
    .render_P(props => {
      val (name, key) = props
  ...
  private[chicagos] val playerScore = ScalaComponent
    .builder[(Int, String)]("PlayerScore")
    .render_P(props => {
      val (score, key) = props
  ...
  private[chicagos] val summaryRoundRow = ScalaComponent
    .builder[(Props, Int, List[Int], List[Int])]("SummaryRoundRow")
    .render_P(rowprops => {
      val (props, round, scores, order) = rowprops
      val r = props.chicagoScoring.rounds(round)

      def getPlayer(pos: PlayerPosition) =
        pos match {
