...
object ViewPlayerPlacesGraph {
  ...
  def apply(stats: Option[PlayerPlaces]) =
    component(Props(stats)) // scalafix:ok ExplicitResultTypes; ReactComponent
...
object ViewPlayerPlacesGraphInternal {
  ...
  /**
    ...
    */
  def getPlaceString(place: Int): String =
    sPlaces.drop(place).headOption.getOrElse(s"${place + 1} place")

  //                                                        cols                         getSize
  private[duplicate] val Row = ScalaComponent
    .builder[
      (Props, List[PlayerPlace], Int, IntLegendUtil[(Int, Int)], Int => Int)
    ]("ComponentBoard.TeamRow")
    .render_P(cprops => {
  ...
  //
  private[duplicate] val Legend = ScalaComponent
    .builder[(Props, IntLegendUtil[(Int, Int)], Int, Int)](
      "ComponentBoard.TeamRow"
    )
    .render_P(cprops => {
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      props.stats match {
        case Some(pps) =>
          ...
          val (maxOther, minPlayed, maxPlayed) =
            pps.players.foldLeft((0, 99999, 0)) { (acc, pp) =>
              val mo = pp.place.foldLeft(0) { (acc, o) =>
                Math.max(acc, o.length)
              }
              (
                Math.max(acc._1, mo),
                Math.min(acc._2, pp.total),
                Math.max(acc._3, pp.total)
              )
            }
          ...
          val legendUtil = new IntLegendUtil[(Int, Int)] {
            def nameToTitle(name: (Int, Int)): String = {
              ...
              getPlaceString(place) + (if (other == 0) ""
                                       else s" with ${other} other")
            }
  ...
  private[duplicate] val component = ScalaComponent
    .builder[Props]("ViewPlayerPlacesGraph")
