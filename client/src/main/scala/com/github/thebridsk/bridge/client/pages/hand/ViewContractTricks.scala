...
object ViewContractTricks {
  ...
  private val component = ScalaComponent
    .builder[Props]("ViewContractTricks")
    .render_P(props => {
      import HandStyles._
      val (missingRequired, missingNotNext) = {
        val mr =
          props.current.isEmpty || (props.current.get.tricks == 0 && !props.allowPassedOut)
        if (mr) {
      ...
      def isSelected(i: Int) =
        props.current.isDefined && props.current.get.tricks == i
      def getButton(i: Int) = {
        val text = if (i > 0) i.toString; else "Passed"
        <.button(
          i == 0 ?= handStyles.contractTricksButton0,
          ^.`type` := "button",
