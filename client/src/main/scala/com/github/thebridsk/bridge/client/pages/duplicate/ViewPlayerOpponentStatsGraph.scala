...
object ViewPlayerOpponentStatsGraphInternal {
  ...
  object ColorByPctBeat extends ColorBy("PctBeat") {
    def value(po: PlayerOpponentStat): Double =
      po.matchesBeat * 100.0 / po.matchesPlayed
  }

  object ColorByPctTied extends ColorBy("PctTied") {
    def value(po: PlayerOpponentStat): Double =
      po.matchesTied * 100.0 / po.matchesPlayed
  }

  object ColorByPctLost extends ColorBy("PctLost") {
    def value(po: PlayerOpponentStat): Double =
      (po.matchesPlayed - po.matchesTied - po.matchesBeat) * 100.0 / po.matchesPlayed
  }
  ...
  def stepTitles(
      stat: ColorStat,
      n: Int
  ): (List[TagMod], TagMod, List[TagMod]) = {
    val min = stat.min
    val ave = stat.ave
    val max = stat.max
  ...
  def getRows(
    ...
    playersOpponentsStats.players.map { rowPlayer =>
      TagMod(rowPlayer.player) ::
        ...
        List(
          getData(
            rowPlayer.playerTotal.copy(opponent = "all"),
            statTotalSize,
            statTotalSizeMP,
            state,
            1.5
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      ...
      props.stats match {
        case Some(posUnsorted) if !posUnsorted.players.isEmpty =>
          ...
          val columns =
            Column("Player") :: pos.players.map(p => Column(p.player)) ::: List(
              Column("Totals")
            )
                  ...
                  <.tr(
                    <.td(
                      ...
                      PieChart(
                        15,
                        1.0 :: Nil,
                        Some(ComparisonPieChart.ColorGood :: Nil),
                      ...
                      PieChart(
                        15,
                        1.0 :: Nil,
                        Some(ComparisonPieChart.ColorNeutral :: Nil),
                        attrs = Some(^.display := "inline-block")
                      ),
                      ...
                      PieChart(
                        15,
                        1.0 :: Nil,
                        Some(ComparisonPieChart.ColorBad :: Nil),
                        attrs = Some(^.display := "inline-block")
                      ),
                      ", indicates losing.",
                      <.br,
  ...
  private[duplicate] val component = ScalaComponent
    .builder[Props]("ViewPlayerOpponentStatsGraph")
