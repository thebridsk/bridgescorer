...
object ViewPlayerOpponentStatsTable {
  ...
  case class Props(
      stats: Option[PlayersOpponentsStats],
...
object ViewPlayerOpponentStatsTableInternal {
  ...
  val pairColumns: List[StatColumn[Any]] = List[StatColumn[Any]](
    new StringColumn("Player", "Player")(
      new PlayerSorter("Player", "Opponent")
    ) {
      def getValue(pd: PlayerOpponentStat) = pd.player
      override val useAdditionalDataWhenSorting = true
    },
    new StringColumn("Opponent", "Opponent")(
      new PlayerSorter("Opponent", "Player")
    ) {
  ...
  val columns: List[StatColumn[Any]] = List[StatColumn[Any]](
    new PercentColumn("MPPct", "% MP") {
      def getValue(pd: PlayerOpponentStat) =
        if (pd.totalMP == 0) 0.0 else pd.wonMP * 100.0 / pd.totalMP
    },
    new PercentColumn("BeatPts", "% Beat") {
      def getValue(pd: PlayerOpponentStat) =
        if (pd.matchesPlayed == 0) 0.0
        else pd.matchesBeat * 100.0 / pd.matchesPlayed
    },
    new PercentColumn("TiedPts", "% Tied") {
      def getValue(pd: PlayerOpponentStat) =
        if (pd.matchesPlayed == 0) 0.0
        else pd.matchesTied * 100.0 / pd.matchesPlayed
    },
    new PercentColumn("LostPts", "% Lost") {
      def getValue(pd: PlayerOpponentStat) =
        if (pd.matchesPlayed == 0) 0.0
        else
          (pd.matchesPlayed - pd.matchesBeat - pd.matchesTied) * 100.0 / pd.matchesPlayed
    },
    ...
    new IntColumn("Lost", "Lost") {
      def getValue(pd: PlayerOpponentStat) =
        pd.matchesPlayed - pd.matchesBeat - pd.matchesTied
    }
  )
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      props.stats match {
        case Some(pd) =>
          ...
          val rows =
            if (props.showPairs) {
              sortedpd.players.flatMap(pos =>
                pos.opponents.map(ps => getRow(ps, allColumns))
              )
            } else {
...
//                ),
              ...
              caption = Some(
                TagMod(
                  if (props.showPairs) "Opponents Results"
                  else "Opponents Total Results"
                )
              )
            )
          )
  ...
  private[duplicate] val component = ScalaComponent
    .builder[Props]("ViewPlayerOpponentStatsTable")
