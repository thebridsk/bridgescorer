...
object PageMovements {
  ...
  case class Props(
      ...
      initialDisplay: Option[Movement.Id]
  )
...
object PageMovementsInternal {
  ...
  /**
    ...
    */
  case class State(
      movements: Map[Movement.Id, Movement] = Map(),
  ...
  private[boardsets] val SummaryHeader = ScalaComponent
    .builder[State]("PageMovements.SummaryHeader")
    .render_P(state => {
      <.tr(
        <.th("Name"),
        <.th("Description"),
  ...
  private[boardsets] val SummaryRow = ScalaComponent
    .builder[
      (Backend, Props, State, Movement.Id, Callback, Option[Movement.Id])
    ]("PageMovements.SummaryRow")
    .render_P(args => {
      ...
      <.tr(
        ...
        <.td(
          ...
          mov.isDeletable ?= AppButton(
            s"${mov.name}_delete",
            "Delete",
            ^.onClick --> backend.deleteCB(mov.id)
          ),
          mov.isResetToDefault ?= AppButton(
            s"${mov.name}_reset",
            "Reset",
            ^.onClick --> backend.resetCB(mov.id)
          )
        )
      )
    })
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def toggleBoardSet(name: Movement.Id): Callback =
      scope.props >>= { props =>
    ...
    def deleteCB(id: Movement.Id): Callback =
      scope.modState(
        { s =>
          s.setMsg(s"Deleting movement $id")
        },
        Callback {
          BoardSetController.deleteMovement(id).recordFailure().onComplete {
            tr =>
              logger.fine(s"movement $id deleted: $tr")
              tr match {
                case Success(x) =>
                  scope.withEffectsImpure
                    .modState(_.setMsg(s"Deleted movement $id"))
                case Failure(ex) =>
                  scope.withEffectsImpure
                    .modState(_.setMsg(s"Error deleting movement $id"))
              }
          }
        }
      )

    def resetCB(id: Movement.Id): Callback =
      scope.modState(
        { s =>
            ...
            .onComplete { tr =>
              ...
              tr match {
                case Success(x) =>
                  ...
                  scope.withEffectsImpure
                    .modState(_.setMsg(s"Movement $id was reset to default"))
                case Failure(ex) =>
                  scope.withEffectsImpure.modState(
                    _.setMsg(s"Error resetting movement $id to default")
                  )
              }
            }
        }
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      logger.info(
        "PageMovements.Backend.render: display " + props.initialDisplay
      )
      <.div(
  ...
  private[boardsets] val component = ScalaComponent
    ...
    .initialStateFromProps { props =>
      {
        logger.info(
          "PageMovements.component.initialState: initial display " + props.initialDisplay
        )
        State()
      }
    }
