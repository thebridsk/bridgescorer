    ...
    Alerter.tryitWithUnit {
      msg match {
        case m: ChicagoBridgeAction =>
          m match {
            case ActionUpdateChicagoHand(chiid, roundid, handid, hand, cb) =>
              updateChicagoHand(chiid, roundid, handid, hand, cb)
            case ActionUpdateChicagoNames(
                  chiid,
                  nplayer1,
                  nplayer2,
                  nplayer3,
            ...
            case ActionUpdateChicago5(chiid, extraPlayer, cb) =>
              updateChicago5(chiid, extraPlayer, cb)
            case ActionUpdateChicagoRound(chiid, round, cb) =>
              updateChicagoRound(chiid, round, cb)
...
//      logger.warning("BoardSetStore: Unknown msg dispatched, "+x)
  ...
  def isMonitoredId(chiid: MatchChicago.Id): Boolean =
    monitoredId match {
  ...
  private def update(
      ...
      fun: (Option[MatchChicago]) => Option[MatchChicago],
      callback: Option[MatchChicago => Unit]
  ) = {
    monitoredId match {
      case Some(id) if (id == chiid) =>
        logger.info(
          "ChicagoStore." + funName + ": updating chicagostore id=" + id
        )
        chicago = fun(chicago)
        chicago match {
          case Some(chi) =>
          ...
          case None =>
            logger.warning(
              "ChicagoStore." + funName + ": did not have chicago, monitoredId is " + monitoredId
            )
        }
      case _ =>
        logger.warning(
          "ChicagoStore." + funName + ": expecting id " + monitoredId + ", got " + chiid
        )
    }
  }
            ...
            extra match {
              case Some(e) =>
                val nchi = if (mc.isConvertableToChicago5) {
                  mc.setPlayers(nplayer1, nplayer2, nplayer3, nplayer4)
                    .playChicago5(e)
                  ...
                  // not valid
                  logger.severe(
                    "Not valid to set 5 names on chicago match " + mc.id
                  )
                  mc
                }
                Some(if (quintet) {
                  if (nchi.gamesPerRound == 0 && nchi.rounds.isEmpty) {
                    ...
                    logger.severe(
                      "Setting Quintet is not valid if gamesPerRound is not 0 or rounds is not empty"
                    )
                    nchi
                  }
                } else {
              ...
              case _ =>
                Some(mc.setPlayers(nplayer1, nplayer2, nplayer3, nplayer4))
            }
          case None =>
            None
        }
