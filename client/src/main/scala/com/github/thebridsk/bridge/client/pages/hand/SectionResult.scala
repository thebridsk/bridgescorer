      ...
      contractTricks match {
        case Some(ct) =>
          if (ct.tricks == 0) false
            ...
            madeOrDown.isEmpty || tricks.isEmpty ||
...
//                    north,south,east,west,
  ...
  private val component = ScalaComponent
    ...
    .render_P(props => {
      ...
      <.div(
        handStyles.sectionResult,
        props.callbackHonors.whenDefined(x =>
          ViewHonors(
            props.currentHonors,
            props.currentHonorsPlayer,
        ...
        <.div(
          ...
          header(),
          <.div(
            showButtons ?= ViewMadeOrDown(
              props.madeOrDown,
              props.callbackMadeOrDown,
              props.nextInput,
              show(InputResultMadeOrDown)
            ),
            showButtons ?= ViewTricks(
              props.tricks,
              props.contractTricks,
              props.madeOrDown,
              props.callbackTricks,
              props.nextInput,
              show(InputResultTricks)
            )
          )
        ),
        props.callbackHonors.isEmpty ?= {
          ...
          props.contract.scorer match {
            case Some(Right(score)) /* Duplicate */ =>
              <.div(
                score.contractAndResultAsString,
                handStyles.contractAndResult
              )
