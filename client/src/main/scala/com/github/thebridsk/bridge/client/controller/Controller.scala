...
object Controller extends {
  ...
  class CreateResultMatchDuplicate(
    ...
    def this(
        result: RestResult[MatchDuplicate]
  ...
  def createMatchDuplicate(
    ...
    val result = RestClientDuplicate
  ...
  def updateMatch(dup: MatchDuplicate): Unit = {
    ...
    if (!BridgeDemo.isDemo) {
      if (useRestToServer) {
        val resource =
          RestClientDuplicate.update(dup.id, dup).recordFailure().onComplete {
            t =>
              if (t.isFailure) {
  ...
  def updateHand(dup: MatchDuplicate, hand: DuplicateHand): Unit = {
    ...
    if (!BridgeDemo.isDemo) {
      if (useRestToServer) {
        val resource =
          RestClientDuplicate.boardResource(dup.id).handResource(hand.board)
...
//          logger.info(s"Controller.getDemoSummary got json = ${bm}")
  ...
  def getSummary(error: () => Unit /* = ()=>{} */ ): Unit = {
    ...
    setTimeout(1) { // note the absence of () =>
      if (BridgeDemo.isDemo) {
        ...
        RestClientDuplicateSummary.list().recordFailure().onComplete {
          trylist =>
            trylist match {
              case Success(list) =>
                ...
                case JsDefined(jds) =>
                  ...
                  Json.fromJson[List[DuplicateSummary]](jds) match {
                    case JsSuccess(ds, path) =>
                      ...
                      BridgeDispatcher.updateDuplicateSummary(
                        Some(importId),
                        ds
                      )
                    case JsError(err) =>
  ...
  def monitorDuplicateResult(id: MatchDuplicateResult.Id): Unit = {
    ...
    RestClientDuplicateResult.get(id).recordFailure().foreach { dr =>
      Alerter.tryitWithUnit {
