package com.github.thebridsk.bridge.client.pages.individual.support
...
object SummaryPeople {
  ...
  case class FirstPlaces(firstPlace: Int, total: Int) {
    override def toString(): String = s"${firstPlace}/${total}"
  ...
  case class PlayerScores(score: Double, total: Double) {
    override def toString(): String =
      s"${Utils.toPointsString(score)}/${total}"
...
class SummaryPeople(osummaries: Option[List[DuplicateSummary]]) {
  ...
  val allPlayers: List[String] =
    summaries.flatMap(_.playerNames().toSet).toList.distinct.sortWith((p1, p2) => p1 < p2)
