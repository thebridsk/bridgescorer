...
object ViewScoreboardInternal {
  ...
  private[duplicate] val Header = ScalaComponent
    ...
    .render_P(props => {
      val (currentRound, currentTable) = props.page match {
        case TableRoundScoreboardView(dupid, tableid, roundid) =>
          (roundid, tableid)
        case _ => (-1, "")
      }
      val tablePerspective: Option[PerspectiveTable] =
      ...
      def boardButton(b: BoardScore) = {
        ...
        val clickPage =
          if (played) props.page.toBoardView(id)
          else props.page.toBoardView(id).toHandView(ns)
        AppButton(
          s"Board_${id.id}",
          id.toNumber,
      ...
      <.thead(
        <.tr(
          ...
          <.th(^.rowSpan := 2, "Total"),
          <.th(
            ^.colSpan := props.score.sortedBoards.size,
            "Boards",
            props.useIMP ?= " (IMPs)"
          )
        ),
  ...
  private[duplicate] val TeamRow = ScalaComponent
    .builder[(Team, Props)]("ViewScoreboard.TeamRow")
    .render_P(props => {
      val (team, p) = props
      val md = p.score
      <.tr(
        ...
        <.td(
          if (p.useIMP) f"${md.teamImps(team.id)}%.1f"
          else Utils.toPointsString(md.teamScores(team.id))
        ),
  ...
  class Backend(scope: BackendScope[Props, State]) {
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      {
        val names =
          props.score.teams.flatMap(t => t.player1 :: t.player2 :: Nil)
      ...
      def teamColumn(teams: List[Team]) = {
        ...
        for (team <- teams.sortWith((t1, t2) => t1.id < t2.id)) yield {
          ...
          <.span(
            count != 1 ?= <.br(),
            team.id.toNumber + " " + team.player1 + " " + team.player2
          )
        }
      }
          ...
          <.caption(
            ...
            <.span(
              ...
              showidbutton ?= <.span(
                ...
                AppButton(
                  "Duplicate_" + props.score.id.id,
                  props.score.id.id,
                  props.routerCtl.setOnClick(
                    CompleteScoreboardView(props.score.id.id)
                  )
                )
              )
            ),
