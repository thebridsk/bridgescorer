...
object ViewScoreboardHelp {
  ...
  def apply(page: BaseScoreboardViewWithPerspective, md: MatchDuplicateScore) =
    ViewScoreboardHelpInternal.component(
      Props(page, md)
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      ...
      def showBoardAndMovement() = {
        <.p(
          "Boards " + props.md.getBoardSet.id + ", Movement " + props.md.getMovement.id
        )
      }
      <.div(
        ...
        props.page.getPerspective match {
          case PerspectiveTable(t1, t2) =>
            ...
            Seq(
              <.h1(
                s"Table ${currentTable.toNumber} Scoreboard, Round ${currentRound}, Teams ${team1.toNumber} and ${team2.toNumber}"
              ),
