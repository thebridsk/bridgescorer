...
object BoardSetController {
  ...
  def getBoardSets(): Result[Unit] = {
    val r = if (!BridgeDemo.isDemo) {
      ...
      RestClientBoardSet.list().recordFailure().map( items => {
  ...
  def updateBoardSet( bs: BoardSet ): Result[Unit] = {
    val r = if (!BridgeDemo.isDemo) {
      RestClientBoardSet.update(bs.id,bs).recordFailure().map( item => {
  ...
  def getMovement(): Result[Unit] = {
    val r = if (!BridgeDemo.isDemo) {
      RestClientMovement.list().recordFailure().map( items => {
  ...
  def updateMovement( mov: Movement ): Result[Unit] = {
    val r = if (!BridgeDemo.isDemo) {
      RestClientMovement.update(mov.id,mov).recordFailure().map( item => {
  ...
  def getBoardsetsAndMovements( demoCB: =>Unit = {} ): Result[Unit] = {
    val r = if (!BridgeDemo.isDemo) {
      RestClientBoardSetsAndMovements.list().recordFailure().map( items => {
        items.foreach { bm =>
