...
/**
  * Component page for making pairing suggestions for
  * a two table duplicate match.
  *
  * The page will display a list of known names with a checkbox,
  * and from zero to eight input fields, depending on how many known names are checked.
  * Optionally, never pairs can also be selected.
  *
  * The result of the pairing suggestion calculation shows the top ten suggestions.
  *
...
object PageSuggestion {
  ...
  /**
    ...
    */
  def apply(routerCtl: BridgeRouter[DuplicatePage]) =
    component(
      Props(routerCtl)
  ...
  protected object Internal {
    ...
    /**
      ...
      */
    case class State(
        numberPlayers: Option[Int] = Some(8),
    ...
    class Backend(scope: BackendScope[Props, State]) {
      ...
      def toggleNeverPairPlayers(p1: String, p2: String): Callback =
        scope.modState { s =>
          if (s.isNeverPair(p1, p2)) s.removeNeverPair(p1, p2)
          else s.addNeverPair(p1, p2)
        }

      val toggleNeverPair: Callback = scope.modState { s =>
        s.copy(showNeverPair = !s.showNeverPair)
      }

      val cancelNeverPair: Callback = scope.modState { s =>
        s.copy(showNeverPair = false, neverPair = List())
      }

      val clearNeverPair: Callback = scope.modState { s =>
        s.copy(neverPair = List())
      }

      val toggleDetails: Callback = scope.modState { s =>
        s.copy(showDetails = !s.showDetails)
      }

      def toggleKnownPlayer(p: String): Callback =
        scope.modState { s =>
          val kps =
            if (s.knownPlayersSelected.contains(p))
              s.knownPlayersSelected.filter(pp => pp != p)
            else p :: s.knownPlayersSelected
          val error =
            if (kps.length > s.numberPlayers.getOrElse(0))
              Some("Too many players are selected")
            else None
          val np =
            if (kps.length + s.newPlayers.length > s.numberPlayers.getOrElse(0)) {
              val keep = s.numberPlayers.getOrElse(0) - kps.length
              if (keep >= 0) s.newPlayers.take(keep) else Nil
            } else {
              s.newPlayers
            }
          s.copy(knownPlayersSelected = kps, newPlayers = np, error = error)
            .trace(s"""toggleKnownPlayer(${p})""")
        }

      import com.github.thebridsk.bridge.clientcommon.react.Utils._
      def setNumberPlayers(e: ReactEventFromInput): Callback =
        e.inputText { number =>
          scope.modState { s =>
            val error =
              if (s.knownPlayersSelected.length > s.numberPlayers.getOrElse(0))
                Some("Too many players are selected")
              else None
            s.copy(numberPlayers = Some(number.toInt), error = error)
              .trace(s"""setNumberPlayer(${number})""")
          }
        }

      def setNewPlayers(i: Int)(e: ReactEventFromInput): Callback =
        e.inputText { p =>
          scope.modState { s =>
      ...
      val clear: Callback = scope.modState { s =>
        s.copy(
          knownPlayersSelected = List(),
          newPlayers = List(),
          suggestion = None
        )
      }
      ...
      val calculateLocal: Callback = scope.modState { s =>
        setTimeout(0) {
          Alerter.tryitWithUnit {
            val summary =
              DuplicateSummaryStore.getDuplicateSummary.getOrElse(List())

            val input = DuplicateSuggestions(
              s.knownPlayersSelected ::: s.newPlayers,
              10,
              neverPair = Some(s.neverPair)
            )
            val suggestion =
              DuplicateSuggestionsCalculation.calculate(input, summary)
            suggestion.suggestions match {
              case None =>
                logger.fine("Did not get any suggestions")
              case Some(list) =>
                logger.fine(s"""Suggestions:${list
                  .map(su => su.toString())
                  .mkString("\n  ", "\n  ", "")}""")
            }
            if (mounted) {
        ...
        s.copy(error =
          Some(s"Calculating best pairing for ${s.getSelected.mkString(", ")}")
        )
      }
      ...
      val calculateServer: Callback = scope.modState { s =>
        ...
        result.foreach { suggestion =>
          logger.info(
            s"Got new duplicate suggestion ${suggestion}.  PageSuggestion.mounted=${mounted}"
          )
          if (mounted) {
            scope.withEffectsImpure.modState { ss =>
              ss.copy(suggestion = Some(suggestion), error = None)
          ...
          t match {
            ...
            case _ =>
              scope.withEffectsImpure.modState(s =>
                s.copy(error = Some("Failed to calculate suggestion"))
              )
          }
        })
      ...
      def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
        ...
        val history = state.suggestion
          ...
          .getOrElse(
            s"Have a history of ${DuplicateSummaryStore.getDuplicateSummary.map(l => l.length).getOrElse(0)} matches"
          )

        val sortedPlayers =
          (state.knownPlayersSelected ::: state.newPlayers).sorted
        ...
        <.div(
          ...
          <.div(
                  ...
                  val x = list.zipWithIndex.map { entry =>
                    ...
                    <.li(
                      CheckBox(
                        s"KP${i}",
                        p,
                        state.knownPlayersSelected.contains(p),
                        toggleKnownPlayer(p)
                      )
                    )
                  }.toTagMod
            ...
            <.div(
              <.ul(
                (0 until state.numberPlayers.getOrElse(
                  0
                ) - state.knownPlayersSelected.length).map { i =>
                  <.li(
                    <.input(
                      ^.`type` := "text",
                      ^.name := s"NP${i}",
                      ^.value := (if (i < state.newPlayers.length)
                                    state.newPlayers(i)
                                  else ""),
              ...
              <.table(
                ...
                <.tbody(
                  sortedPlayers.zipWithIndex.map { e =>
                    val (p, i) = e
                    NeverPairRow.withKey(s"Player${i}")(
                      (props, state, this, p, sortedPlayers)
                    )
                  }.toTagMod
                )
              )
                      ...
                      <.table(
                        ...
                        <.tfoot(
                          <.tr(
                            <.td(),
                            <.td(
                              ^.colSpan := sugs.head.players.length,
                              "Note: (games since last played, games played together)"
                            ),
                            if (state.showDetails)
                              <.td(
                                ^.colSpan := 8,
                                ^.rowSpan := 2,
                                "The higher the weight the better the pairing is"
                              )
                            else TagMod()
                          ),
                          <.tr(
                            <.td(
                              ^.colSpan := sugs.head.players.length + 1,
                              suggestion.calcTimeMillis.whenDefined(calcTime =>
                                f"Calculation time: ${calcTime}%.0f milliseconds"
                              )
                            )
                          )
                        ),
                        <.tbody(
                          sugs.zipWithIndex.map { e =>
                            val (su, i) = e
                            SummaryRow.withKey(s"Suggestion${i}")(
                              (props, state, this, i, su)
                            )
                          }.toTagMod
                        )
                      )
            ...
            <.div(
              ...
              <.div(
                ...
                AppButton(
                  "NeverPair",
                  "Never Pair",
                  state.showNeverPair || !state.isValid ?= baseStyles.alwaysHide,
                  ^.onClick --> toggleNeverPair
                ),
                AppButton(
                  "ClearNeverPair",
                  "Clear Never Pair",
                  !state.showNeverPair ?= baseStyles.alwaysHide,
                  ^.onClick --> clearNeverPair
                ),
                AppButton(
                  "CancelNeverPair",
                  "Cancel Never Pair",
                  !state.showNeverPair ?= baseStyles.alwaysHide,
                  ^.onClick --> cancelNeverPair
                ),
                AppButton(
                  "ToggleDetails",
      ...
      val didMount: Callback = Callback {
        ...
        Controller.getSummary(() =>
          scope.withEffectsImpure.modState(s =>
            s.copy(error = Some("Error getting duplicate summary"))
          )
        )
      }
    ...
    private val SummaryHeader = ScalaComponent
      .builder[(Props, State, Backend)]("SuggestionHeader")
      .render_P(args => {
    ...
    private val SummaryRow = ScalaComponent
      .builder[(Props, State, Backend, Int, Suggestion)]("SuggestionRow")
      .render_P(args => {
        // row is zero based
        ...
        <.tr(
          ...
          sug.players
            ...
            .map { p =>
              <.td(
                s"${p.player1}-${p.player2} (${p.lastPlayed},${p.timesPlayed})"
              )
            }
            .toTagMod,
          if (state.showDetails) {
    ...
    private val NeverPairHeader = ScalaComponent
      .builder[(Props, State, Backend, List[String])]("SuggestionHeader")
      .render_P(args => {
    ...
    private val NeverPairRow = ScalaComponent
      .builder[(Props, State, Backend, String, List[String])]("SuggestionRow")
      .render_P(args => {
        // row is zero based
        ...
        <.tr(
          ...
          allplayers.map { p =>
            <.td(
              ...
              state
                .isNeverPair(player, p) ?= TagMod(baseStyles.buttonSelected, "X")
            )
          }.toTagMod
        )
      })
