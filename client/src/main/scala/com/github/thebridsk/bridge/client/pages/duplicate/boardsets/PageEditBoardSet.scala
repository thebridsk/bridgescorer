...
object PageEditBoardSetInternal {
  ...
  case class State(
    ...
    def getBoardSet: BoardSet = {
      boardset.getOrElse(
        BoardSet(
          ...
          "",
          if (nboards > 0)
            (1 to nboards).map(i => BoardInSet(i, false, false, "")).toList
          else List(),
          deletable = Some(true)
        )
      )
    }
    def setName(name: BoardSet.Id): State =
      copy(boardset = Some(getBoardSet.copy(name = name)))
    def setShort(description: String): State =
      copy(boardset = Some(getBoardSet.copy(short = description)))
    ...
    def setNBoards(n: Int): State = {
      (if (n < nboards) {
        ...
        val curbs = getBoardSet
        val nb =
          (nboards + 1 to n).map(i => BoardInSet(i, false, false, "")).toList
        copy(
          boardset = Some(curbs.copy(boards = curbs.boards ::: nb)),
          nboards = n
        )
      } else {
        this
  ...
  private[boardsets] val BoardHeader = ScalaComponent
    .builder[Props]("PageEditBoardSet.BoardHeader")
    .render_P(props => {
      <.thead(
        <.tr(
          <.th(^.rowSpan := 2, "Board"),
  ...
  private[boardsets] val BoardRow = ScalaComponent
    ...
    .render_P(args => {
      ...
      def radioDealer(pos: String) = {
        ...
        RadioButton(
          id = s"${board.id}_D_$pos",
          text = "",
          value = isDealer,
          toggle = setDealerCB(board.id, pos)
        )
      }
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def inputCB(data: ReactEventFromInput): Callback =
      data.inputText(text => scope.modState(_.setName(BoardSet.id(text))))
    ...
    def toggleNSVul(boardId: Int): Callback =
      scope.modState(s => s.toggleNSVul(boardId))
    def toggleEWVul(boardId: Int): Callback =
      scope.modState(s => s.toggleEWVul(boardId))

    val clickOk: Callback = scope.modState(
      { s =>
        s.setMsg(
          s.boardSetId
            .map(i => s"Updating boardset $i")
            .getOrElse("Creating new boardset")
        )
      },
      Callback {
                ...
                listofboardsets.map { boardsets =>
                  <.div(
                    <.table(
                      ...
                      <.tbody(
                        boardsets.map { b =>
                          BoardRow.withKey(b.id)(
                            (props, b, setDealer, toggleNSVul, toggleEWVul)
                          )
                        }.toTagMod
                      )
                    )
      ...
      props.page match {
        case bsev: BoardSetEditView =>
          ...
          BoardSetStore.getBoardSet(display) match {
            case Some(bs) =>
              val bs1 =
                if (bs.isDeletable) bs else bs.copy(resetToDefault = Some(true))
              Some(
                state.copy(boardset = Some(bs1), nboards = bs1.boards.length)
              )
            case None =>
              None
          }
  ...
  private[boardsets] val component = ScalaComponent
    .builder[Props]("PageEditBoardSet")
