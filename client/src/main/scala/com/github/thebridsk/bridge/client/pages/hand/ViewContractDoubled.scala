...
object ViewContractDoubled {
  ...
  case class Props(
      current: Option[ContractDoubled],
      callback: CallbackDoubled,
  ...
  def apply(
    ...
    component(
      Props(current, callback, nextInput, visible)
    ) // scalafix:ok ExplicitResultTypes; ReactComponent

  private val component = ScalaComponent
    .builder[Props]("ViewContractDoubled")
    .render_P(props /* (props,state,backend) */ => {
      ...
      def isSelected(d: ContractDoubled) =
      ...
      <.div(
        handStyles.viewContractDoubled,
