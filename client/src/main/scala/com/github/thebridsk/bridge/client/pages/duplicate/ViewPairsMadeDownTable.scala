...
object ViewPairsMadeDownTableInternal {
  ...
  val columns: List[StatColumn[Any]] = List[StatColumn[Any]](
    new PercentColumn("DeclarerPct", "% Declarer") {
      def getValue(pd: PairData) =
        pd.details.map(d => d.percentDeclared).getOrElse(0.0)
    },
    new PercentColumn("DefendedPct", "% Defended") {
      def getValue(pd: PairData) =
        pd.details.map(d => d.percentDefended).getOrElse(0.0)
    },
    new PercentColumn("PassedPct", "% Passed") {
      def getValue(pd: PairData) =
        pd.details.map(d => d.percentPassed).getOrElse(0.0)
    },
    new PercentColumn("MadePct", "% Made") {
      def getValue(pd: PairData) =
        pd.details.map(d => d.percentMade).getOrElse(0.0)
    },
    new PercentColumn("DownPct", "% Down") {
      def getValue(pd: PairData) =
        pd.details.map(d => d.percentDown).getOrElse(0.0)
    },
    new PercentColumn("AllowedMadePct", "% Allowed Made") {
      def getValue(pd: PairData) =
        pd.details.map(d => d.percentAllowedMade).getOrElse(0.0)
    },
    new PercentColumn("TookDownPct", "% Took Down") {
      def getValue(pd: PairData) =
        pd.details.map(d => d.percentTookDown).getOrElse(0.0)
    },
    new PercentColumn("GoodResult", "% Good") {
      def getValue(pd: PairData) =
        pd.details
          .map(d =>
            d.percentTookDown * d.percentDefended / 100 + d.percentMade * d.percentDeclared / 100
          )
          .getOrElse(0.0)
    },
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      props.filter.pairsData match {
        case Some(pd) =>
          val cols =
            (if (props.showPairs) pairColumns else peopleColumns) ::: columns
          ...
          val totals = if (props.showPairs) {
            ...
            val tpd = pds
              .map(pd => pd.details)
              .foldLeft(DuplicateSummaryDetails.zero(Team.idNul)) { (ac, v) =>
                v.map(vv => ac.add(vv)).getOrElse(ac)
              }
            val totalpd = pds.head.copy(
              player1 = "Totals",
              player2 = "",
              details = Some(tpd)
            )
          ...
          <.div(
            if (props.showPairs) dupStyles.viewPairsDetailsTable
            else dupStyles.viewPeopleDetailsTable,
            Table(
              cols,
              rows,
              Some("GoodResult"),
              ...
              caption = Some(
                TagMod(
                  if (props.showPairs) "Pairs Hand Results"
                  else "Player Hand Results"
                )
              )
            )
          )
  ...
  private[duplicate] val component = ScalaComponent
    .builder[Props]("ViewPairsMadeDownTable")
