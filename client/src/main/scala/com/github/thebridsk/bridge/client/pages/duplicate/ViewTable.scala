...
object ViewTable {
  ...
  def apply(
    ...
    component(
      Props(routerCtl, page, showTableButton)
...
object ViewTableInternal {
  ...
  private[duplicate] val Header = ScalaComponent
    .builder[(Props, Boolean)]("ComponentBoard.Header")
    .render_P { args =>
  ...
  private[duplicate] val RoundRow = ScalaComponent
    .builder[(Round, MatchDuplicateScore, Props, Int, Boolean)](
            ...
            .map { board =>
              val clickFromBoard =
                if (allUnplayed) {
                  ...
                  if (board.hasTeamPlayed(round.ns.id)) {
                    ...
                    props.page
                      .toBoardView(round.round, board.id)
                      .toHandView(round.ns.id)
                  }
                }
              ...
              <.span(
                ...
                AppButton(
                  ...
                  dupStyles.boardButtonInTable,
                  ^.disabled := round.round > currentRound,
                  (round.round == currentRound && !board
                    .hasTeamPlayed(round.ns.id)) ?= baseStyles.requiredNotNext,
                  ...
                  <.caption(
                    if (props.showTableButton) {
                      ...
                      AppButton(
                        "Table_" + table,
                        "Table " + table,
                        props.routerCtl.setOnClick(props.page)
                      )
                    } else {
                  ...
                  <.tbody(
                    {
                      ...
                      rounds
                        .sortWith((r1, r2) => r1.round < r2.round)
                        .map { round =>
                          if (round.complete) lastRoundAllPlay = round.round
                          RoundRow.withKey("Round_" + round.round)(
                            (round, score, props, lastRoundAllPlay + 1, relay)
                          )
                        }
                        .toTagMod
                    }
