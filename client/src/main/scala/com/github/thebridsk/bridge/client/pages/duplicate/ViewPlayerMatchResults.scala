...
object ViewPlayerMatchResult {
  case class Props(
      score: List[MatchDuplicateScore.Place],
      mdr: Option[MatchDuplicateResult],
      iws: Int,
      nws: Int,
  ...
  def apply(
      score: List[MatchDuplicateScore.Place],
      mdr: MatchDuplicateResult,
      iws: Int,
      nws: Int,
      useIMPs: Boolean = false
  ) = // scalafix:ok ExplicitResultTypes; ReactComponent
    ViewPlayerMatchResultInternal.component(
      Props(score, Some(mdr), iws, nws, useIMPs)
    )
              ...
              <.tr(
                ...
                <.td(
                  (if (props.useIMPs) f"${place.score}%.1f"
                   else Utils.toPointsString(place.score))
                ),
