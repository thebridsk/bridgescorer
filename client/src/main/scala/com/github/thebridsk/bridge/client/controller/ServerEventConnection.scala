...
abstract class ServerEventConnection[T <: Id[_]](val listener: SECListener[T]) {
  ...
  def delayStop(): Unit = {
    ...
    logger.fine(
      s"DelayStop: Requesting stop monitoring duplicate match on server in 30 seconds: ${monitoredId}"
    )
    if (monitoredId.isDefined) {
      ...
      delayHandle = Some(setTimeout(30000) { // note the absence of () =>
        delayHandle = None
        logger.fine(
          s"DelayStop: Stopping monitoring duplicate match on server: ${monitoredId}"
        )
        stop()
      })
    }
  ...
  def getDuplexPipeServerEventConnection()
      : Option[DuplexPipeServerEventConnection[T]]
}
