...
object TrickPieChart {
  ...
  val allMadeColors: List[HSLColor] =
    for (
      hue <- 120 :: 150 :: 180 :: Nil;
  ...
  val allDownColors: List[HSLColor] =
    for (
  ...
  def description(
      maxDown: Int,
      maxMade: Int,
      nopassedout: Boolean = false
  ): TagMod = {
    val p =
      if (nopassedout) (maxDown to maxMade).toList
