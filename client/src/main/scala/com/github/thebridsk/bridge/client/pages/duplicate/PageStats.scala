  ...
  protected object Internal {
    ...
    case class State(
      ...
      def toggle(view: ShowView): State = {
        ...
        val newsv =
          if (r.length == showViews.length) view :: showViews
          else r
    ...
    class Backend(scope: BackendScope[Props, State]) {
      ...
      val toggleShowPeopleTable: Callback =
        scope.modState(s => s.toggle(ShowPeopleTable))

      val toggleShowPairs: Callback = scope.modState(s => s.toggle(ShowPairs))

      val toggleShowPeopleTableDetail: Callback =
        scope.modState(s => s.toggle(ShowPeopleTableDetail))

      val toggleShowPairsDetail: Callback =
        scope.modState(s => s.toggle(ShowPairsDetail))

      val toggleShowPairsGrid: Callback =
        scope.modState(s => s.toggle(ShowPairsGrid))

      val toggleShowMadeDownGrid: Callback =
        scope.modState(s => s.toggle(ShowMadeDownGrid))

      val togglePlayerOpponentsStatsTable: Callback = scope.modState { s =>
        getDuplicateStats(
          s.toggle(ShowPlayerOpponentsStatsTable),
          playersOpponentsStats =
            s.stats.map(cs => cs.playersOpponentsStats.isEmpty).getOrElse(true)
        )
      }
      ...
      val toggleShowPlayerContractResults: Callback = scope.modState { s =>
        getDuplicateStats(
          ...
          contractStats =
            s.stats.map(cs => cs.contractStats.isEmpty).getOrElse(true)
        )
      }
      ...
      val toggleShowPlayerDoubledContractResults: Callback = scope.modState { s =>
        getDuplicateStats(
          ...
          playerDoubledStats =
            s.stats.map(cs => cs.playerDoubledStats.isEmpty).getOrElse(true),
          contractStats =
            s.stats.map(cs => cs.contractStats.isEmpty).getOrElse(true)
        )
      }
      ...
      val toggleShowPlayerAggressiveness: Callback = scope.modState { s =>
        getDuplicateStats(
          ...
          comparisonStats =
            s.stats.map(cs => cs.comparisonStats.isEmpty).getOrElse(true)
        )
      }
      ...
      def getPlayerPlaces(
        ...
        if (
          s.isVisible(ShowPlayerPlacesGraph)
          && (s.stats.isEmpty || s.stats.get.playerPlacesStats.isEmpty)
        ) {
          RestClientDuplicatePlayerPlaces.get("").recordFailure().onComplete {
            tpp =>
              scope.withEffectsImpure.modState { ss =>
                tpp match {
                  case Success(ppStats) =>
                    ...
                    val ds =
                      DuplicateStats(None, None, None, None, None, Some(ppStats))
                    ss.copy(stats =
                      ss.stats.map(sss => sss.update(ds)).orElse(Option(ds))
                    )
                  case _ =>
              ...
              scope.withEffectsImpure.modState { s =>
                result match {
                  case Right(stats) =>
                    s.copy(stats =
                      s.stats
                        .map(s => s.update(stats.duplicatestats))
                        .orElse(Option(stats.duplicatestats))
                    )
                  case Left(error) =>
      ...
      def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
        ...
        def optionalStats1View[S](
            view: (S) => TagMod,
            stat: DuplicateStats => Option[S]
        ): TagMod = {
          state.stats
            .map { cs =>
              stat(cs)
        ...
        def optionalStats2View[S, T](
            view: (S, T) => TagMod,
            statS: DuplicateStats => Option[S],
            statT: DuplicateStats => Option[T]
        ) = {
          state.stats
            .map { cs =>
              statS(cs)
          ...
          <.div(
              ...
              <.div(
                ...
                AppButton(
                  ...
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPeopleTable)
                  ),
            ...
            <.div(
              ...
              <.div(
                ...
                AppButton(
                  ...
                  "Show Made/Down",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowMadeDownGrid)
                  ),
                ...
                AppButton(
                  ...
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPeopleTableDetail)
                  ),
                ...
                AppButton(
                  ...
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPairsDetail)
                  ),
            ...
            <.div(
              ...
              <.div(
                baseStyles.divFooterLeft,
                AppButton(
                  "ShowPlayerOpponentGrid",
                  "Show Opponent Graph",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPlayerOpponentsStatsGraph)
                  ),
                  ^.onClick --> togglePlayerOpponentsStatsGraph
                ),
                AppButton(
                  "ShowPlayerOpponentResults",
                  "Show Opponent Results",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPlayerOpponentsStatsTable)
                  ),
                  ^.onClick --> togglePlayerOpponentsStatsTable
                ),
                AppButton(
                  "ShowPlayerOpponentPairsResults",
                  "Show Opponent Pairs Results",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPlayerOpponentsPairsStatsTable)
                  ),
            ...
            <.div(
              ...
              <.div(
                ...
                AppButton(
                  ...
                  "Show Contracts",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowContractResults)
                  ),
                  ^.onClick --> toggleShowContractResults
                ),
                AppButton(
                  "ShowPlayerContractResults",
                  "Show Player Contracts",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPlayerContractResults)
                  ),
                  ^.onClick --> toggleShowPlayerContractResults
                ),
                AppButton(
                  "ShowPlayerDoubledContractResults",
                  "Show Player Doubled Contracts",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPlayerDoubledContractResults)
                  ),
            ...
            <.div(
              ...
              <.div(
                ...
                AppButton(
                  ...
                  "Show Player Places",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPlayerPlacesGraph)
                  ),
                ...
                AppButton(
                  ...
                  "Aggressive Stats",
                  BaseStyles.highlight(selected =
                    state.isVisible(ShowPlayerAggressiveness)
                  ),
              ...
              val v: TagMod = view match {
                case ShowPairsGrid =>
                  optionalView(
                    ...
                    state.filter.pairsData
                  )
                case ShowMadeDownGrid =>
                  optionalView(
                    ViewPairsMadeDownGrid(state.filter),
                    state.filter.pairsData
                  )
                case ShowPlayerOpponentsStatsTable =>
                  optionalStatsView(cs =>
                    ViewPlayerOpponentStatsTable(
                ...
                case ShowPlayerContractResults =>
                  optionalStats2View(
                    (s: PlayerStats, t: ContractStats) =>
                      ViewPlayerContractResults(s, t),
                    _.playerStats,
                    _.contractStats
                  )
                case ShowPlayerDoubledContractResults =>
                  optionalStats2View(
                    (s: PlayerStats, t: ContractStats) =>
                      ViewPlayerDoubledContractResults(s, t),
                ...
                case ShowPlayerPlacesGraph =>
                  ViewPlayerPlacesGraph(
                    state.stats.flatMap(s => s.playerPlacesStats)
                  )
                case ShowPeopleTable => ViewPairsTable(state.filter, false)
                case ShowPairs       => ViewPairsTable(state.filter, true)
                case ShowPeopleTableDetail =>
                  ViewPairsMadeDownTable(state.filter, false)
      ...
      val storeCallback: Callback = scope.modState { s =>
        val pd = DuplicateSummaryStore.getDuplicateSummary.map { lds =>
          new PairsData(lds)
        }
      ...
      val didMount: Callback = Callback {
        ...
        Controller.getSummary(() =>
          scope.withEffectsImpure.modState(s =>
            s.copy(msg = Some("Error getting duplicate summary"))
          )
        )
      }
