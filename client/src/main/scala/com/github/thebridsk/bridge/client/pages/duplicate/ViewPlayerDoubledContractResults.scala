...
object ViewPlayerDoubledContractResultsInternal {
  ...
  val contractTypeOrderWithPlayerStats: List[ContractType] =
    ContractTypePartial :: ContractTypeDoubledToGame :: ContractTypeGame :: ContractTypeSlam :: ContractTypeGrandSlam :: Nil
  ...
  val order: List[ContractType] =
    ContractTypeTotal :: ContractTypePartial :: ContractTypeDoubledToGame :: ContractTypeGame :: ContractTypeSlam :: ContractTypeGrandSlam :: Nil
  def genData(
  ...
  val toGame: List[String] =
    "2N" :: "2H" :: "2S" :: "3H" :: "3S" :: "3C" :: "3D" :: "4C" :: "4D" :: Nil
  ...
  def genTotalStats(stats: ContractStats): (List[ContractStat], Int, Int) = {
    ...
    val almostAll = (extraStats ::: addDoubledToGame(stats.data))
  ...
  class Backend(scope: BackendScope[Props, State]) {
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      ...
      val rows = players.map { p =>
        ...
        val defender = defenderWithPass.filter(p =>
          p.contractType != ContractTypePassed.value
        )
        ...
        TagMod(p) :: byType(declarer, "Declarer", Some(p)) :: byType(
          defender,
          "Defender",
          Some(p)
        ) :: data
  ...
  private[duplicate] val component = ScalaComponent
    .builder[Props]("ViewPlayerDoubledContractResults")
