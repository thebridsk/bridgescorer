...
object ViewPlayersQuintetInternal {
  ...
  case class State(
      ...
      nextSittingOut: Option[
        String
      ], // for fair rotation selecting next person out
    ...
    def fairRotation(nextSittingOut: String): State = {
      ...
      copy(
        sittingOut = n.sittingOut,
        north = n.north,
        south = n.south,
        east = n.east,
        west = n.west,
        nextSittingOut = Some(nextSittingOut),
        swapping = swap
      )
    }
  }
...
//  val colors = ("lightgreen", "aquamarine")::("aqua", "paleturquoise")::Nil
  ...
  val colors: List[(TagMod, TagMod)] = (
    baseStyles.baseColor2,
    baseStyles.baseColor2
  ) :: (baseStyles.baseColor3, baseStyles.baseColor3) :: Nil
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      ...
      def showPlayerWithColor(
          name: String,
          colormap: Map[String, TagMod] = playerColors
      ) = {
        <.span(
      ...
      <.div(
        ChicagoPageBridgeAppBar(
          title = Seq[CtorType.ChildArg](
            MuiTypography(
              ...
              <.span(
                if (state.isSimple()) "Simple Rotation"
                else "Fair Rotation"
              )
            )
          ),
          helpurl =
            if (state.isSimple())
              "../help/chicago/fastsimple/selectnamessimple.html"
            else "../help/chicago/fastfair/selectnamesfair.html",
        ...
        <.div(
          ...
          <.div(
            ...
            showNewPositions(
              state.isSimple() || state.nextSittingOut.isDefined,
              "Next hand",
              dealer,
              state.north,
              state.south,
  ...
  private[chicagos] val component = ScalaComponent
    .builder[Props]("ViewPlayersQuintet")
