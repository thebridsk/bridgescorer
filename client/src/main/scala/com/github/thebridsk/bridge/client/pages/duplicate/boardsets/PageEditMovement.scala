...
object PageEditMovementInternal {
  ...
  case class State(
    ...
    def setName(name: Movement.Id): State =
      copy(movement = Some(getMovement.copy(name = name)))
    def setShort(description: String): State =
      copy(movement = Some(getMovement.copy(short = description)))
    def setDescription(description: String): State =
      copy(movement = Some(getMovement.copy(description = description)))
    ...
    private def adjustHands(newTables: Int, newRounds: Int) = {
      val mov = getMovement
      logger.fine(
        s"Entry: newTables=$newTables/$ntables, newRounds=$newRounds/$nrounds, mov=$mov"
      )
      if (newTables <= 0 || newRounds <= 0) {
        logger.fine(
          s"Tables or rounds is zero: newTables=$newTables/$ntables, newRounds=$newRounds/$nrounds, mov=$mov"
        )
        copy(
          ntables = newTables,
          nrounds = newRounds,
          movement = Some(getMovement.copy(hands = List()))
        )
      } else {
        val step1 = if (newTables < ntables || newRounds < nrounds) {
          logger.fine(
            s"Removing tables or rounds: newTables=$newTables, newRounds=$newRounds, mov=$mov"
          )
          mov.hands.filter { hit =>
            hit.table <= newTables && hit.round <= newRounds
          }
        } else {
          mov.hands
        }
        val step2 = if (newTables > ntables || newRounds > nrounds) {
          val step2a = if (newTables > ntables) {
            logger.fine(
              s"Adding tables: newTables=$newTables, newRounds=$newRounds, mov=$step1"
            )
            step1 ::: (for {
          ...
          if (newRounds > nrounds) {
            logger.fine(
              s"Adding rounds: newTables=$newTables, newRounds=$newRounds, mov=$step2a"
            )
            step2a ::: (for {
              t <- (1 to ntables)
    ...
    def setBoards(table: Int, round: Int, boards: String): State = {
      ...
      val ns =
        try {
          ...
          logger.fine(
            s"setBoards table=$table, round=$round, boards=$boards, b=${b.mkString(",")}"
          )
          set(table, round, hit => hit.copy(boards = b))
            .copy(boards = this.boards + ((table, round) -> boards))
        } catch {
          ...
          if (mov.short != null && mov.short != "") {
            mov.hands
              .find { hit =>
                ...
                hit.boards.isEmpty || hit.boards
                  .find(b => b <= 0 || b > nboards)
                  .isDefined
              }
              .toLeft(1)
              .toOption
  ...
  case class VulStat(
    ...
    def add(v: VulStat): VulStat =
      copy(
        bothVul = bothVul + v.bothVul,
        meVul = meVul + v.meVul,
        oppVul = oppVul + v.oppVul,
        neitherVul = neitherVul + v.neitherVul
      )

    def swapTeams: VulStat =
      copy(me = opp, opp = me, meVul = oppVul, oppVul = meVul)
  ...
  class Stats(state: State) {
    ...
    def getPairingVul(): Option[Map[(Int, Int), VulStat]] = {
      state.boardset.map { bs =>
        ...
        state.getMovement.hands
          .map { hit =>
            val ns = hit.ns
            val ew = hit.ew
            hit.boards
              .foldLeft(VulStat(ns, ew)) { (ac, boardn) =>
  ...
  class Checker(state: State) {
    ...
    state.getMovement.hands.foreach { hit =>
      ...
      hit.boards.foreach { b =>
        if (b <= 0 || b > state.nboards)
          errors += s"Table ${hit.table} round ${hit.round} board is out of range: $b"
        else {
          if (hit.ns > state.nteams)
            errors += s"Table ${hit.table} round ${hit.round} north-south team number is out of range: ${hit.ns}"
          else if (hit.ns > 0) played(hit.ns - 1)(b - 1) += 1
          else
            errors += s"North-south team not defined on table ${hit.table} round ${hit.round}"

          if (hit.ew > state.nteams)
            errors += s"Table ${hit.table} round ${hit.round} east-west team number is out of range: ${hit.ew}"
          else if (hit.ew > 0) played(hit.ew - 1)(b - 1) += 1
          else
            errors += s"East-west team not defined on table ${hit.table} round ${hit.round}"
        }
      }
    }
  ...
  private[boardsets] val TableCaption = ScalaComponent
    .builder[(Props, State, Int)]("PageEditMovement.TableCaption")
    .render_P { args =>
  ...
  private[boardsets] val TableHeader = ScalaComponent
    .builder[(Props, State)]("PageEditMovement.TableHeader")
    .render_P { args =>
  ...
  private[boardsets] val TableRow = ScalaComponent
    .builder[
      (
          Props,
          HandInTable,
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def inputCB(data: ReactEventFromInput): Callback =
      data.inputText(text => scope.modState(_.setName(Movement.id(text))))
    def shortCB(data: ReactEventFromInput): Callback =
    ...
    def setNTeamsCB(data: ReactEventFromInput): Callback =
      data.inputText { text =>
        scope.modState { s =>
          try {
    ...
    def setNTablesCB(data: ReactEventFromInput): Callback =
      data.inputText { text =>
        scope.modState { s =>
          try {
            val n = stringToInt(text)
    ...
    def setNRoundsCB(data: ReactEventFromInput): Callback =
      data.inputText { text =>
        scope.modState { s =>
          try {
            val n = stringToInt(text)
    ...
    def setNSTeam(table: Int, round: Int)(data: ReactEventFromInput): Callback =
      data.inputText { text =>
        val nsTeam = stringToInt(text)
        scope.modState(s => s.setNSTeam(table, round, nsTeam))
      }
    def setEWTeam(table: Int, round: Int)(data: ReactEventFromInput): Callback =
      data.inputText { text =>
        val ewTeam = stringToInt(text)
        scope.modState(s => s.setEWTeam(table, round, ewTeam))
      }
    def setBoards(table: Int, round: Int)(data: ReactEventFromInput): Callback =
      data.inputText { boards =>
    ...
    val clickOk: Callback = scope.modState(
      { s =>
        s.setMsg(
          s.movementId
            .map(i => s"Updating movement $i")
            .getOrElse("Creating new movement")
        )
      },
      Callback {
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      ...
      val listoftables =
        if (vlistoftables.isEmpty || vlistoftables.head.isEmpty) List()
        else vlistoftables
            ...
            TagMod(
              <.div(
                ...
                CheckBox(
                  id = "Disabled",
                  text = "Disabled",
                  value = state.getMovement.isDisabled,
                    ...
                    data = {
                      ...
                      if (state.gotBoardsets) {
                        val ddl = js
                          ...
                          .toList
                        Some(
                          js.Array((nullboardsetToDropdownListValue :: ddl): _*)
                        )
                      } else {
                        None
                      }
                  ...
                  <.div(
                    <.table(
                      ...
                      <.tbody(
                        rounds.map { round =>
                          val rid = round.round
                          TableRow.withKey(s"T${round.table}_R${round.round}")(
                            (
                              props,
                              round,
                              state.boards.getOrElse((table, rid), ""),
                              setNSTeam(table, rid),
                              setEWTeam(table, rid),
                              setBoards(table, rid)
                            )
                          ...
                          <.tr(
                            ...
                            (
                              (1 to state.nteams).map { c =>
                                ...
                                val (key, swap: Swapper) =
                                  if (r < c) ((r, c), (vv: VulStat) => vv)
                                  else ((c, r), (vv: VulStat) => vv.swapTeams)
                                val v = played.getOrElse(key, 0)
                                val p = pair
                                  .map { p =>
                                    p.get(key)
      ...
      props.page match {
        case mev: MovementEditView =>
          ...
          BoardSetStore.getMovement(display) match {
            case Some(mov) =>
              val mov1 =
                if (mov.isDeletable) mov
                else mov.copy(resetToDefault = Some(true))
              Some(
                state.copy(
                  movement = Some(mov1),
    ...
    val didMount: Callback = scope.stateProps { (state, props) =>
      Callback {
          ...
          tr match {
            ...
            case Failure(ex) =>
              scope.withEffectsImpure
                .modState(_.setMsg("Unable to get list of boardsets"))
          }
        }
        props.page match {
          case mev: MovementEditView =>
  ...
  private[boardsets] val component = ScalaComponent
    .builder[Props]("PageEditMovement")
