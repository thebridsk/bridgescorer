...
object ViewPairsMadeDownGridInternal {
  ...
  def size(
    ...
    if (vmax == vmin) sizemax
      ...
      ((v.toDouble - vmin) * (sizemax - sizemin) / (vmax - vmin) + sizemin).toInt
  }
  ...
  object ColorByPlayedResults extends ColorBy {
    ...
    def value(pd: PairData): Double = {
      val x =
        pd.details.map { d => d.declarer + d.defended + d.passed }.getOrElse(0)
      x
    }
      ...
      props.filter.pairsData match {
        case Some(pds) if !pds.players.isEmpty =>
          val summary = new PairsDataSummary(
            pds,
            ColorByDeclarerResults,
            props.filter.selected,
            props.filter.filterDisplayOnly,
            ColorByDefendedResults,
            ColorByPlayedResults,
            ColorByPassedResults
          )
          ...
          val vMin =
            Math.min(statDec.min, Math.min(statDef.min, statPassed.min))
          val vMax =
            Math.max(statDec.max, Math.max(statDef.max, statPassed.max))
          val vTotalMin = Math.min(
            statTotalDec.min,
            Math.min(statTotalDef.min, statTotalPassed.min)
          )
          val vTotalMax = Math.max(
            statTotalDec.max,
            Math.max(statTotalDef.max, statTotalPassed.max)
          )

          val allPlayers = summary.playerTotals
            .filter { e =>
            ...
            .sorted
          val sortedPlayers =
            props.filter.selected.map(l => l).getOrElse(allPlayers)

          def cellAll(pd: PairData, isTotals: Boolean): TagMod = {
            val (vmin, vmax) =
              if (isTotals) (statTotalPlayed.min, statTotalPlayed.max)
              else (statPlayed.min, statPlayed.max)
            pd.details match {
              case Some(det) =>
                val sizeAll =
                  size(det.total, vmin, vmax, state.minSize, state.maxSize)
                MadeDownPieChart(
                  made = det.made,
                  down = det.down,
          ...
          val shownSorted =
            sortedPlayers.filter(e => props.filter.isPlayerShown(e))

          val columns =
            Column("Player") :: shownSorted.map(p => Column(p)) ::: List(
              Column("Totals")
            )
          ...
          val rows = shownSorted.map { rowPlayer =>
            ...
            val pdTotal = summary.playerTotals
              .get(rowPlayer)
              .getOrElse(
                PairData(
                  rowPlayer,
  ...
  private[duplicate] val component = ScalaComponent
    .builder[Props]("ViewPairsMadeDownGrid")
