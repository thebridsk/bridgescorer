...
object ComponentInputStyleButton {
  ...
  class Backend(scope: BackendScope[Props, State]) {
    def render(props: Props) = { // scalafix:ok ExplicitResultTypes; ReactComponent
      import HandStyles._
      Button(
        if (props.useHandStyle) handStyles.footerButton
        else baseStyles.appButton,
        "InputStyle",
    ...
    } >> scope.forceUpdate >> scope.props >>= { props =>
      props.callback
    }
  }

  private[hand] val component = ScalaComponent
    .builder[Props]("ComponentInputStyleButton")
