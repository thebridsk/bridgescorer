...
object ViewRoundTableInternal {
      ...
      <.table(
        ...
        <.tbody(
          (0 until byHands.length).map { i =>
            ...
            summaryHandRow.withKey(key)(
              (roundNumber, i, Some(v), dealer, props)
            )
          }.toTagMod,
  ...
  private[chicagos] val summaryHandRow = ScalaComponent
    .builder[
      (Int, Int, Option[DuplicateBridge.ScoreHand], PlayerPosition, Props)
    ]("SummaryHandRow")
    .render_P(myprops => {
      val (round, hand, scores, dealer, props) = myprops
      def scoreOrBlank(v: Int) = if (v > 0) v.toString(); else ""

      def playerAtPos(pos: PlayerPosition) =
        pos match {
        ...
        <.td(
          if (scores.isDefined) {
            AppButton(
              "Hand_" + (hand + 1),
              (hand + 1).toString(),
              props.routerCtl.setOnClick(props.page.toHandView(hand))
            )
          } else {
            (hand + 1).toString()
          }
