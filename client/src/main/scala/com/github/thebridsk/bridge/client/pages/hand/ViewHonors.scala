...
object ViewHonors {
  ...
  case class Props(
      ...
      currentPlayer: Option[PlayerPosition],
  ...
  private val component = ScalaComponent
    ...
    .render_P(props /* (props,state,backend) */ => {
      ...
      def show() = {
        val r =
          ComponentInputStyleButton.inputMethod != InputMethod.Prompt || PageHandNextInput.InputHonors <= props.nextInput
        val shbt =
      ...
      def showP() = {
        val r =
          ComponentInputStyleButton.inputMethod != InputMethod.Prompt || PageHandNextInput.InputHonorsPlayer <= props.nextInput
        val shbt =
          props.currentContractTricks.isDefined && props.currentContractTricks.get.tricks > 0 &&
            props.currentSuit.isDefined
        r && shbt
      ...
      def isSelected(d: Int) =
        props.currentHonors.isDefined && props.currentHonors.get == d
      def isPlayerSelected(d: PlayerPosition) =
        props.currentPlayer.isDefined && props.currentPlayer.get == d
      ...
      val notrump =
        props.currentSuit.isDefined && props.currentSuit.get == NoTrump
      <.div(
        handStyles.viewHonors,
        <.div("Honors:"),
        <.div(
          <.div(
          ...
          <.div(
            !(props.currentHonors
              .getOrElse(0) > 0 && showP()) ?= handStyles.notVisible,
            // HandStyles.highlight(
            //     required = hpMissingRequired,
            //     requiredNotNext = hpMissingNotNext
            // ),
