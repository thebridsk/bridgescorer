...
object SectionScore {
  ...
  def apply(contract: Contract, nextInput: PageHandNextInput.Value) =
    component(
      Props(contract, nextInput)
    ) // scalafix:ok ExplicitResultTypes; ReactComponent

  private val component = ScalaComponent
    .builder[Props]("SectionScore")
    .render_P(props => {
      import HandStyles._
      <.div(
        ...
        props.contract.scorer match {
          case Some(Left(score)) /* Rubber */ =>
            ...
            TagMod(
              <.div(
                "Score: " + score.totalScore(c.north, c.south, c.east, c.west)
              ),
              <.div(
          ...
          case Some(Right(score)) /* Duplicate */ =>
            ...
            val ts = c.scoringSystem match {
              case _: Chicago =>
                score.totalScoreNoPos(c.north, c.south, c.east, c.west)
