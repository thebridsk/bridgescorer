...
//    if (jqueryComponentInternal == null)
  ...
  private val PROCESSING_INSTRUCTION_NODE =
...
//        }
  ...
  def findTagById(tag: String, id: String): Element = {
    ...
    assert(
      r.length == 1,
      "Did not find only one element named " + tag + " with id " + id
    )
    r(0)
  }
  ...
  def clickTagById(tag: String, id: String): Unit = {
    ...
    assertFalse(
      isElementDisabled(c),
      "Tag " + tag + " with id " + id + " is disabled"
    )
  ...
  def setInputFieldById(id: String, value: String): Unit = {
    ...
    assertFalse(
      isElementDisabled(c),
      "Tag input with id " + id + " is disabled"
    )
    i.value = value
  ...
  def findTagByName(tag: String, name: String): Element = {
    ...
    assert(
      r.length == 1,
      "Did not find only one element named " + tag + " with name " + name
    )
    r(0)
  }
  ...
  def clickTagByName(tag: String, name: String): Unit = {
    ...
    assertFalse(
      isElementDisabled(c),
      "Tag " + tag + " with name " + name + " is disabled"
    )
    Simulation.click run c
  }
  ...
  def setInputFieldByName(name: String, value: String): Unit = {
    ...
    assertFalse(
      isElementDisabled(c),
      "Tag input with name " + name + " is disabled"
    )
    i.value = value
    SimEvent.Change(value) simulate c
  }
...
//    ReactTestUtils.findAllInRenderedTree(component, isComponentTag(name) _)
  ...
  def clickById(id: String): Unit = {
    ...
    if (comps.length == 0)
      fail("Did not find element with id " + id + " to click")
    if (comps.length > 1)
      fail("Find multiple elements with id " + id + " to click")
    val b = comps(0)
  ...
  trait WebElement {
    ...
    def click: Unit = {
      ...
      if (comps.length == 0)
        fail("Did not find element with " + name + " to click")
      else if (comps.length > 1) {
        if (
          comps.length == 2 && (comps(0) == comps(1) || js.isUndefined(
            comps(1)
          ))
        ) {
      ...
      if (comps.length > 1) {
        if (comps.length == 2 && comps(0) == comps(1)) {
          ...
          if (comps(0) /* .getDOMNode() */.isInstanceOf[HTMLElement]) i = 0
            ...
            println(
              "Find multiple elements with " + name + " no HTMLElement: " + comps(
                0
              ) + " " + comps(1)
            )
...
//      val n = ReactDOM.findDOMNode(c)
    ...
    def findField: HTMLInputElement =
      findElement.get.asInstanceOf[HTMLInputElement]
  ...
  class MustMatcher[A](left: A) {
    def mustBe(right: A): Unit =
      if (left != right) fail("Left " + left + " does not equal right " + right)
  }
  implicit def toMustMatchers[A](left: A): MustMatcher[A] =
    new MustMatcher(left)
