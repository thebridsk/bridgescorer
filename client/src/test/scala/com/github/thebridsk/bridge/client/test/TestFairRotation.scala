...
object TestFairRotation {
  ...
  val allPlayers: List[String] =
    pNorth :: pSouth :: pEast :: pWest :: pExtra :: Nil
}
...
class TestFairRotation extends AnyFlatSpec with Matchers {
  ...
  def checkForDuplicates(pairs: List[(String, String)]): Unit = {
    ...
    def check(pairs: List[(String, String)]): Unit = {
      pairs match {
        ...
        case head :: Nil =>
          withClue(
            s"All pairs must be with two people, found $head, in $pairs"
          ) {
    ...
    allPlayers.foreach { p =>
      val pairsWithPlayer = pairs.filter { pair =>
        pair._1 == p || pair._2 == p
      }
  ...
  it should "test all combinations of sitting out in fair rotation" in {
    val r0 = Round.create("0", pNorth, pSouth, pEast, pWest, North.pos, Nil)
    val mc0 =
      MatchChicago(MatchChicago.id(1), List("", "", "", ""), Nil, 0, false)
        .setPlayers(pNorth, pSouth, pEast, pWest)
        .playChicago5(pExtra)
        .setQuintet(false)
        .addRound(r0)
    ...
    def play(
        ...
        notYetSittingOut: List[String],
        results: List[List[(String, String)]]
      ...
      if (notYetSittingOut.isEmpty) {
        ...
        notYetSittingOut.foldLeft(results) {
          case (r, nextSittingOut) =>
            val nextNotYetSittingOut =
              notYetSittingOut.filter(_ != nextSittingOut)
            val nextState = state.fairRotation(nextSittingOut)
            val nextPairs = (nextState.north, nextState.south) :: (
              nextState.east,
              nextState.west
            ) :: pairs
