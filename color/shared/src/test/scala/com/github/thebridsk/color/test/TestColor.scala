...
class TestColor extends AnyFlatSpec with Matchers {
  ...
  it should "convert a string to an HSLColor object" in {
    ...
    Color("hsl( 1rad 50% 50% / .5)") mustBe HSLColor(
      1 / Math.PI * 180,
      50,
      50,
      50
...
//        println( s"${rgb} ok, ${hsl}, ${back}" )
  ...
  def testRgbHslPerRgb(rgb: RGBColor): Unit = {
    ...
    if (rgb != back) {
      fail(
        s"${rgb} did not convert back through hsl->per, ${hsl}, ${per}, ${back}"
      )
    } else {
//        println( s"${rgb} ok, ${hsl}, ${back}" )
    }
  ...
  def testRgbPerHslRgb(rgb: RGBColor): Unit = {
    ...
    if (rgb != back) {
      fail(
        s"${rgb} did not convert back through per->hsl, ${per}, ${hsl}, ${back}"
      )
    } else {
//        println( s"${rgb} ok, ${hsl}, ${back}" )
    }
  ...
  it should "Categorize all the color names" in {
    val allNamedColors = NamedColor.namedColors.map {
      case (name, hex) =>
        val color = Color(hex).toHSLColor
        val c = color.hue
        (name, color, c)
    }.toList
    val grouped = allNamedColors
      .groupBy {
        case (name, color, c) =>
    ...
    println(
      s"There are ${allNamedColors.length} color names, with ${grouped.size} colors"
    )

    val s = grouped
      .map {
        case (c, list) =>
          val colors = list.sortBy(e => (e._2.saturation, e._2.lightness)).map {
            case (name, color, hue) =>
              f"${name}(${color.saturation}%.2f%%,${color.lightness}%.2f%%)"
          }
  ...
  it should "Show ghostwhite and blue in hsl" in {
    ...
    println(
      s"ghostwhite saturation=${ghostwhite.saturation}, blue saturation=${blue.saturation}"
    )
