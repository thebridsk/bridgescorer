...
case class Gray( gray: Double, alpha: Double = 100 ) extends Color {
  ...
  def toLab: Lab = Lab( gray, 0, 0, alpha )
...
object Lab {
  ...
  val white: Array[Double] = Array(0.9642, 1.0000, 0.8249)   // D50 reference white
}
...
case class SRGB( r: Double, g: Double, b: Double, alpha: Double = 100 ) {
  ...
  def gammaEncoding: RGBPercentColor = {
...
case class Matrix( rows: Array[Double]* ) {
   ...
   */
  def multiply( vector: Array[Double] ): Seq[Double] = {
    rows.map { row =>
