  ...
  def sum( other: PlayerOpponentsStat) = {
    ...
    PlayerOpponentsStat(
      ...
      allOpponents.map { opp =>
        ...
        thisOp.map( top =>
          otherOp.map( oop => oop.sum(top) ).getOrElse(top)
  ...
  def sum( other: PlayersOpponentsStats ) = {
    ...
    PlayersOpponentsStats(
      allPlayers.map { play =>
        ...
        thisPl.map( top =>
          otherPl.map( oop => oop.sum(top) ).getOrElse(top)
...
object PlayersOpponentsStats {
  ...
  def getBothStat(totalMatchPoints: Int, teamscores: Map[Id.Team, TeamBoardScore], team1: Team, team2: Team ) = {
    getStat(totalMatchPoints,teamscores,team1,team2):::getStat(totalMatchPoints,teamscores,team2,team1)
  }


  def getStat(totalMatchPoints: Int, teamscores: Map[Id.Team, TeamBoardScore], team1: Team, team2: Team ) = {
    PlayerOpponentsStat(
        team1.player1,
        new PlayerOpponentStat( team1.player1, team2.player1, totalMatchPoints, teamscores.get(team1.id).get.points.toInt )::
        new PlayerOpponentStat( team1.player1, team2.player2, totalMatchPoints, teamscores.get(team1.id).get.points.toInt )::
        Nil
    )::
    PlayerOpponentsStat(
        team1.player2,
        new PlayerOpponentStat( team1.player2, team2.player1, totalMatchPoints, teamscores.get(team1.id).get.points.toInt )::
        new PlayerOpponentStat( team1.player2, team2.player2, totalMatchPoints, teamscores.get(team1.id).get.points.toInt )::
        Nil
    )::
    Nil
  }

  def getBothTeamStat( teamscores: Map[Id.Team, Double], team1: Team, team2: Team ) = {
    getTeamStat(teamscores,team1,team2):::getTeamStat(teamscores,team2,team1)
  }


  def getTeamStat( teamscores: Map[Id.Team, Double], team1: Team, team2: Team ) = {

    val t1s = teamscores.get(team1.id).get
    val t2s = teamscores.get(team2.id).get

    val (tied,t1won,t2won) = if (t1s==t2s) (1,0,0) else if (t1s < t2s) (0,0,1) else (0,1,0)

    PlayerOpponentsStat(
        team1.player1,
//                                              played,beat,tied
        new PlayerOpponentStat( team1.player1, team2.player1, 1,t1won,tied, 0,0 )::
        new PlayerOpponentStat( team1.player1, team2.player2, 1,t1won,tied, 0,0 )::
        Nil
    )::
    PlayerOpponentsStat(
        team1.player2,
        new PlayerOpponentStat( team1.player2, team2.player1, 1,t1won,tied, 0,0 )::
        new PlayerOpponentStat( team1.player2, team2.player2, 1,t1won,tied, 0,0 )::
        Nil
    )::
    Nil
  }
  ...
  def stats( dups: Map[Id.MatchDuplicate,MatchDuplicate] ): PlayersOpponentsStats = {
    val ss = dups.values.flatMap { md =>
      val score = MatchDuplicateScore(md,PerspectiveComplete)
      if (score.alldone) {

        val teams = md.teams.map( t => (t.id,t)).toMap

        val list = score.boards.values.flatMap { b =>
          ...
          if (b.board.hands.length == 2) {
            ...
            getBothStat(2, teamscores, teams.get(h1ns).get, teams.get(h1ew).get) :::
            getBothStat(2, teamscores, teams.get(h1ns).get, teams.get(h2ns).get) :::
            getBothStat(2, teamscores, teams.get(h2ew).get, teams.get(h2ns).get) :::
            getBothStat(2, teamscores, teams.get(h2ew).get, teams.get(h1ew).get)
        ...
        val teamlist = md.teams.flatMap {t1 =>
          md.teams.flatMap { t2 =>
            if (t1 == t2) Nil
            else {
