...
case class MatchDuplicateV3 private(
  ...
  def equalsInId( other: MatchDuplicateV3, throwit: Boolean = false ) = {
    val rc = id == other.id
    if (!rc&&throwit) throw new Exception(s"MatchDuplicateV3 other did not have id equal to ${id}: ${other.id}")
    rc
  }
  ...
  def equalsInTeams( other: MatchDuplicateV3, throwit: Boolean = false ) = {
    if (teams.length == other.teams.length) {
      teams.find { t1 =>
        ...
        val rc = other.teams.find { t2 => t1.equalsIgnoreModifyTime(t2) }.isEmpty
        if (rc&&throwit) throw new Exception("MatchDuplicateV3 other did not have team equal to: "+t1)
        rc
      }.isEmpty
    } else {
      if (throwit) throw new Exception("MatchDuplicateV3 teams don't have same key: "+teams.map{t=>t.id}+" "+other.teams.map{t=>t.id})
      false
    }
  }
  ...
  def equalsInBoards( other: MatchDuplicateV3, throwit: Boolean = false ) = {
    if (boards.length == other.boards.length) {
      boards.find { t1 =>
        // this function must return true if t1 is NOT in other.team
        val rc = other.boards.find { t2 => t1.id == t2.id && t1.equalsIgnoreModifyTime(t2) }.isEmpty
        if (rc&&throwit) throw new Exception("MatchDuplicateV3 other did not have board equal to: "+t1)
        rc
      }.isEmpty
    } else {
      if (throwit) throw new Exception("MatchDuplicateV3 boards don't have same key: "+boards.map{t=>t.id}+" "+other.boards.map{t=>t.id})
      false
    }
  }
  ...
  def updateBoard( board: BoardV2 ): MatchDuplicateV3 = {
    val nb = boards.map{b=> if (b.id==board.id) (true,board) else (false,b) }
    val nb1 = nb.foldLeft((false,List[BoardV2]()))((ag,b) => ( ag._1||b._1, b._2::ag._2))
    val nb2 = if (nb1._1) nb1._2 else board::nb1._2
    copy( boards=nb2.sortWith(MatchDuplicateV3.sort), updated=SystemTime.currentTimeMillis() )
  }

  def setBoards( nboards: List[BoardV2] ) = {
    copy( boards=nboards.sortWith(MatchDuplicateV3.sort), updated=SystemTime.currentTimeMillis() )
  }

  def deleteBoard( boardid: Id.DuplicateBoard ) = {
    copy( boards=boards.filter{b=>b.id != boardid}, updated=SystemTime.currentTimeMillis() )
  }

  def getBoard( boardid: Id.DuplicateBoard ) = {
    boards.find { b => b.id==boardid }
  }

  def updateHand( hand: DuplicateHandV2 ): MatchDuplicateV3 = getBoard(hand.board) match {
  ...
  def updateHand( boardId: String, hand: DuplicateHandV2 ): MatchDuplicateV3 = getBoard(boardId) match {
    case Some(board) => updateBoard( board.updateHand(hand) )
    case None => throw new IndexOutOfBoundsException("Board "+boardId+" not found")
  }
  def updateHand( boardId: Id.DuplicateBoard, handId: String, hand: Hand ): MatchDuplicateV3 = getBoard(boardId) match {
    case Some(board) => updateBoard( board.updateHand(handId,hand) )
    case None => throw new IndexOutOfBoundsException("Board "+boardId+" not found")
  }

  def updateTeam( team: Team ): MatchDuplicateV3 = {
    copy( teams= teams.map { t=> if (t.id==team.id) team else t },
          updated=SystemTime.currentTimeMillis() )
  }

  def setTeams( nteams: List[Team] ) = copy( teams=nteams.sortWith(MatchDuplicateV3.sort), updated=SystemTime.currentTimeMillis() )

  def deleteTeam( teamid: Id.Team ) = copy( teams=teams.filter(t=>t.id!=teamid), updated=SystemTime.currentTimeMillis() )
  ...
  def getHand( tableid: Id.Table, round: Int, boardId: Id.DuplicateBoard ) = {
    getBoard(boardId) match {
      case Some(b) => b.hands.find { h => h.table==tableid&&h.round==round }
      case _ => None
    }
  }
   ...
   */
  def determinePlayerPositionFromCaller( hand: DuplicateHandV2, pN1: Boolean, pE1: Boolean ): MatchPlayerPosition = {
    var n = "Unknown"
    var s = n
    var e = n
    var w = n
   ...
   * @return (north,south,east,west, allspecified)
   */
  def determinePlayerPositionFromRound( tableid: Id.Table, round: Int ): MatchPlayerPosition = {
   ...
   * @return (north,south,east,west, allspecified)
   */
  def determinePlayerPositionFromBoard( tableid: Id.Table, round: Int, boardId: Id.DuplicateBoard ): MatchPlayerPosition = {
  ...
  def setPlayerPositionForRound( tableid: Id.Table, round: Int, north: String, south: String, east: String, west: String ) =
    getHandsInRound(tableid, round).headOption match {
      case Some(hand) =>
        val MatchPlayerPosition(cn,cs,ce,cw,allplayed,gotns,gotew) = determinePlayerPosition(hand)
   ...
   */
  def fixVulnerability( correctVulnerability: MatchDuplicateV3 ): (MatchDuplicateV3,List[String]) = {
    var msgs: List[String] = Nil
    val md: MatchDuplicateV3 = copy( boards=
      boards.map{ board =>
        val id = board.id
  ...
  def fillBoards( bs: BoardSet, mov: Movement ): MatchDuplicateV3 = {
    ...
    val brds = bs.boards.map( b => "B"+b.id->b ).toMap
    val bbb = mov.hands.flatMap { htp =>
      val ew = ("T"+htp.ew).asInstanceOf[Id.Team]
      val ns = ("T"+htp.ns).asInstanceOf[Id.Team]
      htp.boards.map { b =>
        val bid = "B"+b
        (DuplicateHandV2.create(htp.table.toString(),htp.round,bid,ns,ew),bid)
      }
    }.groupBy{ e => e._2 }.flatMap { e =>
      val (b,dhs) = e
      val hands = dhs.map{e=>e._1}
      if (!hands.isEmpty) {
        val board = brds.get(b).get
        Some(BoardV2.create( "B"+board.id, board.nsVul, board.ewVul, board.dealer, hands ))
      } else {
        None
      }
    }.toList.sortWith(MatchDuplicateV3.sort)

    copy( teams = useteams.sortWith(MatchDuplicateV3.sort), boards=bbb, boardset=bs.name, movement=mov.name, updated=SystemTime.currentTimeMillis() )

//    val filledB = scala.collection.mutable.Map[Id.DuplicateBoard, BoardV2]()
//
//    bs.boards.foreach { board =>
//      val bb = BoardV2.create( "B"+board.id, board.nsVul, board.ewVul, board.dealer, List() )
//      filledB += (bb.id->bb)
//    }
...
//        val hand = DuplicateHandV2.create(htp.table.toString(),htp.round,"B"+b,ns,ew)
//
//        val board = filledB.get(hand.board).get
//
//        val nb = board.updateHand(hand)
//        filledB += board.id->nb
//      }
//    }
//
...
//     name: String, short: String, description: String, boards: List[BoardInSet]
    BoardSet( boardset+"In"+id, "Used in match "+id, "Used in match "+id,bins)
  }
...
object MatchDuplicateV3 {
  ...
  def apply(
    ...
    created: Timestamp,
    updated: Timestamp ) = {
      new MatchDuplicateV3(id,teams.sortWith(sort),boards.sortWith(sort),boardset,movement,created,updated)
  }

  def createTeams( numberTeams: Int ) = {
    (1 to numberTeams).map( t => "T"+t ).map( id => Team.create(id,"","")).toList
  }

  def sort(l: BoardInSet,r: BoardInSet) = l.id<r.id
  def sort(l: BoardV2,r: BoardV2) = Id.idComparer(l.id, r.id)<0
  def sort(l: Team,r: Team) = Id.idComparer(l.id, r.id)<0
}
