...
object RubberBridge {
  ...
  class ScoreHand( id: String,
                   ...
                   tricks: Int,
                   val honors: Int = 0,
                   honorsPlayer: Option[PlayerPosition] = None) extends BridgeHand(id,contractTricks,contractSuit,contractDoubled,declarer,nsVul,ewVul,madeContract,tricks) {

    if (honors == 100 && contractSuit == NoTrump) throw new IllegalArgumentException
    val isDeclarerNorthSouth = declarer==North||declarer==South
    val isDeclarerEastWest = !isDeclarerNorthSouth

    val (nsScored,above,below,isGame,explainAbove,explainBelow) = calculate()
    ...
    def explain() = {
      def show( pos: String, list: List[String] ) = if (list.isEmpty) "" else pos+" "+list.mkString(", ")
      ...
      if (honors == 0) {
        ...
        if (nsScored == nsScoredHonors) {
          ...
          show(sc,explainAbove)+ ", "+scoring(nsScoredHonors)+" "+explainHonor+" / "+show(sc,explainBelow)
        }
      }
    }
    ...
    def scoring( ns: Boolean, north: String, south: String, east: String, west: String ) = {
    ...
    def totalScoreNoPos( north: String, south: String, east: String, west: String ) = {
    ...
    val contractAndResultAsString = if (contractTricks.tricks==0) "Passed Out"; else contractTricks.tricks.toString()+contractSuit.suit+contractDoubled.forScore+" "+
    ...
    def contractAsString( vul: String = "Vul", notvul: String = "NotVul" ) = {
      if (contractTricks.tricks==0) "Passed Out"; else contractTricks.tricks.toString()+contractSuit.suit+contractDoubled.forScore+" "+
          (if (isDeclarerVulnerable) vul; else notvul)
    }

    def contractAsStringNoVul = if (contractTricks.tricks==0) "Passed Out"; else contractTricks.tricks.toString()+contractSuit.suit+contractDoubled.forScore

    def getDeclarerForScoring = if (contractTricks.tricks==0) "-"; else declarer.pos

    def getMadeForScoring =
      if (contractTricks.tricks==0) Some("-")
      else madeOrDown match { case Made => Some(tricks.toString); case Down => None}

    def getDownForScoring =
      if (contractTricks.tricks==0) Some("-")
      else madeOrDown match { case Down => Some(tricks.toString); case Made => None}
     ...
     * </ol>
     */
    private def calculateMade(): (Boolean, Int, Int, Boolean, List[String], List[String]) = {
      def timesDoubling( firstTrickBonus: Int, trickValue: Int ): (Int,Int,Int,Int) = contractDoubled match {
        case NotDoubled => (firstTrickBonus, trickValue, trickValue, 0)
        case Doubled => (firstTrickBonus*2, trickValue*2, if (isDeclarerVulnerable) 200; else 100, 50 )
        case Redoubled => (firstTrickBonus*4, trickValue*4, if (isDeclarerVulnerable) 400; else 200, 100 )
      }

      val (firstTrickBonus, trickValue, overTricksValue, insultBonus) = contractSuit match {
      ...
      if (overTricks > 0) reason = "Overtricks("+overTricks+") "+valFromOverTricks::reason
     ...
     * </ol>
     */
    private def calculateDown(): (Boolean, Int, Int, Boolean, List[String], List[String]) = {
      ...
      var reason: List[String] = Nil
      if (tricks4p > 0) reason = ""+tricks4p+" at "+remainingTrickValue::reason
      if (tricks23 > 0) reason = ""+tricks23+" at "+secondThirdTrickValue::reason
     ...
     * </ol>
     */
    private def calculate(): (Boolean, Int, Int, Boolean, List[String], List[String]) =
    ...
    def toStringRubber() = {
      if (contractTricks.tricks == 0) "Passed out"
        ...
        val reason = explainAbove.mkString(", ")+" / "+explainBelow.mkString(", ")
        con+"   "+made+"   "+res+"   "+reason+(if (honors!=0) ", "+explainHonor else "")
      }
    }
  }
  ...
  object ScoreHand {
    def apply( id: String,
      ...
      new ScoreHand(id,contractTricks,contractSuit,contractDoubled,declarer,nsvul,ewvul,madeContract,tricks,honors,honorsPlayer)
    }

    def apply( hand: BridgeHand) = {
      new ScoreHand(hand.id,
                    hand.contractTricks,
                    hand.contractSuit,
