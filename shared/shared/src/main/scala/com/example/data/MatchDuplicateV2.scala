...
case class MatchDuplicateV2(
  ...
  def equalsInTeams( other: MatchDuplicateV2, throwit: Boolean = false ) = {
    if (teams.keySet == other.teams.keySet) {
      ...
      if (throwit) throw new Exception("MatchDuplicateV2 teams don't have same key: "+teams.keySet+" "+other.teams.keySet)
      false
    }
  }
  ...
  def equalsInBoards( other: MatchDuplicateV2, throwit: Boolean = false ) = {
    if (boards.keySet == other.boards.keySet) {
      ...
      if (throwit) notequalboard.foreach(key => {
        ...
        throw new Exception("MatchDuplicateV2 boards don't same value for key: "+key+"\n  "+b1+"\n  "+b2)
      })
      notequalboard.isEmpty
    } else {
      if (throwit) throw new Exception("MatchDuplicateV2 boards don't have same key: "+teams.keySet+" "+other.teams.keySet)
      false
    }
  }
  ...
  def updateBoard( board: BoardV1 ): MatchDuplicateV2 = copy( boards=boards+(board.id->board), updated=SystemTime.currentTimeMillis())
  ...
  def updateHand( hand: DuplicateHandV1 ): MatchDuplicateV2 = boards.get(hand.board) match {
  ...
  def updateHand( boardId: String, hand: DuplicateHandV1 ): MatchDuplicateV2 = boards.get(boardId) match {
    case Some(board) => updateBoard( board.updateHand(hand) )
    case None => throw new IndexOutOfBoundsException("Board "+boardId+" not found")
  }
  def updateHand( boardId: Id.DuplicateBoard, handId: String, hand: Hand ): MatchDuplicateV2 = boards.get(boardId) match {
    case Some(board) => updateBoard( board.updateHand(handId,hand) )
    case None => throw new IndexOutOfBoundsException("Board "+boardId+" not found")
  }

  def updateTeam( team: Team ): MatchDuplicateV2 = copy( teams= teams+(team.id->team), updated=SystemTime.currentTimeMillis() )

  def setTeams( teams: Map[Id.Team, Team] ) = copy( teams=teams, updated=SystemTime.currentTimeMillis() )

  def deleteTeam( teamid: Id.Team ) = copy( teams=teams-id, updated=SystemTime.currentTimeMillis() )
   ...
   */
  def determinePlayerPositionFromCaller( hand: DuplicateHandV1, pN1: Boolean, pE1: Boolean ): MatchPlayerPosition = {
    var n = "Unknown"
    var s = n
    var e = n
    var w = n
   ...
   */
  def fixVulnerability( correctVulnerability: MatchDuplicateV2 ): (MatchDuplicateV2,List[String]) = {
    var msgs: List[String] = Nil
    val md: MatchDuplicateV2 = copy( boards=
  ...
  def fillBoards( bs: BoardSet, mov: Movement ): MatchDuplicateV2 = {
    ...
    copy( teams = useteams, boards=filledB.toMap.filter( e => !e._2.hands.isEmpty ), boardset=bs.name, movement=mov.name, updated=SystemTime.currentTimeMillis() )
  }
...
object MatchDuplicateV2 {
  ...
  def create(id: String = "",
             ...
             movement: String = "Armonk2Tables" ) = new MatchDuplicateV2(id,Map(),Map(),boardset,movement,time,time)
