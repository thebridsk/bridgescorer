...
object DuplicateBridge {
  ...
  class ScoreHand( private val sid: String,
                   ...
                   private val stricks: Int) extends RubberBridge.ScoreHand(sid,scontractTricks,scontractSuit,scontractDoubled,sdeclarer,snsvul,sewvul,smadeContract,stricks) {

    private val gameBonus = if (madeOrDown == Made && contractTricks.tricks>0 && tricks>0) if (isGame) if (isDeclarerVulnerable) 500; else 300; else 50; else 0;
    ...
    override
    def explain() = explainList.mkString(", ")

    override
    def totalScore() = if (score.ns>=0) "NS "+score.ns; else "EW "+score.ew
  ...
  object ScoreHand {
    def apply( id: String,
               ...
               tricks: Int) = {
      new ScoreHand(id,contractTricks,contractSuit,contractDoubled,declarer,nsvul,ewvul,madeContract,tricks)
    }
