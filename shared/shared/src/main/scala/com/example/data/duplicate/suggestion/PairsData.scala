...
object PairsData {
  ...
  def apply( pastgames: List[DuplicateSummary], calc: CalculationType = CalculationAsPlayed ) = new PairsData(pastgames,calc)
 ...
 */
class PairsData( val pastgames: List[DuplicateSummary], val calc: CalculationType = CalculationAsPlayed ) {
  ...
  val (data,players): (Map[ (String,String), PairData ], List[String]) = {
    ...
    def add( player1: String, player2: String, win: Boolean, winPts: Double, pts: Double, totpts: Double, incomplete: Boolean, details: Option[DuplicateSummaryDetails], winImp: Boolean, winImpPts: Double, imp: Double, playMP: Boolean ): Unit = {
      addPerson(player1)
      addPerson(player2)
     ...
     *           showIMP: Boolean
     */
    def useStat( playedMP: Boolean, playedIMP: Boolean, hasMP: Boolean, hasIMP: Boolean ): (Boolean,Boolean) = {
      calc match {
    ...
    pastgames.foreach { ds =>
      ...
      val total = ds.teams.map( dse => dse.result.getOrElse(0.0) ).foldLeft(0.0)((ac,v)=>ac+v)/2
      val numberWinners = ds.teams.filter { dse => dse.hasMp && dse.getPlaceMp==1 }.length
      val numberWinnersImp = ds.teams.filter { dse => dse.hasImp && dse.getPlaceImp==1 }.length
