...
class ChicagoScoring(val chicago: MatchChicago) {
  val rounds = chicago.rounds.map { r => RoundScoring(r) }
  ...
  def sortedResults() = {
    ...
    }.map { i => ( players(i), totals(i) ) }.unzip
  }
  ...
  def toString() = {
    ...
    "\n                byRounds="+byRounds.map( e => e.mkString("{",", ","}")).mkString(", ")+
    ")"
  }
  ...
  private def calculate(): (List[String], List[Int], List[List[Int]]) = {
    ...
    if (players.find(s => s == null).isEmpty) {
      val playerMap = Array.tabulate(players.length)( i => (players(i) -> i) ).toMap
  ...
  def getFixturesSoFar() = {
    val pathSoFar = rounds.map(r => {
      ...
      ChicagoScoring.createFixture(r.round.north, r.round.south, r.round.east, r.round.west, extra)
    })
    pathSoFar
  }
  ...
  def getNextPossibleFixtures(): Map[String,Set[ChicagoScoring.Fixture]] = {
    ...
    while (pathSoFar.length >= players.length) pathSoFar = pathSoFar.drop(players.length)
...
object ChicagoScoring {
  ...
  def createMix( north: String, south: String, east: String, west: String, extra: String ) = {
  ...
  def createFixture( north: String, south: String, east: String, west: String, extra: String ) = {
    var n = north
    var s = south
    var e = east
    var w = west
   ...
   *   north < east
   */
  case class Fixture private( north: String, south: String, east: String, west: String, extra: String ) {
   ...
   */
  def calculateNextPossible( pathSoFar: Seq[Fixture], possiblePaths: List[List[Fixture]] ): Set[Fixture] = {
  ...
  def canUseFixture( candidate: Fixture, pathSoFar: Seq[Fixture] ) = {
    ...
    if (pathSoFar.isEmpty) true
      ...
      pathSoFar.find { f => {
        ...
        candidate.hasPair(f.north, f.south) || candidate.hasPair(f.east, f.west)
      }}.isEmpty
    }
//    if (!r) logger.info("Can't use "+candidate+" in "+pathSoFar) else  logger.info("Can use "+candidate+" in "+pathSoFar)
   ...
   */
  def calculatePaths( allFixtures: Seq[Fixture], pathSoFar: List[Fixture], nextOut: String ) = {
    if ( pathSoFar.find(f=>f.extra == nextOut).isEmpty ) {
      ...
      val x = fixturesWithNextOut.filter(candidate => canUseFixture(candidate,pathSoFar)).map(f => {
        val pSoFar = pathSoFar:::List(f)
        if (pSoFar.size == n) {
          List(pSoFar)
        } else {
          val validRemaining = allFixtures.filter(candidate => canUseFixture(candidate,pSoFar))
          calculateRemainingPath(n,pSoFar,validRemaining)
        }
      }).flatten.filter { l => !l.isEmpty }.toList
      x
    } else {
      List()
    }
  ...
  def calculatePaths( allFixtures: Seq[Fixture] ): List[List[Fixture]] = {
    ...
    val validRemaining = allFixtures.filter(candidate => canUseFixture(candidate,pathSoFar))
   ...
   */
  def calculateRemainingPath( target: Int, pathSoFar: List[Fixture], validRemaining: Seq[Fixture]): List[List[Fixture]] = {
    val x = validRemaining.map(vr => {
      val pSoFar = pathSoFar:::List(vr)
      if (pSoFar.length == target) {
        List(pSoFar)
      } else {
        val vRemaining = validRemaining.filter(candidate => canUseFixture(candidate,pSoFar))
