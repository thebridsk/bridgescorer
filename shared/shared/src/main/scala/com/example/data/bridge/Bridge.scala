...
abstract sealed case class ContractSuit( suit: String )
...
abstract sealed case class ContractDoubled( doubled: String, forScore: String )
...
object ContractDoubled {
  def apply( doubled: String ) = doubled match {
    ...
    case "D" => Doubled
    case "R" => Redoubled
    case _ => throw new IllegalArgumentException("Unknown value for a contract doubled: "+doubled)
  }
}

abstract sealed case class PlayerPosition( pos: String, name: String ) {
  ...
  def player( north: String, south: String, east: String, west: String ) = this match {
    case North => north
    case East => east
    case West => west
    case South => south
    case _ => throw new IllegalArgumentException("Unknown value for a player position: "+this)
  }
}
...
object PlayerPosition {
  def apply( pos: String ) = pos match {
    ...
    case "W" => West
    case "S" => South
    case _ => throw new IllegalArgumentException("Unknown value for a player position: "+pos)
  }
  ...
  def left( current: PlayerPosition ) = current match {
    ...
    case West => North
    case South => West
    case _ => throw new IllegalArgumentException("Unknown value for a player position: "+current)
  }
...
abstract sealed case class MadeOrDown( made: Boolean, forScore: String )
...
abstract sealed case class Vulnerability( vul: Boolean )
...
class BridgeHand( val id: String,
  ...
  case Made =>
    if (contractTricks > tricks || tricks > 7) {
      throw new IllegalArgumentException("For a made contract, tricks must be in range of [contractTricks,7], tricks="+tricks+", contract="+contractTricks.tricks)
    }
  case Down =>
    if (1 > tricks || tricks > 6+contractTricks.tricks) {
      throw new IllegalArgumentException("For a down contract, tricks must be in range of [1,6+contractTricks], tricks="+tricks+", contract="+contractTricks.tricks)
    }
  }

  def asHand() = Hand.create(id,
...
object BridgeHand {
  def apply( id: String,
             ...
             tricks: Int) = {
    new BridgeHand(id,contractTricks,contractSuit,contractDoubled,declarer,nsVul,ewVul,madeContract,tricks)
  }

  def apply( hand: Hand) = {
    new BridgeHand(hand.id,
