...
@Schema(
  ...
  description =
    "A hand from a duplicate match.  On input, the place field in DuplicateSummaryEntry is ignored.  If boardresults is specified, then the result field in DuplicateSummaryEntry is also ignored on input."
)
case class MatchDuplicateResultV1 private (
    ...
    @Schema(
      description = "when the duplicate match was played",
      required = true
    )
    played: Timestamp,
    @Schema(
  ...
  def equalsInResults(
    ...
    if (results.length == other.results.length) {
      results
        ...
        .map { e =>
          ...
          left.find { t1 =>
            ...
            if (rc && throwit)
              throw new Exception(
                "MatchDuplicateResultV1 other did not have result equal to: " + t1
              )
            rc
          }.isEmpty
        }
      ...
      if (throwit)
        throw new Exception(
          "MatchDuplicateResultV1 results don't winner sets: " + results + " " + other.results
        )
      false
    }
  }
  ...
  def copyForCreate(id: MatchDuplicateResult.Id): MatchDuplicateResultV1 = {
    val time = SystemTime.currentTimeMillis()
  ...
  def fixPlaces: MatchDuplicateResultV1 = {
  ...
  @Schema(hidden = true)
  def fixupSummary: MatchDuplicateResultV1 = {
    boardresults match {
      case Some(l) =>
        this
      case None =>
  ...
  @Schema(hidden = true)
  def fixup: MatchDuplicateResultV1 = {
  ...
  def convertToCurrentVersion: (Boolean, MatchDuplicateResultV2) = {
    ...
    (
      ...
      MatchDuplicateResultV2(
        ...
        created,
        updated,
        MatchDuplicate.MatchPoints
      )
    )
  }

  def readyForWrite: MatchDuplicateResultV1 = this
