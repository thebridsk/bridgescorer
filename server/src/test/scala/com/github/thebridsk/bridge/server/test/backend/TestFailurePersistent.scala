...
class TestFailurePersistent[VId <: Comparable[VId], VType <: VersionedInstance[
  VType,
  VType,
  VId
]](implicit
  ...
  /**
    ...
    */
  override def getAllIdsFromPersistent(): Set[VId] = {
  ...
  override def createInPersistent(
    ...
    log.fine(
      s"""createInPersistent failWrite=${failWrite}, failResultWrite=${failResultWrite}, useId=${useId}, v=${v}"""
    )
    if (failWrite) {
      Future {
  ...
  override def getFromPersistent(
    ...
    log.fine(
      s"""getFromPersistent failRead=${failRead}, failResultRead=${failResultRead}, id=${id}"""
    )
    if (failRead) {
      Future {
  ...
  override def putToPersistent(
    ...
    log.fine(
      s"""putToPersistent failWrite=${failWrite}, failResultWrite=${failResultWrite}, id=${id}, v=${v}"""
    )
    if (failWrite) Future {
      Thread.sleep(500)
      throw new Exception("Failure writing to persistent store!")
  ...
  override def deleteFromPersistent(
    ...
    log.fine(
      s"""deleteFromPersistent failDelete=${failDelete}, failResultDelete=${failResultDelete}, id=${id}, cacheValue=${cacheValue}"""
    )
    Future {
      if (failDelete) {
...
class TestFailureStore[VId <: Comparable[VId], VType <: VersionedInstance[
  VType,
  VType,
  VId
]] private (
...
object TestFailureStore {
  def apply[VId <: Comparable[VId], VType <: VersionedInstance[
    ...
    new TestFailureStore(
      name,
      Option(testFailurePersistent).getOrElse(
        TestFailurePersistent[VId, VType]()
      ),
      cacheInitialCapacity,
      cacheMaxCapacity,
