...
object TestDuplicateMatchScoringManual extends MainNoArgs {
  ...
  def execute(): Int = {
    ...
    Await.result(
      new BridgeServiceInMemory("test")
        .fillBoards(MatchDuplicate.create(MatchDuplicate.id("M2"))),
      30.seconds
    ) match {
      case Right(md) =>
  ...
  def check(m: MatchDuplicate, score: MatchDuplicateScore): Unit = {
    ...
    val boards =
      score.boards.values.toList.sortWith((one, two) => one.id < two.id)

    val header =
      List(List("team", "total"), boards.map(b => b.id.id).toList).flatten

    val body = teams.map(team =>
      List(
        List(team.toString(), score.teamScores.get(team).get.toString),
        teamScores(boards, team)
      ).flatten
    )
    ...
    val pbody = score.places.map { place =>
      List(
        place.place.toString(),
        place.score.toString(),
        place.teams.mkString(", ")
      )
    }
          ...
          if (teamscore.played) {
            if (teamscore.hidden) {
              List(
                ...
                "?",
                "--",
                teamscore.opponent.get.toString,
                "?"
              )
            } else {
              ...
              List(
                ...
                contract.contract,
                contract.declarer,
                contract.made.getOrElse(""),
                contract.down.getOrElse(""),
                teamscore.score.toString,
                "--",
                teamscore.opponent.get.toString,
                teamscore.points.toString
              )
            }
          } else {
            List(
  ...
  def showTables(md: MatchDuplicate): Unit = {
    MatchDuplicateScore(md, PerspectiveTable(Team.id(1), Team.id(2))).tables
      .foreach {
        case (table, rounds) =>
          println("")
          println("Table " + table)
          ...
          val body = rounds.map { round =>
            {
              List(
                ...
                round.ns.id.id,
                round.ew.id.id,
                round.boards.map { b => b.id.id }.mkString(", ")
              )
            }
          }
          printTable(header, body)
