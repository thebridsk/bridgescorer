...
object Preconditions {
  def checkNotNull[T]( v: T, msg: String ): T = if (v!=null) v else throw new NullPointerException(msg)
}

class Thing(@JsonProperty("colour") _colour: String,
            @JsonProperty("priority") _priority: Int) {
  val colour: String = Preconditions.checkNotNull(_colour, "colour cannot be null")
  val priority: Int = Preconditions.checkNotNull(_priority, "priority cannot be null")
