...
object ShowPassedForPlayer extends Main {
  ...
  def execute(): Int = {
    ...
    val datastore = new SyncStore( new BridgeServiceFileStore(storedir).duplicates )
    ...
    val (pas,passedBoardsWithPlayer) = datastore.readAll() match {
      ...
      case Right( map ) =>
        val pbwp = map.values.flatMap { md =>
          ...
          oteam.map { team =>
            md.boards.flatMap { board =>
              val passedHand = board.hands.find( h => !h.played.isEmpty && h.played.head.contractTricks == 0 )
              if (passedHand.isDefined) {
                val playedHand = board.hands.find( h => h.played.head.contractTricks != 0)
                val playerPlayed = board.hands.find( h=> h.nsTeam == team.id || h.ewTeam == team.id )
                val played = playerPlayed.head == playedHand.head
                val playedByNS = playedHand.head.hand.head.declarer=="N" || playedHand.head.hand.head.declarer=="S"
        ...
        val stats = PlayerComparisonStats.stats(map).data.filter( ps => ps.player == p && ps.stattype == PlayerComparisonStat.PassedOut)
        (stats,pbwp)
      }
    ...
    logger.info(s"Player ${p} was involved in ${passedBoardsWithPlayer.length} boards with passed out hands")
    ...
    grouped.values.foreach { group =>
      group.foreach { pb =>
        ...
        logger.info(s"""-- ${g} -- ${pp} ----------------""")
        logger.info(s"""Player was on team ${pb.team} in match ${pb.id} on board ${pb.board.id}""")
        pb.board.hands.foreach { dh =>
          ...
          if (dh.hand.head.contractTricks == 0) {
            logger.info(s"""  ${pl}Passed hand, teams ${dh.nsTeam} ${dh.ewTeam}""")
          } else {
            val dec = dh.hand.head.declarer
            val (declarer,defender) = if (dec == North.pos || dec == South.pos) (dh.nsTeam,dh.ewTeam) else (dh.ewTeam,dh.nsTeam)
            val cmd = if (dh.hand.head.madeContract) "made" else "down"
            logger.info(s"""  ${pl}${dh.hand.head.contract} ${cmd} by ${declarer}, defended by ${defender}""")
          }
        }
      }
    }
