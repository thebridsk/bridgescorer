...
class TestResourceStore extends AsyncFlatSpec with Matchers {
  ...
  it should "return the same object when adding a MatchDuplicate to the store" in {
    store.createChild(matchdup).map { ret =>
      ret match {
        ...
        case Left(r) =>
          fail(
            "Unable to add MatchDuplicate " + matchdup.id + " to store: " + r._2
          )
      }
    }
  }
  ...
  it should "return the M1 object when getting from store" in {
    store.select(matchdup.id).read().map { ret =>
      ret match {
        ...
        case Left(r) =>
          fail(
            "Unable to query MatchDuplicate " + matchdup.id + " from store: " + r._2
          )
      }
    }
  }
  ...
  it should "return the B1 object when getting boards from match M1 from store" in {
    store.select(matchdup.id).resourceBoards.select(Board.id(1)).read().map {
      ret =>
        ret match {
          case Right(l) =>
            assert(l.id == Board.id(1))
            assert(
              l.equalsIgnoreModifyTime(
                Board.create(
                  Board.id(1),
                  false,
                  false,
    ...
    store
      ...
      .map { ret =>
        ret match {
          ...
          case Left(r) =>
            fail(
              "Unable to get hand T1 from MatchDuplicate,Board " + matchdup.id + ",B1 to store: " + r._2
            )
        }
      }
  }
    ...
    store
      ...
      .map { ret =>
        ret match {
          case Right(l) =>
            fail(
              "Unexpected response to get hand H1 from MatchDuplicate,Board M2,B1 to store: " + l
            )
          case Left(r) =>
            assert(r._1 == StatusCodes.NotFound)
            assert(r._2 == RestMessage("Did not find resource /duplicates/M2"))
    ...
    store
      ...
      .map { ret =>
        ret match {
          case Right(l) =>
            fail(
              "Unexpected response to get hand H1 from MatchDuplicate,Board M2,B4 to store: " + l
            )
          case Left(r) =>
            assert(r._1 == StatusCodes.NotFound)
            assert(
              r._2 == RestMessage(
                s"Did not find resource /duplicates/${matchdup.id.id}/boards/B4"
              )
            )
        }
      }
    ...
    store
      ...
      .map { ret =>
        ret match {
          case Right(l) =>
            fail(
              "Unexpected response to get hand H1 from MatchDuplicate,Board M2,B3 to store: " + l
            )
          case Left(r) =>
            assert(r._1 == StatusCodes.NotFound)
            assert(
              r._2 == RestMessage(
                s"Did not find resource /duplicates/${matchdup.id.id}/boards/B3/hands/T1"
              )
            )
        }
      }
