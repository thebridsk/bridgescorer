...
object TestScoringManual extends Main {
    ...
    for (
      ...
      for (tricks <- ctricks to 7) {
        ...
        for (
          ...
          if (!didsuit) {
            val contract =
              "" + hand.contractTricks.tricks + hand.contractSuit.suit + "   " + hand.tricks
            print(contract)
            didsuit = true
          }
          print("   " + hand.score.ns)
  ...
  def checkMajorMinor(): Unit = {
    ...
    for (
      (suit1, suit2) <- (Spades, Hearts) :: (Diamonds, Clubs) :: Nil;
      ctricks <- 1 to 7
    ) {
      for (tricks <- ctricks to 7) {
        var didsuit = false
        for (
          ...
          if (hand1.score.ns != hand2.score.ns) {
            println(
              "Not equal scores: " + hand1.score.ns + " " + hand2.score.ns
            )
            println("  " + hand1)
            println("  " + hand2)
          }
    ...
    for (ctricks <- 7 to 7) {
      for (tricks <- 1 to 6 + ctricks) {
        ...
        for (
          ...
          if (!didsuit) {
            val contract =
              "" + hand5.contractTricks.tricks + hand5.contractSuit.suit + "   " + hand5.tricks
            print(contract)
            didsuit = true
          }
          print("   " + hand5.score.ew)
          if (hand5.score.ns != -hand5.score.ew)
            print(
              "  ew-ns scores not equal (" + hand5.score.ns + "," + hand5.score.ew
            )
          if (
            hand5.score.ns != hand1.score.ns
            ...
            print(
              "suit score not equal " + hand1.score.ns + "," + hand2.score.ns + "," + hand3.score.ns + "," + hand4.score.ns + "," + hand5.score.ns +
      ...
      for (
        ...
        for (ctricks <- Math.max(1, tricks - 6) to 7) {
          ...
          for (suit <- Spades :: Hearts :: Diamonds :: Clubs :: Nil) {
            ...
            if (handRef.score.ns != -handRef.score.ew)
              print(
                "  ew-ns scores not equal (" + handRef.score.ns + "," + handRef.score.ew
              )
            if (
              handRef.score.ns != hand.score.ns
              || handRef.score.ew != hand.score.ew
            ) {
              println(
                "score not equal " + handRef.score.ns + "," + hand.score.ns
              )
              println("  " + handRef)
              println("  " + hand)
            }
