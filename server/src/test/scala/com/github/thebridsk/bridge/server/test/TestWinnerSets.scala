...
class TestWinnerSets extends AnyFlatSpec with Matchers {
  ...
  def getDup(movementid: Movement.Id): MatchDuplicate = {
    ...
    Await.result(
      restService
        .fillBoards(MatchDuplicate.create(), BoardSet.standard, movementid),
      30.seconds
    ) match {
      case Right(m) => m
      case Left((statuscode, msg)) =>
        testlog.info(
          s"Unable to create MatchDuplicate with movement $movementid: (${statuscode}) ${msg.msg}"
        )
        fail(
          s"Unable to create MatchDuplicate with movement $movementid: (${statuscode}) ${msg.msg}"
        )
...
//    println( s"swapping NS and EW in hand ${hand}")
  ...
  def swapNSEWInBoardOnTable(
      board: Board,
      round: Int,
      table: Table.Id
  ): Board = {
//    println( s"swapping NS and EW in board ${board}")
    val nhs = board.hands.map(h =>
      if (h.table == table && h.round == round) swapNSEWInHand(h) else h
    )
    board.copy(hands = nhs)
  }

  def swapNSEWInHandsInRoundOnTable(
      dup: MatchDuplicate,
      round: Int,
      table: Table.Id
  ): MatchDuplicate = {
