...
class TestFileStore extends AsyncFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  val team: MatchRubberV1 =
    MatchRubber(MatchRubber.id(1), "Fred", "Barney", "", "", "N", Nil)
  val team2: MatchRubberV1 =
    MatchRubber(MatchRubber.id(2), "Wilma", "Betty", "", "", "N", Nil)
  var teamStore: Store[MatchRubber.Id, MatchRubber] = null

  implicit val support: MatchRubberCacheStoreSupport =
    new MatchRubberCacheStoreSupport(false)
  implicit val supportD: MatchDuplicateCacheStoreSupport =
    new MatchDuplicateCacheStoreSupport(false)

  def filename(id: String): String =
    resourceName + "." + id + support.getWriteExtension
  def path(id: String): String =
    tempDir.toString() + File.separator + filename(id)
  ...
  it should "return the new team when adding another team, and create file in temp directory" in {
    val team3 =
      MatchRubber(MatchRubber.id(1), "Yogi", "Boo Boo", "", "", "N", Nil)
        ...
        case Right(t3) =>
          ...
          teamStore.select(MatchRubber.id(3)).read().map { resulta =>
            resulta match {
              ...
              case Left((statuscode, msg)) =>
                fail(
                  "Was expecting success on reading T3, got " + statuscode + " " + msg
                )
            }
          }
        case Left((statuscode, msg)) =>
        ...
        case Right(t3) =>
          ...
          teamStore.select(t3.id).update(nt).map { resu =>
            resu match {
              ...
              case Left((statuscode, msg)) =>
                fail(
                  "Was expecting success on updating T3, got " + statuscode + " " + msg
                )
            }
          }
        case Left((statuscode, msg)) =>
  ...
  it should "only have teams 1 and 3 left" in {
    teamStore.readAll().map { result =>
      result match {
        case Right(teams) =>
          teams.keys.toArray must contain theSameElementsAs Array(
            MatchRubber.id(1),
            MatchRubber.id(3)
          )
        case Left((statuscode, msg)) =>
  ...
  it should "be able to get an old MatchDuplicateV1 from the store" in {
    ...
    def pathMD(id: String) =
      tempDir.toString() + File.separator + filenameMD(id)
    ...
    duplicateStore.select(md.id).read().map { result =>
      result match {
        ...
        case Left((statusCode, restMessage)) =>
          fail(
            "Failed to get MatchDuplicate with id " + md.id + ": statuscode=" + statusCode + ", message=" + restMessage
          )
      }
    }
