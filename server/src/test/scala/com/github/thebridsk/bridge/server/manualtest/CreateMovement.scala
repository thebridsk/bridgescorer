...
class CreateMovementConf extends MainConf {
  ...
  val paramOutput: ScallopOption[Path] = trailArg[Path](
    name = "output",
...
//    return 0
    ...
    implicit val reader = new PushbackReader(
      Source.fromFile(paramInput().jfile, "UTF-8")
    )
    try {
      while (true) {
    ...
    if (reader.hasNext()) {
      ...
      line match {
        case patternTablesLine(ntables, nboards, name, comment) =>
          val na =
            Movement.id(f"Howell${ntables.toInt * 2}%02dT${ntables}B${nboards}")
          val c =
            if (comment == null)
              s"Howell ${ntables.toInt * 2} teams ${ntables} tables ${nboards} boards"
            else comment
          val d = if (comment == null)
            s"Howell movements, ${ntables.toInt * 2} teams, ${ntables} tables, ${nboards} boards"
          else comment
  ...
  def checkForRelay(m: Movement): MovementV1 = {
    ...
    val name =
      relay.map(r => Movement.id(s"${m.name.id}Relay")).getOrElse(m.name)
    val desc =
      relay.map(r => s"${m.description}, relay").getOrElse(m.description)
...
//  it ir ins iew iboards
  ...
  /**
    ...
    *                     If Right, then a Movement object
    */
  def processTableShort(
      m: Movement
  )(implicit reader: PushbackReader): Either[RC, Movement] = {
  ...
  /**
    ...
    *                     If Right, then a Movement object
    */
  def processTableLongRound(m: Movement, itable: Int)(implicit
      reader: PushbackReader
  ): Either[RC, Movement] = {
    get3Ints match {
