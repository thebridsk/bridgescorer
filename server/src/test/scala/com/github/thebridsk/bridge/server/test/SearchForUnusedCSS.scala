...
object SearchForUnusedCSS extends Main[SearchForUnusedCSSConf] {
  ...
  case class Entry(
      cssClass: String,
      styleName: List[String] = List(),
      styleDef: List[SourcePosition] = List(),
      sourceUse: List[SourcePosition] = List()
  ) {
  ...
  def execute(): Int = {
    ...
    use.foreach { e =>
      if (!e.sourceUse.isEmpty) {
        logger.info(s"""CSS ClassName ${e.cssClass} Styles [${e.styleName
          .mkString(",")}]""")
        logger.info(
          e.sourceUse
            .map(p => p.line)
    ...
    dir.files
      .flatMap { f =>
        Source.fromFile(f.jfile, "UTF-8").getLines().flatMap { line =>
          patternClassname.findAllIn(line).matchData.map { matcher =>
            matcher.group(1)
          }
        }
      }
    ...
    dir.deepFiles
      ...
      .foldLeft(entries) { (acentries, f) =>
        f.lines("UTF-8").zipWithIndex.foldLeft(acentries) { (ac, v) =>
          ...
          ac.map { e =>
            val pat =
              s"""val +([a-zA-Z0-9]+) *= *cls\\( *"[ a-zA-Z0-9]* *${e.cssClass}(?: +[a-zA-Z0-9]+)* *" *\\)""".r
            ...
            if (r.isEmpty) e
              ...
              r.map { matcher =>
                ...
                ac.addStyleDef(
                  SourcePosition(Position(f.name, f.toString(), i))
                ).addStyleName(v)
              }
            }
          }
          ...
          ac.map { e =>
            e.styleName.foldLeft(e) { (ace, dname) =>
              if (line.contains(dname))
                ace.addSource(SourcePosition(Position(f.name, f.toString(), i)))
              else ace
            }
          }
        }
