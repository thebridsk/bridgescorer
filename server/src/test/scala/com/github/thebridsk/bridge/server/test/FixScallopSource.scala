...
//  scallop_2.12-3.1.1-sources.jar
  ...
  def execute() = {
    ...
    val srcdir = new File( homedir, """.ivy2\cache\org.rogach\scallop_2.12\srcs""" )
        ...
        case patternScallop(ver,original)  =>
          ...
          files.get(version) match {
            case Some( (jar,originaljar) ) =>
              files += (version -> (if (original==null) (f,originaljar) else (jar,f)) )
            case None =>
              files += (version -> (if (original==null) (f,null) else (null,f)) )
          }
        case _ =>
    ...
    files.filter( p => p._2._2==null ).toList.sortBy(e => e._1).lastOption match {
      case Some(lastversion) =>
        ...
        val originalfilename = new File(file.toString().substring(0, lastdot)+".original.jar")
...
//    n.setCrc( e.getCrc )
    ...
    if (e.getLastModifiedTime != null) n.setLastModifiedTime( e.getLastModifiedTime )
