...
Options:""")
  ...
  def execute(): Int = {
    ...
    val duplicateSummary = optionStore.toOption
      .map { dir =>
        ...
        Await.result(datastore.getDuplicateSummaries(), 30.seconds) match {
          ...
          case Left(error) =>
            log.warning(
              s"Error getting duplicate summaries from store ${dir}, ignoring: ${error}"
            )
            List()
        }
      }
    ...
    playedMatches ++= duplicateSummary
    ...
    log.info(s"""Calculation times:\nmatch,timeInNanos${times
      .map(e => e._1.toString() + "," + e._2)
      .mkString("\n", "\n", "")}""")
    0
  }
  ...
  def toStringDetail(s: Suggestion): String = {
    ...
    x.mkString(
      "",
      ", ",
      f"  lastPlayedAllTeams=${s.lastPlayedAllTeams} minLastPlayed=${s.minLastPlayed}, maxLastPlayed=${s.maxLastPlayed}, maxTimesPlayed=${s.maxTimesPlayed}, avgLastPlayed=${s.avgLastPlayed}, avgTimesPlayed=${s.avgTimesPlayed}, lastPlayedAllTeams=${s.lastPlayedAllTeams}, weight=${s.weight}%.5f, weights=${wghts}, random=${s.random}"
    )
  }
  ...
  def nextSuggestion(
    ...
    val sug =
      DuplicateSuggestionsCalculation.calculate(input, playedMatches.toList)
  ...
  def toDuplicateSummary(
    ...
    val teams = sug.players.zipWithIndex.map { e =>
      ...
      DuplicateSummaryEntry(
        Team(Team.id(i + 1), p.player1, p.player2, 0, 0),
        Some(9.0),
        Some(1)
      )
    }
  ...
  def analyze(players: List[String], ignore: Int): Unit = {
    ...
    res.toList
      .sortWith((l, r) => l.minGamesBetween < r.minGamesBetween)
      .foreach { p =>
        log.info(p.toString())
      }
  ...
  def getKey(l: DuplicateSummaryEntry): (String, String) = {
    if (l.team.player1 < l.team.player2) (l.team.player1, l.team.player2)
    else (l.team.player2, l.team.player1)
  }
  ...
  def analyzePair(pm: List[DuplicateSummary], p1: String, p2: String): Pair = {
    ...
    val playedGames = pm.zipWithIndex
      .filter { e =>
        val (m, i) = e
    ...
    val pg1 =
      if (playedGames.length <= 1) pm.length :: Nil
      else playedGames.drop(1) ::: List(pm.length + playedGames.head)
