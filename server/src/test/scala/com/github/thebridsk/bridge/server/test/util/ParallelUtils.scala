...
object ParallelUtilsInternals {
  ...
  private[util] def toExceptionMsg(
      msg: String,
      causes: (Position, Throwable)*
  ) = {
    msg + causes
      .map { e =>
        val ex = e._2
...
class FutureException(msg: String, cause: Throwable)
    extends Exception(msg, cause)

class ParallelException(msg: String, causes: (Position, Throwable)*)
    extends Exception(ParallelUtilsInternals.toExceptionMsg(msg, causes: _*)) {
  causes.foreach(e =>
    addSuppressed(new FutureException(s"From ${e._1.line}", e._2))
  )
...
trait ParallelUtils {
  ...
  def waitForFuturesIgnoreTimeouts(name: String, funs: CodeBlock[_]*)(implicit
  ...
  def waitForFutures(name: String, cbs: CodeBlock[_]*)(implicit
      timeoutduration: Duration
  ): Unit = {
    try {
  ...
  def waitForFuturesImpl(name: String, cbs: CodeBlock[_]*)(implicit
      timeoutduration: Duration
  ): Unit = {
    ...
    )(List[(Position, Throwable)]())((ac, v) =>
      v._2.map { ex => (v._1, ex) :: ac }.getOrElse(ac)
    )
    val failures = Await.result(futurefailures, timeoutduration)
    if (!failures.isEmpty)
      throw new ParallelException(
        name + failures.map(s => s._2).mkString("\n  ", "\n  ", ""),
        failures: _*
      )
  }

  def executeSerial(name: String, cbs: CodeBlock[_]*)(implicit
      timeoutduration: Duration
  ): Unit = {
    val failures = cbs.flatMap { cb =>
      try {
        ...
        case x: Exception =>
          log.severe(
            s"""Error executing "${name}" code block ${cb.pos.line}""",
            x
          )
          (cb.pos, x) :: Nil
      }
...
object ParallelUtils extends ParallelUtils {
  ...
  def ignoreTimeoutExceptions(name: String, t: Throwable)(implicit
      pos: Position
  ): Unit = {
    t match {
      case x: ParallelException =>
        if (
          x.getSuppressed.find { s =>
            s match {
