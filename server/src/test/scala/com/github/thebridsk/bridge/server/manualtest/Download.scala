...
//              </a>
  ...
  def oldExecute(): Int = {
    ...
    val jarURIPattern =
      s"""<a href="(/${project}/releases/download/[^/]+/${fileToDownload}-.*?.jar)" """.r
            ...
            case jarURIP(jarURI) =>
              ...
              getSha256File(jarURL) match {
                case Some(sha) =>
                  ...
                  val result = HttpUtils.copyFromWebToFile(
                    new URL(jarURL),
                    new File(filename),
                    algo,
                    true
                  ...
                  println(
                    s"Http status is ${result.status}, ${algo} is ${result.hash.get}"
                  )
                  if (sha == result.hash.get) {
                    println(s"Hash is good, downloaded file ${filename}")
                  } else {
                    println(
                      s"Hash does not match, do not trust downloaded file ${filename}"
                    )
                  }
                case None =>
