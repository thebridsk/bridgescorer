...
trait EventuallyUtils {
   ...
   * @throws NoResultYet if <code>fun</code> always returned None
   */
  def eventuallySome[T](fun: => Option[T])(implicit config: PatienceConfig, pos: Position): T = {
    eventually({
      fun match {
        case Some(t) => Some(t)
        case None => throw new NoResultYet("function did not return some object")(pos)
      }
    }).get
  }
