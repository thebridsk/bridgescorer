...
object CheckAndFixBoardsInMatches extends Main[CheckAndFixBoardsInMatchesConf] {
  ...
  def execute(): Int = {
    ...
    Await.result(
      bridgeServer.fillBoards(MatchDuplicate.create(MatchDuplicate.id("xx"))),
      30.seconds
    ) match {
      case Right(correct) =>
        matchkeys
          .flatMap { id =>
            bs.duplicates.syncStore.read(id) match {
              case Right(result) => Some(result)
              case Left(error) =>
                logger.warning(
                  "Did not find MatchDuplicate for id " + id + ": " + error._2.msg
                )
                None
            }
          }
          .foreach { md =>
            ...
            if (msgs.isEmpty) {
              ...
              logger.info(
                "Fixing duplicate match with ID " + md.id + ", errors were:"
              )
