...
/**
  ...
  */
case class StoreIdMeta[VId](
    id: VId,
    timestamp: String,
    persistentMeta: String,
    persistentMeta2: String
)
...
object StoreIdMeta {
  ...
  val resultNotSupported: Result[Nothing] = Result(
    StatusCodes.BadRequest,
    RestMessage("Metadata is not supported for store")
  )

  val notSupported: Future[Result[Nothing]] =
    Future.successful(resultNotSupported)
...
trait MetaData[VId] {
  ...
  /**
    ...
    * To read the file, the read method must be used on this object.
    */
  def listFilesFilter(id: VId)(
      filter: MetaDataFile => Boolean
  ): Result[Iterator[MetaDataFile]] = StoreIdMeta.resultNotSupported
...
trait StoreMetaData[VId] {
  ...
  /**
    ...
    * To read the file, the read method must be used on this object.
    */
  def listFiles(id: VId): Future[Result[Iterator[MetaDataFile]]] =
    StoreIdMeta.notSupported
  ...
  def listFilesFilter(id: VId)(
      filter: MetaDataFile => Boolean
  ): Future[Result[Iterator[MetaDataFile]]] = StoreIdMeta.notSupported
  ...
  def write(
      ...
      sourceFile: File,
      targetFile: MetaDataFile
  ): Future[Result[Unit]] = StoreIdMeta.notSupported

  /**
    * read the specified file, the file is relative to the store directory for specified match.
    */
  def read(id: VId, file: MetaDataFile): Future[Result[InputStream]] =
    StoreIdMeta.notSupported

  /**
    * delete the specified file, the file is relative to the store directory for specified match.
    */
  def delete(id: VId, file: MetaDataFile): Future[Result[Unit]] =
    StoreIdMeta.notSupported
...
abstract class PersistentSupport[
  ...
  /**
    ...
    * @return the next ID
    */
  def generateNextId(v: VType): Result[VId] =
    synchronized {
      import Implicits._
      ...
      val id = maxId.orElse {
        ...
        current.foldLeft(PersistentSupport.zero[VId]) { (ac, vid) =>
          val mm =
            ac.map(m => if (support.idSupport.compare(m, vid) < 0) vid else m)
          mm.orElse(Some(vid))
        }
      }
  ...
  def notFound(id: VId): Result[Nothing] =
    Result(
      ...
      s"Did not find resource $resourceURI/${support.idSupport.toString(id)}"
    )
