...
object RestNestedPictureIndividualHand {
  val log: Logger = Logger[RestNestedPictureIndividualHand]()

  val patternImageFile: Regex = """Image\.([^.]+)\.([^.]+)\.(jpg)""".r

  case class PictureFilenameParts(
  ...
  def getPartsMetadataFile(mdf: MetaDataFile): Option[PictureFilenameParts] = {
    mdf match {
      case patternImageFile(bid, hid, ext) =>
        Some(PictureFilenameParts(IndividualBoard.id(bid), IndividualDuplicateHand.id(hid), ext))
      case _ => None
    }
  }
  ...
  def isImageFilename(file: MetaDataFile, boardId: IndividualBoard.Id): Boolean = {
  ...
  def tempDestination(boardId: IndividualBoard.Id, handId: IndividualDuplicateHand.Id)(
      fileInfo: FileInfo
  ): JFile = {
...
import RestNestedPictureIndividualHand._
...
/**
  ...
  * swagger annotations.
  */
@Path("/rest/individualduplicates/{dupId}/pictures/{boardId}/hands")
@Tags(Array(new Tag(name = "IndividualDuplicate")))
class RestNestedPictureIndividualHand(
    store: Store[IndividualDuplicate.Id, IndividualDuplicate],
    parent: RestNestedPictureIndividual
) {
  ...
  def route(implicit
      ...
      @Parameter(hidden = true) boardId: IndividualBoard.Id
  ): Route =
    pathPrefix("hands") {
      logRequestResult("RestNestedPictureIndividualHand.route", DebugLevel) {
        getPicture(dupId, boardId) ~ getPictures(dupId, boardId) ~ putPicture(
          dupId,
          boardId
        ) ~ deletePicture(dupId, boardId)
      }
    }
        ...
        ri match {
          case Right(it) =>
            val r = it.flatMap { mdf =>
              mdf match {
                case patternImageFile(bid, hid, ext) =>
                  val handid = IndividualDuplicateHand.id(hid)
                  val boardid = IndividualBoard.id(bid)
                  IndividualDuplicatePicture(
                    boardid,
                    handid,
                    getPictureUrl(dupId, boardId, handid)
                  ) :: Nil
                case _ =>
                  Nil
              }
  ...
  @Operation(
    ...
    parameters = Array(
      ...
      new Parameter(
        allowEmptyValue = false,
        description =
          "ID of the hand for the picture that information is wanted for",
        in = ParameterIn.PATH,
        name = "handId",
        required = true,
        schema = new Schema(`type` = "string")
    ...
    logRequest("getPictureHand", DebugLevel) {
      get {
        path("""[a-zA-Z0-9]+""".r) { hids =>
          val hid = IndividualDuplicateHand.id(hids)
          val fprefix = s"${hids}."
          val fut = store.metaData
            .listFilesFilter(dupId) { f =>
              isImageFilename(f) && getPartsMetadataFile(f)
                .map { e => e.boardId == boardId && e.handId == hid }
                .getOrElse(false)
            }
            .map { ri =>
              ri match {
                        ...
                        optionalAccept
                          ...
                          .map { mr =>
                              ...
                              store.metaData.read(dupId, mdf).map { ris =>
                                ris match {
                                  case Right(is) =>
                                    ...
                                    val byteSource: Source[ByteString, Any] =
                                      Source.fromIterator(() => itbs)
                                    val mediatype = MediaTypes
                                      .forExtension(mdfparts.ext)
                                      .asInstanceOf[MediaType.Binary]
                                    log.fine(
                                      s"For file with extension of ${mdfparts.ext} Responding with content-type: ${mediatype.value} ${mediatype.fileExtensions}"
                                    )
                                    val contenttype = ContentType(mediatype)
                                    val entity: HttpEntity.Chunked =
                                      HttpEntity(contenttype, byteSource)
                                    complete(
                                      HttpResponse(
                                        entity = entity
                                      )
                            ...
                            onComplete(fr) {
                              ...
                              case Failure(ex) =>
                                log.warning(
                                  s"Error getting image file for ${mdfparts}",
                                  ex
                                )
                                complete(
                                  StatusCodes.InternalServerError,
                            ...
                            val r =
                              mdf match {
                                case patternImageFile(bid, tid, ext) =>
                                  val teamid = IndividualDuplicateHand.id(tid)
                                  val boardid = IndividualBoard.id(bid)
                                  val v = IndividualDuplicatePicture(
                                    boardid,
                                    teamid,
                                    getPictureUrl(dupId, boardId, teamid)
                                  )
    ...
    logRequest("putPictureHand", DebugLevel) {
      logResult("putPictureHand", DebugLevel) {
        put {
          path("""[a-zA-Z0-9]+""".r) { hids =>
            val hid = IndividualDuplicateHand.id(hids)
            storeUploadedFiles("picture", tempDestination(boardId, hid)) {
              files =>
                if (files.length != 1) {
                  complete(
                  ...
                  val fut = store.metaData.listFiles(dupId).map { ri =>
                    ri match {
                      case Right(it) =>
                        val filelist = it
                          .filter(f => isImageFilename(f, boardId, hid))
                          .toList
                  ...
                  onComplete(fut) {
                    case Success(rf) =>
                      rf match {
                        case Right((filelist, omdf)) =>
                          val deletes =
                            if (
                              filelist.length > 1 || omdf
                                .map(f => f != mdf)
                                .getOrElse(false)
                            ) {
                              val dels = filelist
                                .filter(f => f != mdf)
                                .map(f => store.metaData.delete(dupId, f))
                              Future.foldLeft(dels)(Result.unit) { (ac, v) =>
                                ac
                              }
                            } else {
                              Future(Result.unit)
                          ...
                          onComplete(deletes) {
                            case Success(runit) =>
                                ...
                                UpdateIndividualDuplicatePicture(
                                  ...
                                  picture = Some(
                                    IndividualDuplicatePicture(
                                      boardId,
                                      hid,
                                      getPictureUrl(dupId, boardId, hid)
                                    )
                                  )
                                )
                              )
                              val write = store.metaData
                                .write(dupId, new File(file), mdf)
                                .map { rw =>
                                  rw match {
                                    case Right(unit) =>
                              ...
                              onComplete(write) {
                                ...
                                case Failure(ex) =>
                                  log.warning(
                                    s"Error writing image file for board",
                                    ex
                                  )
                                  complete(
                                    StatusCodes.InternalServerError,
                            ...
                            case Failure(ex) =>
                              log.warning(
                                s"Error deleting old image file for board",
                                ex
                              )
                              complete(
                                StatusCodes.InternalServerError,
                    ...
                    case Failure(ex) =>
                      log.warning(
                        s"Error listing current image file for board",
                        ex
                      )
                      complete(
                        StatusCodes.InternalServerError,
  ...
  def deletePicture(implicit
    ...
    delete {
      path("""[a-zA-Z0-9]+""".r) { handIds =>
        val handId = IndividualDuplicateHand.id(handIds)
          ...
          rimdf match {
            case Right(imdf) =>
              ...
              onComplete(Future.foldLeft(deletes)(Result.unit) { (ac, v) =>
                ...
                case Success(value) =>
                  log.fine(
                    s"RestNestedPicture.delete(${dupId},${boardId},${handId}): deleted image"
                  )
                  complete(StatusCodes.NoContent)
                case Failure(ex) =>
                  log.warning(
                    s"Error deleting image file (${dupId},${boardId},${handId}): ${ex}",
                    ex
                  )
                  complete(
                    StatusCodes.InternalServerError,
        ...
        onComplete(fut) {
          ...
          case Failure(ex) =>
            log.warning(
              s"Error deleting image file (${dupId},${boardId},${handId})",
              ex
            )
            complete(
              StatusCodes.InternalServerError,
