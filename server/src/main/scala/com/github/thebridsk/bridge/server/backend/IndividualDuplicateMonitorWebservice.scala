...
import com.github.thebridsk.bridge.server.backend.StoreMonitor.NewParticipantSSEIndividualDuplicate
...
@Path("")
class IndividualDuplicateMonitorWebservice(
...
) extends MonitorWebservice[IndividualDuplicate.Id, IndividualDuplicate](
      totallyMissingResourceHandler
    ) {
  val log: LoggingAdapter = Logging(system, classOf[IndividualDuplicateMonitorWebservice])
  val monitor = new StoreMonitorManager(
    system,
    bridgeService.individualduplicates,
    classOf[IndividualDuplicateStoreMonitor],
    NewParticipantSSEIndividualDuplicate.apply _,
    service
  )
                    ...
                    extractClientIP { ip =>
                      {
                        ...
                        complete {
                          val dupid = IndividualDuplicate.id(id)
