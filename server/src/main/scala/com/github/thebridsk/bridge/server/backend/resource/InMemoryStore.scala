...
class InMemoryPersistent[VId <: Comparable[VId], VType <: VersionedInstance[
  VType,
  VType,
  VId
]](implicit
  ...
  override def putToPersistent(
    ...
    log.fine(
      s"""putToPersistent id=${support.idSupport.toString(id)}, v=${v}"""
    )
    Future {
  ...
  override def deleteFromPersistent(
    ...
    log.fine(s"""deleteFromPersistent id=${support.idSupport.toString(
      id
  ...
  def write(
    ...
    synchronized {
      metadataStore.get(id) match {
        case Some(old) =>
          val n =
            MDData(targetFile, data) :: old.filter(d => d.name != targetFile)
...
class InMemoryStore[VId <: Comparable[VId], VType <: VersionedInstance[
  VType,
  VType,
  VId
]](
