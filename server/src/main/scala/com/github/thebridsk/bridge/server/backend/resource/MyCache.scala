...
class MyCache[K, V](
  ...
  def update(
      key: K,
      generateNewValue: Option[Future[V]] => Future[V],
      currentValue: Option[() => Future[V]] = None
  ): Future[V] = {
    synchronized {
      ...
      oldv match {
        case Some(f) =>
          log.finer(
            s"Cache update ${key} got future to current value ${Implicits.toObjectId(f)}"
          )
        case None =>
          log.finer(
  ...
  def createOnlyIfNotExist(
      key: K,
      block: () => Future[V],
      existreturn: => Future[V]
  ): Future[V] = {
    synchronized {
