...
Options:""")
  ...
  val optionSort = opt[String]( "sort",
                                short = 's',
                                descr = "the sort mechanism to use, valid values are id, created.  default is created",
                                required = false,
                                validate = (s)=>s=="id"||s=="created",
                                default = Some("created") )

  val paramTarget = trailArg[Path](name="target",
...
//  }
  ...
  def executeSubcommand(): Int = {
    ...
    try {
      ...
      copy( "Duplicate", datastore.duplicates, outdatastore.duplicates, sortByMDId _, sortByMDCreated _, idFilter _ )

      copy( "DuplicateResult", datastore.duplicateresults, outdatastore.duplicateresults, sortByMDRId _, sortByMDRCreated _, idFilter _ )

      copy( "Chicago", datastore.chicagos, outdatastore.chicagos, sortByMCId _, sortByMCCreated _, idFilter _ )

      copy( "Rubber", datastore.rubbers, outdatastore.rubbers, sortByMRId _, sortByMRCreated _, idFilter _ )

      copyIfNotExist( "Boardsets", datastore.boardSets, outdatastore.boardSets, idFilter _ )

      copyIfNotExist( "Movements", datastore.movements, outdatastore.movements, idFilter _ )
      ...
      case Right(dups) =>
        ...
        keys.filter( id => idfilter(id) ).foreach { id =>
          ...
          if (await( out.read(id) ) match {
            ...
            case Right(v) =>
              if (md == v) {
                log.info( s"Not copying ${name} ${id}, already in the target store" )
                false
              } else {
                true
              }
