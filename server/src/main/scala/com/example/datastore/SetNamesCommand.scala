...
Options:""")
  ...
  val optionIds = opt[List[String]]( "ids",
                                     short='i',
                                     descr="the ids to map",
                                     required=false,
                                     default=None )

  val optionSort = opt[String]( "sort",
                                noshort = true,
                                descr = "the sort mechanism to use, valid values are id, created.  default is id",
                                required = false,
                                validate = (s)=>s=="id"||s=="created",
                                default = Some("id") )

  val optionTarget = opt[Path]("target",
                               short='t',
                               descr="directory for new datastore, default is to overwrite existing",
                               required=false,
                               default=None)

  val optionAdd = toggle( "add",
                          default=Some(false),
                          noshort=true,
                          descrYes="add modified items to datastore, generates new Ids",
                          descrNo="replace modified items to new datastore, uses same Id as original" )
...
//  }
  ...
  def sortByMDCreated( t1: MatchDuplicate, t2: MatchDuplicate ): Boolean = t1.created < t2.created
  def sortByMDRCreated( t1: MatchDuplicateResult, t2: MatchDuplicateResult ): Boolean = t1.played < t2.played
  def sortByMCCreated( t1: MatchChicago, t2: MatchChicago ): Boolean = t1.created < t2.created
  def sortByMRCreated( t1: MatchRubber, t2: MatchRubber ): Boolean = t1.created < t2.created

  def sortByMDId( t1: MatchDuplicate, t2: MatchDuplicate ): Boolean = Id.idComparer(t1.id.toString(), t2.id.toString()) < 0
  def sortByMDRId( t1: MatchDuplicateResult, t2: MatchDuplicateResult ): Boolean = Id.idComparer(t1.id.toString(), t2.id.toString()) < 0
  def sortByMCId( t1: MatchChicago, t2: MatchChicago ): Boolean = Id.idComparer(t1.id.toString(), t2.id.toString()) < 0
  def sortByMRId( t1: MatchRubber, t2: MatchRubber ): Boolean = Id.idComparer(t1.id.toString(), t2.id.toString()) < 0
...
//        ( new BridgeServiceInMemory(true), true )
      ...
      val nameMap = optionMapFileName.toOption.map { m => getNamesMap(new File(m)) }.getOrElse(Map())
      ...
      change( "Duplicate", datastore.duplicates, outdatastore.duplicates, istemp, sortByMDId _, sortByMDCreated _, getidd _, add, idFilter _ ) { (id,md) =>
        md.modifyPlayers(nameMap)
      }

      change( "DuplicateResult", datastore.duplicateresults, outdatastore.duplicateresults, istemp, sortByMDRId _, sortByMDRCreated _, getiddr _, add, idFilter _ ) { (id,mdr) =>
        mdr.modifyPlayers(nameMap)
      }

      change( "Chicago", datastore.chicagos, outdatastore.chicagos, istemp, sortByMCId _, sortByMCCreated _, getidc _, add, idFilter _ ) { (id,md) =>
        md.modifyPlayers(nameMap)
      }

      change( "Rubber", datastore.rubbers, outdatastore.rubbers, istemp, sortByMRId _, sortByMRCreated _, getidr _, add, idFilter _ ) { (id,md) =>
        md.modifyPlayers(nameMap)
      }
   ...
   * @param dest
   */
  def copyStore[K, T <: VersionedInstance[T,T,K]]( name: String,
                      src: Store[K,T],
                      dest: Store[K,T] ) = {
  ...
  def setValue[K, T <: VersionedInstance[T,T,K]]( name: String,
                     out: Store[K,T],
                     id: K,
                     newmd: T,
                     getid: T=>K,
                     add: Boolean ): Option[K] = {
    if (add) {
  ...
  def change[K, T <: VersionedInstance[T,T,K]](
    ...
    await(in.readAll()) match {
      case Right(dups) =>
        val keys = dups.values.toList.sortWith(comparer).map { t => t.id }
  ...
  def getNamesMap( r: BufferedSource, name: String ): Map[String,String] = {
    r.getLines().zipWithIndex.flatMap{ case (line,index) =>
      line match {
        ...
        case _ =>
          ...
          throw new Exception(s"Line not valid in ${name}:${linenumber}: ${line}")
      }
    }.toMap
  }
