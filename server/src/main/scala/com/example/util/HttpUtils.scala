...
trait HttpUtils {
   ...
   */
  private def readAllBytesAndCloseInputStream( contentEncoding: Option[String], is: InputStream, out: OutputStream ): Unit = {
    try {
      var len = 0
   ...
   * @throws IOException if there was an error
   */
  def getHttpAllBytes( url: URL, out: OutputStream, followRedirects: Boolean = false ): ResponseFromHttp = {
   ...
   * @throws IOException if there was an error
   */
  def getHttpAsString( url: URL, charset: String = "UTF8", followRedirects: Boolean = false ): ResponseFromHttp = {
    var conn: HttpURLConnection = null
    try {
      conn = url.openConnection().asInstanceOf[HttpURLConnection]
      conn.setInstanceFollowRedirects(followRedirects)
      ...
      val b = new InputStreamReader( new ByteArrayInputStream(baos.toByteArray()), charset )
   ...
   */
  def copyFromWeb( url: URL, out: OutputStream, hashalgo: String = "SHA-256", followRedirects: Boolean = false ): ResponseFromHttp = {
   ...
   * @return a tuple2, the first is the status code from the HTTP GET, the second is the <i>hash</i> hash of the file contents.
   */
  def copyFromWebToFile( url: URL, outfile: File, hashalgo: String = "SHA-256", followRedirects: Boolean = false ): ResponseFromHttp = {
...
object HttpUtils extends HttpUtils {
  ...
  case class ResponseFromHttp( val status: Int, location: Option[String], contentencoding: Option[String], data: Option[String]=None, hash: Option[String] = None )
