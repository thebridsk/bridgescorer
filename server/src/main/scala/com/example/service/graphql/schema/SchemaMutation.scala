  ...
  val MutationImportType = ObjectType(
      ...
      fields[BridgeService,BridgeService](
          ...
          Field("importduplicate",
                ...
                resolve = ctx => ImportAction.importDuplicate(ctx).map { md => (None,md) }
          ),
          Field("importduplicateresult",
                ...
                resolve = ctx => ImportAction.importDuplicateResult(ctx).map { md => (None,md) }
          ),
          Field("importrubber",
                MatchRubberType,
                description = Some("Import a rubber match."),
                arguments = ArgRubberId::Nil,
                resolve = ctx => ImportAction.importRubber(ctx).map { md => (None,md) }
          ),
          Field("importchicago",
                MatchChicagoType,
                description = Some("Import a chicago match."),
                arguments = ArgChicagoId::Nil,
                resolve = ctx => ImportAction.importChicago(ctx).map { md => (None,md) }
          ),
          Field("delete",
                BooleanType,
