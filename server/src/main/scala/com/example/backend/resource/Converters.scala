 ...
 */
class JsonYamlConverter( primary: Converter, secondary: Converter* ) extends Converter {

  def isPossible( s: String ): Boolean = primary.isPossible(s) || secondary.find( sec => sec.isPossible(s) ).isDefined
    ...
    if (primary.isPossible(s)) {
      ...
      for ( sec <- secondary ) {
        try {
          ...
          case x: Exception =>
            Converter.log.fine(s"""Error parsing with ${sec.getClass.getName}: ${s}""",x)
            lastx = Some(x)
        }
      }
   ...
   */
  override
  def getReadExtensions(): List[String] = secondary.foldLeft( primary.getReadExtensions() ) { (ac,s) =>
