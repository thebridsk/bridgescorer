...
class MultiPersistentSupport[VId,VType <: VersionedInstance[VType,VType,VId]](
  ...
  def getFromPersistent(
    ...
    @tailrec
    def get( list: List[PersistentSupport[VId,VType]], future: Future[Result[VType]] ): Future[Result[VType]] = {
      if (list.isEmpty) {
        future.transform { t =>
          t.map { r =>
            if (r.isOk) r
  ...
  def retryAfterDeleteMsg( id: VId ) = s"Resource $resourceURI/$id not found, retry"
   ...
   */
  override
  def resultAfterDelete( id: VId ): Result[VType] = Result(StatusCodes.RetryWith,RestMessage(retryAfterDeleteMsg(id)))
   ...
   */
  override
  def readCheckForDelete( id: VId, cachedValue: Result[VType] ) = cachedValue match {
    case Left((statuscode,RestMessage(msg))) if statuscode==StatusCodes.RetryWith && msg==retryAfterDeleteMsg(id) => true
    case _ => false
  }
...
object MultiPersistentSupport {
  ...
  def createFileAndResource[VId,VType <: VersionedInstance[VType,VType,VId]] (
    ...
    apply( new FilePersistentSupport[VId,VType](directory),
  ...
  def createInMemoryAndResource[VId,VType <: VersionedInstance[VType,VType,VId]] (
    ...
    apply( new InMemoryPersistent[VId,VType](),
           new JavaResourcePersistentSupport[VId,VType](resourcedirectory,masterfile,loader)
           )
  }
