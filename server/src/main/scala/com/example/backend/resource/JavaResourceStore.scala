 ...
 */
class JavaResourcePersistentSupport[VId,VType <: VersionedInstance[VType,VType,VId]](
  ...
  log.fine(s"JavaResourcePersistentSupport ${resourceURI} resourceDirectory=${resourcedirectory}, masterfile=${masterfile}")
  ...
  private def getSource( resource: String ) = {
    ...
    log.fine(s"JavaResourcePersistentSupport ${resourceURI} looking for ${res} resourceDirectory=${resourcedirectory}, masterfile=${masterfile}")
    ...
    try {
      ...
      getSource(f).flatMap { d =>
        d.map { source =>
          ...
          if (!goodInStore) {
            log.warning("Entry is not good in store, resource="+f+" json is "+json)
          }
          vt
        }.opt
      }
  ...
  def getAllIdsFromPersistent(cacheKeys: ()=>Set[VId]): Set[VId] = {
    ...
    knownIds.getOrElse {
      ...
      import resource._
      val map = getSource(resdir+masterfile).flatMap { mf =>
        mf.map { mfile =>
    ...
    log.fine(s"JavaResourcePersistentSupport ${resourceURI} resourceDirectory=${resourcedirectory}, masterfile=${masterfile}, Ids=${ret}")
    ret
  }
...
//    ).logit("JavaResourcePersistentSupport.getFromPersistent")
  ...
  def read( id: VId ): Result[VType] = synchronized {
    ...
    def read( list: List[String] ): Result[VType] = {
      if (list.isEmpty) {
        log.warning(s"Did not find resource $resourceURI/${id}, tried Java resources ${potential}")
        notFound(id)
      }
      else {
        val f = list.head
