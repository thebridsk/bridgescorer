...
class NestedResources[PVId, PVType, NVId, NVType](
   ...
   */
  def createChild( newvalue: NVType,
  ...
  def updateAll(
    ...
    parent.update(
      new Updator[PVType, PVType, Map[NVId,NVType]] {
        ...
        def responder( r: Result[(PVType,PVType)]): Result[Map[NVId,NVType]] = {
   ...
   */
  def update[T,R]( id: NVId,
                   updator: Updator[NVType,T,R]
                 )(
                   implicit caller: SourcePosition
                 ): Future[Result[R]] = {
    ...
    val u = new Updator[PVType,T,R] {
      override
      val changeContext = updator.changeContext
