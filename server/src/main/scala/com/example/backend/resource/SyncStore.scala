...
class SyncStore[VId, VType <: VersionedInstance[VType,VType,VId]]( store: Store[VId, VType]) {

  val resourceURI = store.resourceURI

  def support: StoreSupport[VId,VType] = store.support

  private def await[T](timeout: Duration)( fut: Future[T] ) = Await.result(fut, timeout)
