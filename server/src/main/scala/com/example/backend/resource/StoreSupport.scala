...
object StoreSupport {
  val readOnlyStoreError = Result(StatusCodes.BadRequest,RestMessage("Store is read only"))
}

abstract
class StoreSupport[VId, VType <: VersionedInstance[VType,VType,VId]](
                     val idprefix: String,
   ...
   */
  def setId( id: VId, v: VType, forCreate: Boolean ): VType = v.setId(id, forCreate, dontUpdateTime)
