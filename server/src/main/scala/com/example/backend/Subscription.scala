...
class DuplicateSubscription( id: String, actor: ActorRef, val matchid: Id.MatchDuplicate ) extends Subscription(id,actor) {

  def this( subscription: Subscription, matchid: Id.MatchDuplicate ) = this(subscription.id,subscription.actor,matchid)

  def isMatch( mid: Id.MatchDuplicate ) = matchid==mid

  override
  def needsStoreMonitored = true
}
...
object DuplicateSubscription {
  ...
  def unapply( obj: DuplicateSubscription ): Option[(String, ActorRef, Id.MatchDuplicate)] = {
...
class ChicagoSubscription( id: String, actor: ActorRef, val matchid: Id.MatchChicago ) extends Subscription(id,actor) {
...
object ChicagoSubscription {
  ...
  def unapply( obj: ChicagoSubscription ): Option[(String, ActorRef, Id.MatchChicago)] = {
    Some(( obj.id, obj.actor, obj.matchid ))
  }
...
class RubberSubscription( id: String, actor: ActorRef, val matchid: String ) extends Subscription(id,actor) {
...
abstract class Subscriptions {
  ...
  def dispatchTo( message: DuplexMessage, id: String )(implicit sender: ActorRef = Actor.noSender) = {
    get(id) match {
      case Some(sub) => sub ! message
      case None => throw new IllegalArgumentException(s"Subscription with id ${id} not found")
    }
  }

  def dispatchToAll( message: DuplexMessage )(implicit sender: ActorRef = Actor.noSender): Unit = {
  ...
  def dispatchToFiltered( message: DuplexMessage )( filt: Subscription => Boolean )(implicit sender: ActorRef = Actor.noSender) = {
