...
""")
  ...
  private def getServer( expectRunning: Boolean, startServer: Boolean = false ) = {
    server match {
      case Some(s) =>
        if (expectRunning) s
   ...
   */
  def start( interface: String = "loopback",
...
//    trustManagerFactory.init(ks)
  ...
  def redirectRoute( scheme: String, port: Int ) =
    extractUri { uri =>
      redirect(uri.withScheme(scheme).withPort(port), StatusCodes.PermanentRedirect)
    }
  ...
  def start( interface: String = "loopback",
    ...
    val myService = new MyService {
      ...
      override
      lazy val cacheDuration = cache.getOrElse(Duration(defaultCacheFor))
    ...
    bindingHttp.foreach { f =>
      f.onComplete(_ match {
        case Success(binding) =>
          ...
          log.info(s"Server is listening on ${localAddress.getHostName}:${localAddress.getPort} for HTTP")
        case Failure(e) =>
          log.severe(s"HTTP binding failed with ${e.getClass.getName} ${e.getMessage}")
//          system.terminate()
      })
    }
    bindingHttps.foreach { f =>
      f.onComplete(_ match {
        case Success(binding) =>
          val localAddress = binding.localAddress
          log.info(s"Server is listening on ${localAddress.getHostName}:${localAddress.getPort} for HTTPS")
        case Failure(e) =>
          log.severe(s"HTTPS binding failed with ${e.getClass.getName} ${e.getMessage}")
//          system.terminate()
      })
    }
    ...
    f.onComplete(_ match {
      ...
      case Failure(e) =>
        log.severe(s"Waiting for bindings to finish failed with ${e.getMessage}")
    })
    ...
    // not sure this works
      ...
      val stophttp = bindingHttp.map { binding =>
        ...
        f.onComplete(_ match {
          ...
          case Failure(e) => log.severe(s"Unbinding HTTP server failed with ${e.getMessage}")
        })
        f
      }
      val stophttps = bindingHttps.map {  binding =>
        ...
        f.onComplete(_ match {
          ...
          case Failure(e) => log.severe(s"Unbinding HTTPS server failed with ${e.getMessage}")
        })
        f
      }
