...
keytool -genkeypair -v -alias bridgescorekeeperca -dname "CN=BridgeScoreKeeperCA, OU=BridgeScoreKeeper, O=BridgeScoreKeeper, L=New York, ST=New York, C=US" -keystore examplebridgescorekeeperca.jks -keypass:env PW -storepass:env PW -keyalg RSA -keysize 4096 -ext KeyUsage:critical="keyCertSign" -ext BasicConstraints:critical="ca:true" -validity 9999

rem # Export the bridgescorekeeperca public certificate as examplebridgescorekeeperca.crt so that it can be used in trust stores.
keytool -export -v -alias bridgescorekeeperca -file examplebridgescorekeeperca.crt -keypass:env PW -storepass:env PW -keystore examplebridgescorekeeperca.jks -rfc
...
rem # Tell bridgescorekeeperca to sign the bridgescorekeeper certificate. Note the extension is on the request, not the
rem # original certificate.
rem # Technically, keyUsage should be digitalSignature for DHE or ECDHE, keyEncipherment for RSA.
keytool -gencert -v -alias bridgescorekeeperca -keypass:env PW -storepass:env PW -keystore examplebridgescorekeeperca.jks -infile examplebridgescorekeeper.csr -outfile examplebridgescorekeeper.crt -ext KeyUsage:critical="digitalSignature,keyEncipherment" -ext EKU="serverAuth" -ext SAN="DNS:bridgescorekeeper" -rfc

rem # Tell examplebridgescorekeeper.jks it can trust bridgescorekeeperca as a signer.
echo yes >yes
keytool -import -v -alias bridgescorekeeperca -file examplebridgescorekeeperca.crt -keystore examplebridgescorekeeper.jks -storetype JKS -storepass:env PW < yes
...
bash -c "openssl pkcs12 -nocerts -nodes -passout env:PW -passin env:PW -in examplebridgescorekeeper.p12 -out examplebridgescorekeeper.key"

rem # Create a JKS keystore that trusts the example CA, with the default password.
keytool -import -v -alias bridgescorekeeperca -file examplebridgescorekeeperca.crt -keypass:env PW -storepass %tpw% -keystore examplebridgescorekeepertrust.jks < yes
