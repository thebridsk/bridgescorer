...
object DuplexPipe {
  ...
  implicit def ackToResponse(
      ack: Ack
  ): Either[Protocol.ToBrowserMessage, Exception] => Unit =
    (resp: Either[Protocol.ToBrowserMessage, Exception]) =>
      resp match {
...
/**
  ...
  */
class DuplexPipe(url: String, protocol: String = "") {
  ...
  /**
    ...
    */
  def removeListener(l: Listener): List[Listener] =
    listeners.filter { listener => listener != l }
  ...
  /**
    ...
    */
  def request(
      req: Protocol.ToServerMessage,
      response: Response,
      timeoutSeconds: Int = 60
  ): Unit = {
    val seq = nextSeq()
    prepareForSending(
      DuplexProtocol.Request(req, seq, false),
      Some(seq),
      Some(response)
    )
  }
  ...
  /**
    ...
    */
  def send(
      data: Protocol.ToServerMessage,
      ack: Ack,
      timeoutSeconds: Int = 60
  ): Unit = {
    val seq = nextSeq()
    prepareForSending(
      DuplexProtocol.Request(data, seq, true),
      Some(seq),
      Some(ack)
    )
  }
  ...
  /**
    ...
    * @param data the data to send
    */
  def send(data: Protocol.ToServerMessage): Unit =
    prepareForSending(DuplexProtocol.Send(data))
  ...
  private def notifyUnsolicited(data: Protocol.ToBrowserMessage) =
    listeners.foreach { l => l.onMessage(data) }

  private class Pending(
      val data: DuplexProtocol.DuplexMessage,
      val seq: Option[Int],
  ...
  private def prepareForSending(
      data: DuplexProtocol.DuplexMessage,
      seq: Option[Int] = None,
      resp: Option[Response] = None,
      timeoutSeconds: Int = 60
  ) = {
    log.fine(
      "PrepareForSending seq=" + seq + " timeout=" + timeoutSeconds + ": " + data
    )
  ...
  private def kick(): Unit = {
    try {
      // no resending
      log.fine(
        "Kick pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + websocket.bufferedAmount + "/" + maxBufferedAmount
      )
      ...
      if (!pending.isEmpty) {
        if (websocket.readyState == WebSocket.OPEN) {
          log.fine(
            "Kick trying to send pending, pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + websocket.bufferedAmount + "/" + maxBufferedAmount
          )
          while (
            !pending.isEmpty && websocket.bufferedAmount < maxBufferedAmount
          ) {
          ...
          log.fine(
            "Kick done sending pending, pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + websocket.bufferedAmount + "/" + maxBufferedAmount
          )
          if (!pending.isEmpty) kickagain = true
        } else {
          log.warning(
            "Kick found websocket was not open: " + websocket.readyState
          )
        }
      }
      ...
      log.fine(
        "Kick return pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + websocket.bufferedAmount + "/" + maxBufferedAmount
      )
    } catch {
      case t: Throwable =>
  ...
  private def fromWebsocket(msg: DuplexProtocol.DuplexMessage) = {
    ...
    log.fine(
      "fromWebsocket pending=" + pending.size + ", outstanding=" + outstanding.size
    )
  ...
  private class WS(url: String, protocol: String)
    ...
    override def onClose(wasClean: Boolean, code: Int, reason: String): Unit = {
      log.info(
        "WS.onClose: wasClean=" + wasClean + ", code=" + code + ", reason=" + reason + ", codestring=" + MyWebsocket
          .getMsgFromCode(Code(code))
      )
