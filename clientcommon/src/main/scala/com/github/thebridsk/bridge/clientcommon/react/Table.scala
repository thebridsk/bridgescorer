    ...
    def compare( cols: List[ColumnBase], row1: Row, row2: Row, index: Int ) = {
      ...
      def comp( orderlist: List[(String, Option[Ordering[Any]], Boolean)] ): Int = {
        if (orderlist.isEmpty) 0
          ...
          val id = cur._1
          cols.find( c => if (c.isInstanceOf[SortableColumn[_]]) c.asInstanceOf[SortableColumn[_]].id == id else false ) match {
            case Some(nextcol: SortableColumn[_]) =>
              val i = cols.indexWhere( c => if (c.isInstanceOf[SortableColumn[_]]) c.asInstanceOf[SortableColumn[_]].id == id else false )
              val order = cur._2.getOrElse( nextcol.asInstanceOf[SortableColumn[Any]].sorter.ordering )
...
//      log.fine(s"MultiColumnSort rc=${rc},\n  row1=${row1},\n  row2=${row2}")
  ...
  object MultiColumnSort {
    ...
    def apply[T]( cols: (String, Option[Ordering[_]], Boolean)* )(implicit ordering: Ordering[T]) = new MultiColumnSort(cols:_*)(ordering)
    def create2[T](cols: (String,Boolean)* )(implicit ordering: Ordering[T]) = new MultiColumnSort(cols.map(c=>(c._1,None,c._2)):_*)(ordering)
    ...
    def render( props: Props, state: State ) = {
      ...
      val rows = state.currentSort.map { sortid =>
                   props.columns.indexWhere { c =>
                     if (c.isInstanceOf[SortableColumn[_]]) c.asInstanceOf[SortableColumn[_]].id == sortid
                     else false
                   } match {
                     case sort if sort < 0 =>
                       props.rows
