...
object DebugLoggerComponent {
  ...
  def setLoggingLevel(loggername: String, level: Level): Unit = {
    ...
    target.getHandlers.find(h => h.isInstanceOf[DebugLoggerHandler]) match {
      case Some(h) =>
        logger.info(
          s"On ${targetdisp} setting debug logger trace level to ${level.name}"
        )
        h.level = level
      case None =>
        logger.info(
          s"On ${targetdisp} starting debug logger trace with level ${level.name}"
        )
...
object DebugLoggerComponentInternal {
  ...
  /**
    ...
    */
  case class State(
      level: Level = DebugLoggerComponent.getLoggingLevel(
        DebugLoggerComponentInternal.rootlogger
      )
  ) {
  ...
  def format(msg: TraceMsg): String = {
    var ret =
      s"${DateUtils.formatLogTime(msg.time)} ${msg.level.short} ${msg.pos.fileName}:${msg.pos.lineNumber} ${substitute(msg.message, msg.args: _*)}"
    if (msg.cause != null) {
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def setLoggingLevelCB(level: Level): Callback =
      scope.modState { s =>
    ...
    def render(props: Props, state: State) = { // scalafix:ok ExplicitResultTypes; React
      val txt = LoggerStore
        .getMessages()
        .map(m => m.i.toString + ": " + format(m.traceMsg))
        .mkString("\n")
      ...
      <.div(
        ...
        <.div(
          ...
          AppButton("StopLogs", "Stop Logs", ^.onClick --> stopLogs())
            .when(enabled),
          AppButton("StartLogs", "Start Logs", ^.onClick --> startLogs())
            .unless(enabled),
          Level.allLevels
            .filter(l => l != Level.STDERR && l != Level.STDOUT)
            .map { l =>
              val styl =
                if (state.level != l) baseStyles.normal
                else baseStyles.buttonSelected
              AppButton(
                l.name,
                l.name,
                ^.onClick --> setLoggingLevelCB(l),
                styl
              ).when(enabled)
            }
            .toTagMod
  ...
  private[debug] val component = ScalaComponent
    .builder[Props]("DebugLoggerComponent")
