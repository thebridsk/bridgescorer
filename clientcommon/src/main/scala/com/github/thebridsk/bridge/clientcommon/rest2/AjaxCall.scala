...
trait IAjaxCall {
  def send(
      ...
      responseType: String
  )(implicit pos: SourcePosition): AjaxResult[WrapperXMLHttpRequest]
}
...
object AjaxCall extends IAjaxCall {
          ...
          case x: JsUndefined =>
            resp \ "error" match {
              ...
              case x: JsUndefined =>
                // no error
                RestMessage(
                  s"Got statusCode ${statusCode} but no error or errors field: ${x}.  Response is ${resp}"
                )
            }
        }
  ...
  def send(
    ...
    val result =
      new AjaxResult(wreq, url, data, promise.future, promise, pos.pos)

    req.onreadystatechange = CommAlerter.tryitfun { (e: dom.Event) =>
      log.fine(
        s"${method} ${url}, readyState ${readyStateToString(req.readyState)}, status ${req.status}, called from ${pos.line}"
      )
      if (req.readyState == Done) {
        if ((req.status >= 200 && req.status < 300) || req.status == 304) {
          log.fine(
            s"${method} ${url}, status ${req.status} called from ${pos.line}"
          )
          try {
    ...
    req.ontimeout = CommAlerter.tryitfun { (e: dom.Event) =>
      log.warning(
        s"${method} ${url}, timeout status ${req.status} called from ${pos.line}"
      )
      try {
...
object AjaxCallDisabled extends IAjaxCall {
  ...
  private def sendImpl(
    ...
    AjaxResult.log.warning(
      s"Ajax is disabled, ${method} ${url}, called from ${pos.line}"
    )
