...
trait Alerter {
  ...
  def tryit[T](f: => T)(implicit pos: Position): T = {
    try {
      ...
      case x: Throwable =>
        ...
        if (isAlertToConsoleEnabled)
          alert(
            s"Alerter(${pos.line}): ${x.toString()}\n${exceptionToString(x)}"
          )
        throw x
    }
  }
  ...
  def tryitWithDefault[T](
      defaultValue: => T
  )(f: => T)(implicit pos: Position): T = {
    try {
      f
    } catch {
  ...
  def tryitfun[A](f: A => Unit)(implicit pos: Position): A => Unit =
    (a: A) => tryitWithUnit { f(a) }

  def tryitfunr[A, R](
      defaultValue: => R
  )(f: A => R)(implicit pos: Position): A => R =
    (a: A) => {
      tryitWithDefault[R](defaultValue) { f(a) }
    }

  def tryit[A, R](f: A => R)(implicit pos: Position): A => R =
    (a: A) => {
      tryit[R] { f(a) }
    }
