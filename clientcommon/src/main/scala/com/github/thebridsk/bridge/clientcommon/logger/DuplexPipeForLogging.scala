...
object DuplexPipeForLogging {
  ...
  def apply(url: String, protocol: String = Protocol.Logging) =
    new DuplexPipeForLogging(url, protocol)
...
class DuplexPipeForLogging(url: String, protocol: String = "") {
  ...
  private def notifyUnsolicited(data: DuplexProtocol.DuplexMessage) =
  ...
  private def kick2(): Unit = {
    ...
    try {
      ...
      log.fine(
        "Kick pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + bufferedAmount + "/" + maxBufferedAmount
      )
      ...
      if (!pending.isEmpty) {
        if (websocket.readyState == WebSocket.OPEN) {
          log.fine(
            "Kick trying to send pending, pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + bufferedAmount + "/" + maxBufferedAmount
          )
          if (!pending.isEmpty && bufferedAmount < maxBufferedAmount) {
            ...
            bufferedAmount += l
            log.fine(
              "Kick after send, pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + bufferedAmount + "/" + maxBufferedAmount
            )
          ...
          log.fine(
            "Kick done sending pending, pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + bufferedAmount + "(" + websocket.bufferedAmount + ")/" + maxBufferedAmount
          )
        } else {
          log.warning(
      ...
      log.fine(
        "Kick return pending=" + pending.size + ", outstanding=" + outstanding.size + " bufferedAmount=" + bufferedAmount + "(" + websocket.bufferedAmount + ")/" + maxBufferedAmount
      )
    } catch {
      case t: Throwable =>
