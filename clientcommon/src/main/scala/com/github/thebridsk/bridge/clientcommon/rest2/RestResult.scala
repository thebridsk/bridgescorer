...
object RestResult {
  ...
  def tryit[T](fun: String, comment: => String)(
      f: => Try[T]
  )(implicit ct: ClassTag[T]): Try[T] = {
    try {
      val ret = f
      ret match {
        case Success(x) =>
        ...
        case Failure(ex) =>
          CommAlerter.tryAlert(
            s"${fun}: failure return type is ${ct.runtimeClass.getName}, data was:\n${comment}",
            ex
          )
      ...
      case x: Throwable =>
        CommAlerter.tryAlert(
          s"${fun}: return type is ${ct.runtimeClass.getName}, data was:\n${comment}",
          x
        )
        log.warning(
          s"${fun}: exception ${x}: return type is ${ct.runtimeClass.getName}, data was:\n${comment}\n******** ${Alerter
            .exceptionToString(x)}"
        )
        Failure(x)
    }
  ...
  implicit private def transformToT[T](
      t: Try[WrapperXMLHttpRequest]
  )(implicit classtag: ClassTag[T], reader: Reads[T]): Try[T] =
    t match {
      case Success(req) =>
      ...
      case Failure(fail) =>
        if (!fail.isInstanceOf[AjaxDisabled]) {
          RestResult.log.info(
            s"RestResult.transformToT: failure ${fail.getClass.getName} ${fail}",
            fail
          )
        }
        Failure(fail)
  ...
  private def transformToArrayT[T](
      t: Try[WrapperXMLHttpRequest]
  )(implicit classtag: ClassTag[T], reader: Reads[T]): Try[Array[T]] =
    t match {
      case Success(req) =>
        val j = req.responseText
      ...
      case Failure(fail) =>
        if (!fail.isInstanceOf[AjaxDisabled]) {
          RestResult.log.info(
            s"RestResultArray.transformToT: failure ${fail.getClass.getName} ${fail}",
            fail
          )
        }
        Failure(fail)
  ...
  implicit def ajaxToRestResultArray[T](
    ...
    new RestResultArray[T](
      ajaxResult,
      ajaxResult.transform(t => transformToArrayT(t))
    )
  }
...
trait Result[T] extends Cancellable[T] with Awaitable[T] {
  ...
  def recordFailure(
      rec: ResultRecorder = ResultRecorder
  )(implicit executor: ExecutionContext, pos: Position): Result[T]
  ...
  def map[S](f: T => S)(implicit executor: ExecutionContext): Result[S] =
    transform(_ map f)
...
class ResultObject[T](future: Future[T]) extends Result[T] {
  ...
  implicit def wrap[S](
      r: Future[S]
  )(implicit pos: Position): ResultObject[S] = {
    new ResultObject(r)
  }
  ...
  def onComplete[U](f: Try[T] => U)(implicit executor: ExecutionContext): Unit =
    future.onComplete(t => f(t))
  def transform[S](f: Try[T] => Try[S])(implicit
      executor: ExecutionContext
  ): ResultObject[S] = future.transform(t => f(t))
  def transformWith[S](f: Try[T] => Future[S])(implicit
      executor: ExecutionContext
  ): ResultObject[S] = future.transformWith(t => f(t))
  def value: Option[Try[T]] =
    future.value match {
      case None    => None
      case Some(t) => Some(t)
  ...
  def recordFailure(
      rec: ResultRecorder = ResultRecorder
  )(implicit executor: ExecutionContext, pos: Position): ResultObject[T] = {
    this
  }
}

class RestResult[T](
    val ajaxResult: AjaxResult[WrapperXMLHttpRequest],
    val future: Future[T]
  ...
  implicit def wrap[S](r: Future[S])(implicit pos: Position): RestResult[S] = {
  ...
  def transform[S](
      f: Try[T] => Try[S]
  )(implicit executor: ExecutionContext): Result[S] = {
  ...
  def value: Option[Try[T]] =
    Alerter.tryit(future.value match {
      case None    => None
      case Some(t) => Some(t)
    })
  ...
  def recordFailure(
      rec: ResultRecorder = ResultRecorder
  )(implicit executor: ExecutionContext, pos: Position): RestResult[T] = {
    ajaxResult.recordFailure(rec)
    this
  }
}

class RestResultArray[T](
    ajaxResult: AjaxResult[WrapperXMLHttpRequest],
    result: Future[Array[T]]
)(implicit pos: Position)
    extends Result[Array[T]] {
  ...
  def transform[S](
      f: Try[Array[T]] => Try[S]
  )(implicit executor: ExecutionContext): RestResult[S] = {
    result.transform(t => Alerter.tryit(f(t)))
  }
  def transformWith[S](
      f: Try[Array[T]] => Future[S]
  )(implicit executor: ExecutionContext): RestResult[S] = {
    result.transformWith(t => Alerter.tryit(f(t)))
  }
  def value: Option[Try[Array[T]]] =
    Alerter.tryit(result.value match {
      case None    => None
      case Some(t) => Some(t)
    })
  ...
  def recordFailure(
      rec: ResultRecorder = ResultRecorder
  )(implicit executor: ExecutionContext, pos: Position): RestResultArray[T] = {
    ajaxResult.recordFailure(rec)
    this
  }
