...
object LogLifecycleToServer {
  ...
  private[this] def log(m: js.Any, ps: js.Any*)(implicit
      level: Level,
      logger: Logger
  ) = {
    val msg: String =
      "" + m + (if (ps.length > 0)
                  "  args=" + ps.map { x => s"$x" }.mkString(", ");
                else "")
    CallbackTo { logger.log(level, msg) }
  }

  private[this] def logc(m: js.Any, c: js.Any, ps: js.Any*)(implicit
      level: Level,
      logger: Logger
  ) =
    log(s"$m\n ", /* c +: */ ps: _*)

  private[this] def log1(m: String)(implicit level: Level, logger: Logger) =
    (c: js.Any) => logc(m, c)

//  private[this] def logp(m: String)(implicit level: Level, logger: Logger) = (c: js.Any, p: Any) =>
//    logc(m, c, fmt("Props", p): _*)

  private[this] def logps(m: String)(implicit level: Level, logger: Logger) =
    (c: js.Any, p: Any, s: Any) =>
      logc(m, c, fmt("Props", p) ++ fmt("State", s): _*)

  private[this] def logP[P <: Product](
      m: String,
      c: P => js.Any,
      extra: P => Seq[js.Any]
  )(implicit level: Level, logger: Logger) =
    (p: P) => logc(m, c(p), extra(p): _*)
