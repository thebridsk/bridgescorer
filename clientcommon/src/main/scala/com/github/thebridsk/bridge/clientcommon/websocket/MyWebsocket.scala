...
abstract class MyWebsocket(url: String, protocol: String = "") {
  ...
  private def isCurrentWebsocket(ws: WebSocket) =
    websocket.map(w => w == ws).getOrElse(false)
        ...
        if (isCurrentWebsocket(ws)) {
          try {
            ...
            if (event.code == CLOSE_NORMAL) {
              log.fine(
                "Mywebsocket.onclose(" + event.wasClean + "," + event.code + "," + r + ") on " + url
              )
            } else {
              log.warning(
                "Mywebsocket.onclose(" + event.wasClean + "," + event.code + "," + r + ") on " + url
...
case class Code(val code: Int) {
  import MyWebsocket._
  if (
    !((code >= CLOSE_NORMAL && code <= TLS_Handshake) ||
      (code >= 3000 && code <= 3999) ||
      (code >= 4000 && code <= 4999))
  ) {
...
trait MyWebsocketCodes {
  ...
  def getMsgFromCode(code: Code): String =
    reasons.getOrElse(code.code, "Reason code " + code.code)
