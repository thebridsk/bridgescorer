...
/**
  ...
  */
class AjaxResult[T](
    val req: WrapperXMLHttpRequest,
    val url: String,
    val reqbody: InputData,
    future: Future[T],
    promise: Promise[WrapperXMLHttpRequest],
    val pos: Position
) extends Future[T]
    with Result[T] {
  ...
  implicit private def withFuture[U](newfuture: Future[U]): AjaxResult[U] =
    new AjaxResult(req, url, reqbody, newfuture, promise, pos)
  ...
  def transform[S](
      f: Try[T] => Try[S]
  )(implicit executor: ExecutionContext): AjaxResult[S] = {
  ...
  override def failed: AjaxResult[Throwable] = future.failed

  override def foreach[U](f: T => U)(implicit
      executor: ExecutionContext
  ): Unit = future.foreach(CommAlerter.tryit(f))

  override def map[S](f: T => S)(implicit
      executor: ExecutionContext
  ): AjaxResult[S] = future.map(CommAlerter.tryit(f))

  override def flatMap[S](f: T => Future[S])(implicit
      executor: ExecutionContext
  ): AjaxResult[S] = future.flatMap(CommAlerter.tryit(f))

  override def flatten[S](implicit ev: T <:< Future[S]): AjaxResult[S] =
    future.flatten(ev)

  override def filter(p: T => Boolean)(implicit
      executor: ExecutionContext
  ): AjaxResult[T] = future.filter(CommAlerter.tryit(p))

  override def collect[S](pf: PartialFunction[T, S])(implicit
      executor: ExecutionContext
  ): AjaxResult[S] = future.collect(pf)

  override def recover[U >: T](pf: PartialFunction[Throwable, U])(implicit
      executor: ExecutionContext
  ): AjaxResult[U] = future.recover(pf)

  override def recoverWith[U >: T](pf: PartialFunction[Throwable, Future[U]])(
      implicit executor: ExecutionContext
  ): AjaxResult[U] = future.recoverWith(pf)

  override def zip[U](that: Future[U]): AjaxResult[(T, U)] = future.zip(that)

  override def zipWith[U, R](that: Future[U])(f: (T, U) => R)(implicit
      executor: ExecutionContext
  ): AjaxResult[R] = future.zipWith(that)(f)

  override def fallbackTo[U >: T](that: Future[U]): AjaxResult[U] =
    future.fallbackTo(that)

  override def mapTo[S](implicit tag: ClassTag[S]): AjaxResult[S] =
    future.mapTo(tag)

  override def andThen[U](pf: PartialFunction[Try[T], U])(implicit
      executor: ExecutionContext
  ): AjaxResult[T] = future.andThen(pf)
  ...
  /**
    ...
    */
  def mapErrorReturn[U](implicit
      executor: ExecutionContext,
      reader: Reads[U],
      classtag: ClassTag[U]
  ): AjaxResult[U] = {
    future.onlyExceptions.map { t =>
  ...
  /**
    ...
    */
  def onlyExceptions(implicit
      executor: ExecutionContext
  ): AjaxResult[Throwable] = {
  ...
  )(implicit executor: ExecutionContext, pos: Position): AjaxResult[T] = {
...
object AjaxResult {
  ...
  def get(
      ...
      responseType: String = ""
  )(implicit pos: Position): AjaxResult[WrapperXMLHttpRequest] = {
  ...
  def head(
    ...
    apply("HEAD", url, data, timeout, headers, withCredentials, responseType)(
      pos
    )
  }
  ...
  def fromJson[U](s: String, url: String, cause: Throwable = null)(implicit
      reader: Reads[U],
      classtag: ClassTag[U]
  ): U = {
  ...
  )(implicit reader: Reads[U], classtag: ClassTag[U]): U = {
    Json.fromJson[U](json) match {
      ...
      case JsError(errors) =>
        ...
        throw new JsonException(
          s"""Error unmarshalling a ${clsname} from request ${url}: ${s}""",
          cause
        )
    }
  }
