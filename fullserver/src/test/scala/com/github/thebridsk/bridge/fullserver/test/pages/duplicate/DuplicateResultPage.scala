...
class DuplicateResultPage(
  ...
  private def getWinnerSetTableElements(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ) = {
  ...
  /**
    ...
    * The columns are: place, points, players
    */
  def getPlaceTable(ws: Int)(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): List[List[String]] = {
    val all = getWinnerSetTableElements
    all(ws)
      .findAll(xpath("""./tbody/tr/td"""))
      .map(c => c.text)
      .grouped(3)
      .toList
  ...
  private def getWinnerSetMappings(table: List[List[List[String]]]) = {
    log.info(
      "DuplicateResultPage.getWinnerSetMappings: " +
        table
          .map { ws =>
            ws.mkString("\n    ", "\n    ", "")
          }
  ...
  def checkPlaceTable(places: PlaceEntry*)(implicit pos: Position): Unit = {
    ...
    places.foreach { pe =>
      ...
      log.info(
        s"""looking for place ${pe.place} points ${pe.points} teams ${pe.teams}, plc=${plc}"""
      )
      withClue(
        s"""working with place ${pe.place} points ${pe.points} teams ${pe.teams}"""
      ) {
        val ws = mapping(pe.teams.head.teamid)
        table(ws).find { row =>
          val rc = row(0) == plc
          log.info(s"""  rc=${rc} looking for ${plc} matching ${row(
            0
          )} trying to match ${row}""")
          rc
        } match {
