...
object HandPage {
  ...
  /**
    ...
    */
  def goto(chiid: String, round: Int, hand: Int, matchType: ChicagoMatchType)(
      implicit
      webDriver: WebDriver,
      patienceConfig: PatienceConfig,
      pos: Position
  ...
  def findIds(implicit
    ...
    withClue(s"Unable to determine duplicate id in HandPage: ${cur}") {
      val rest =
        if (cur.startsWith(prefix)) Some(cur.drop(prefix.length()))
        else if (cur.startsWith(prefix2)) Some(cur.drop(prefix2.length()))
        else None
      rest match {
        case Some(patternForIds(cid, roundid, handid)) =>
          (cid, roundid.toInt, handid.toInt)
...
class HandPage(
    ...
    round: Int,
    hand: Int,
    matchType: ChicagoMatchType
)(implicit webDriver: WebDriver, pageCreated: SourcePosition)
