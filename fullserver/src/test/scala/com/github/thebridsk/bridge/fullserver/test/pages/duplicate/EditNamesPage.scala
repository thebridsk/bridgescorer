...
object EditNamesPage {
  ...
  def current(implicit webDriver: WebDriver, patienceConfig: PatienceConfig, pos: Position): EditNamesPage = {
    new EditNamesPage( getCurrentDupId )
  }

  def urlFor( dupid: String ): String = TestServer.getAppPageUrl(s"duplicate/match/${dupid}/names")
  def restUrlFor( dupid: String, teamId: String ): URL = TestServer.getUrl(s"/v1/rest/duplicate/${dupid}/teams/${teamId}")

  def goto( dupid: String )(implicit webDriver: WebDriver, patienceConfig: PatienceConfig, pos: Position): EditNamesPage = {
    go to urlFor(dupid)
...
class EditNamesPage(
  ...
  def clickOK(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
   ...
   */
  def getNames(implicit patienceConfig: PatienceConfig, pos: Position): List[List[String]] = {
   ...
   */
  def checkNames( players: String * )(implicit patienceConfig: PatienceConfig, pos: Position): EditNamesPage = {
   ...
   * @param name
   */
  def checkName( teamid: Int, player: Int, name: String )(implicit patienceConfig: PatienceConfig, pos: Position): EditNamesPage = {
    val names = getNames
   ...
   * @param name
   */
  def setName( teamId: Int, player: Int, name: String )(implicit patienceConfig: PatienceConfig, pos: Position): EditNamesPage = {
    val text = eventually {
