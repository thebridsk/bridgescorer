...
object BoardPage {
  ...
  def urlFor(
    ...
    TestServer.getAppPageUrl(
      s"duplicate/match/${dupid}/table/${tableId}/round/${roundId}/boards/${boardId}"
    )

  def urlFor(dupid: String, director: Boolean, boardId: String): String = {
    val dir = if (director) "director/" else ""
    TestServer.getAppPageUrl(
      s"duplicate/match/${dupid}/${dir}boards/${boardId}"
    )
  }
...
class BoardPage(implicit val webDriver: WebDriver, pageCreated: SourcePosition)
