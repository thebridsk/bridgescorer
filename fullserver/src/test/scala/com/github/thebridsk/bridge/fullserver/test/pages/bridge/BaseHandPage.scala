...
object BaseHandPage {
  ...
  val nContractTricksButtons: Map[Int, String] =
    (1 to 7).map(i => i -> s"CT$i").toMap
  val contractTricksButtons: Map[Int, String] =
    nContractTricksButtons + (0 -> passedOutButton)
  val contractSuitButtons: Map[ContractSuit, String] =
    "NSHDC".map(c => (ContractSuit(c.toString), s"CS$c")).toMap
  val doubledButtons: Map[ContractDoubled, String] =
    "NDR".map(c => (ContractDoubled(c.toString), s"Doubled$c")).toMap
  val declarerButtons: Map[PlayerPosition, String] =
    "NEWS".map(c => (PlayerPosition(c.toString), s"Dec$c")).toMap
  val madeDownButtons: Map[MadeOrDown, String] =
    Map(Made -> "made", Down -> "down")
  val tricksButtons: Map[Int, String] = (1 to 13).map(i => i -> s"T$i").toMap

  val honorsPoints: Map[Int, String] =
    Map(0 -> "Honors0", 100 -> "Honors100", 150 -> "Honors150")
  val honPlayButtons: Map[PlayerPosition, String] =
    "NEWS".map(c => (PlayerPosition(c.toString), s"HonPlay$c")).toMap
  ...
  def setInputStyle(
      want: String
  )(implicit webDriver: WebDriver, pos: Position): Option[String] = {
    if (!validInputStyles.contains(want))
      fail(
        s"""Specified style, ${want} is not valid, must be one of ${validInputStyles}"""
      )
    val stop = Some(want)
...
abstract class BaseHandPage[T <: Page[T]](implicit
    webDriver: WebDriver,
...
//      val but = findButton("Ok")
  ...
  def clickContractSuit(
      suit: ContractSuit
  )(implicit patienceConfig: PatienceConfig, pos: Position): this.type =
    clickButton(contractSuitButtons(suit))

  def clickContractDoubled(
      doubled: ContractDoubled
  )(implicit patienceConfig: PatienceConfig, pos: Position): this.type =
    clickButton(doubledButtons(doubled))

  def clickDeclarer(
      dec: PlayerPosition
  )(implicit patienceConfig: PatienceConfig, pos: Position): this.type =
    clickButton(declarerButtons(dec))

  def clickMadeOrDown(
      madeOrDown: MadeOrDown
  )(implicit patienceConfig: PatienceConfig, pos: Position): this.type =
    clickButton(madeDownButtons(madeOrDown))

  def clickTricks(
      i: Int
  )(implicit patienceConfig: PatienceConfig, pos: Position): this.type =
    clickButton(tricksButtons(i))

  def clickHonorsPlayer(
      dec: PlayerPosition
  )(implicit patienceConfig: PatienceConfig, pos: Position): this.type =
    clickButton(honPlayButtons(dec))

  def clickHonors(
      i: Int
  )(implicit patienceConfig: PatienceConfig, pos: Position): this.type =
    clickButton(honorsPoints(i))
...
//    clickButton( "Ok" )
  ...
  def clickClear(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): this.type = clickButton("Clear")

  def clickChangeScorekeeper(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): this.type = clickButton("ChangeSK")

  def clickInputStyle(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): this.type = clickButton("InputStyle")
  ...
  def enterPassedOutContract(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): this.type = clickContractTrick(0)
  ...
  def enterContract(
    ...
    logMethod0(
      s"""Entering contract ${contractTricks} ${contractSuit.suit} ${contractDoubled.doubled} by ${declarer.pos} ${madeOrDown.made} ${tricks} ${honors} ${honorsPlayer}"""
    ) {
  ...
  def isContractTricksVisible(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean =
    eventually {
  ...
  def isContractSuitVisible(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean =
    eventually {
  ...
  def isContractDoubledVisible(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean =
    eventually {
  ...
  def isDeclarerEnabled(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean =
    eventually {
  ...
  def isMadeOrDownVisible(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean =
    eventually {
  ...
  def isHonorsPointsVisible(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean =
    eventually {
  ...
  def isHonorPlayersVisible(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean =
    eventually {
  ...
  def isDuplicateBridge(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Boolean = {
  ...
  /**
    ...
    */
  def getSelectedContractTricks(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Option[Int] = {
...
//  }
  ...
  /**
    ...
    * @throws TestFailedException if more than one is selected
    */
  def getSelectedContractSuit(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Option[ContractSuit] = {
  ...
  /**
    ...
    * @throws TestFailedException if more than one is selected
    */
  def getSelectedContractDoubled(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Option[ContractDoubled] = {
  ...
  /**
    ...
    * @throws TestFailedException if more than one is selected
    */
  def getSelectedDeclarer(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ...
  /**
    ...
    * @throws TestFailedException if more than one is selected
    */
  def getSelectedMadeOrDown(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Option[MadeOrDown] = {
    val x =
      """//button[@id='made' or @id='down'][contains(concat(' ', @class, ' '), ' baseButtonSelected ')]"""
...
//    getSelected( "MadeOrDown", madeDownButtons )
  ...
  def findTrickButtons(implicit
    ...
    val tbuttons =
      findElemsByXPath(x).map(e => e.attribute("id").getOrElse("<Unknown>"))

    tricksButtons
      .flatMap(e => if (tbuttons.contains(e._2)) e._1 :: Nil else Nil)
      .toList
...
//    })
  ...
  /**
    ...
    * @throws TestFailedException if more than one is selected
    */
  def getSelectedTricks(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Option[Int] = {
...
//    selected.headOption
  ...
  /**
    ...
    * @throws TestFailedException if more than one is selected
    */
  def getSelectedHonors(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Option[Int] = {
  ...
  /**
    ...
    * @throws TestFailedException if more than one is selected
    */
  def getSelectedHonorsPlayer(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Option[PlayerPosition] = {
  ...
  private def getSelected[T](
      name: String,
      map: Map[T, String],
      prefix: String
  ): Option[T] =
    logMethod(s"getSelected($name)") {

      val x =
        s"""//button[starts-with( @id, '${prefix}' ) and contains(concat(' ', @class, ' '), ' baseButtonSelected ')]"""
...
//    }
  ...
  def getScore(implicit
    ...
    logMethod("getScore") {
      eventually {
        findElemsByXPath(
          "//div[contains(concat(' ', @class, ' '), ' handSectionScore ')]/div"
        ) match {
          case List(r1, r2, r3) => (r1.text, r2.text, r3.text)
          case List(r1, r3) =>
            findElemsByXPath(
              "//div[contains(concat(' ', @class, ' '), ' contractAndResult ')]"
            ) match {
  ...
  def validateContract(
    ...
    log.info(
      s"""validate contract ${contractTricks} ${contractSuit} ${contractDoubled} by ${declarer.get.pos} ${madeOrDown} ${tricks} ${honors} ${honorsPlayer}"""
    )
    ...
    def compare[T](value: Option[T], selected: => Option[T]) = {
      selected match {
        ...
        case None =>
          if (value.isDefined)
            fail(s"${pos.line}: selected is None, expecting $value")
      }
    }
  ...
  def findVulnerableElement(
    ...
    val e = find(
      xpath(
        s"""//span[@id = '${loc.name}']/span[translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')]"""
      )
    )
    e
  }
  ...
  def checkDealer(name: String)(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): BaseHandPage[T] with T = {
  ...
  def checkVulnerable(loc: PlayerPosition, vul: Vulnerability)(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): Assertion = {
