...
//  case class MyDuration( timeout: Long, units: TimeUnit )
  ...
  it should "go to the home page" in {
    ...
    waitForFutures(
      "Starting browsers",
      CodeBlock {
        SessionTable1.sessionStart(getPropOrEnv("SessionTable1")).setQuadrant(4)
      },
      CodeBlock {
        SessionTable2.sessionStart(getPropOrEnv("SessionTable2")).setQuadrant(3)
      },
      CodeBlock {
        SessionComplete
...
//  }
  ...
  it should "allow players names to be entered at both tables" in {
    ...
    waitForFutures(
      ...
      CodeBlock {
        ...
        val scoreboard = en.clickOK
          .asInstanceOf[ScoreboardPage]
          .validate(allHands.getBoardsInTableRound(1, 1))
      },
      CodeBlock {
        import SessionTable2._
        ...
        val scoreboard = en.clickOK
          .asInstanceOf[ScoreboardPage]
          .validate(allHands.getBoardsInTableRound(2, 1))
      }
    )
  }
  ...
  def checkPlayedBoards(
    ...
    sb.withClueAndScreenShot(
      screenshotDir,
      s"screenshotTable${table.getOrElse(0)}Round${round}",
      s"Table ${table}, round ${round} looking at boards ${boards}"
    ) {
      sb.validate
    ...
    waitForFutures(
      ...
      CodeBlock {
        ...
        val em = ScoreboardPage.current
          ...
          .validate
        em.getInputFieldNames.foreach(p =>
          p match {
        ...
        val sn =
          ss.verifyAndSelectScorekeeper(ns.one, ns.two, ew.one, ew.two, North)
        sn.verifyNamesAndSelect(
          ns.teamid,
          ew.teamid,
          ns.one,
  ...
  it should "show the results on the scoreboards after round 2" in {
    ...
    waitForFutures(
      ...
      CodeBlock {
        ...
        val sb = ScoreboardPage.current
        val (ts, pes) = allHands
          .getScoreToRound(2, HandTableView(1, 2, team1.teamid, team2.teamid))
        sb.checkTable(ts: _*)
        sb.checkPlaceTable(pes: _*)
      ...
      CodeBlock {
        ...
        val (ts, pes) = allHands.getScoreToRound(
          2,
          HandTableView(2, 2, team3.teamid, team4.teamid)
        )
        sb.checkTable(ts: _*)
        sb.checkPlaceTable(pes: _*)
  ...
  def playRound(
    ...
    try {
      withClueAndScreenShot(
        screenshotDir,
        s"PlayRoundT${table}R${round}",
        s"On table ${table} round ${round}",
        true
      ) {
        ...
        val board = withClue(s"""board ${boards.head}""") {
          ...
          val brd =
            hand.enterHand(table, round, boards.head, allHands, nsTeam, ewTeam)
          brd
        ...
        while (!unplayedBoards.isEmpty) {
          ...
          val board = withClue(s"""board ${b}""") {
            ...
            currentBoard =
              hand2.enterHand(table, round, b, allHands, nsTeam, ewTeam)
...
//    maximize
    ...
    peoplePage.withClueAndScreenShot(
      screenshotDir,
      "ShowPeoplePage",
      "Checking people"
    ) {
      if (ids.size == 1) {
        ...
        testlog.info(
          s"Not testing the people page with results, number of matchs played was not 1: ${ids.size}"
        )
      }
    }
    if (hasFullscreen) peoplePage.clickFullscreen
  ...
  def getDuplicate(dupid: String): MatchDuplicate = {
    ...
    o.data match {
      ...
      case None =>
        testlog.warning(
          s"Unable to get MatchDuplicate from rest API for ${dupid}: ${o}"
        )
  ...
  def postDuplicate(dup: MatchDuplicate): String = {
    ...
    o.data match {
      ...
      case None =>
        testlog.warning(
          s"Unable to post MatchDuplicate to rest API for ${dup.id}: ${o}"
        )
  ...
  it should "Create another duplicate match from the one just played" in {
    ...
    val ndup =
      tdup.copy(scoringmethod = Some(MatchDuplicate.InternationalMatchPoints))
  ...
  it should "show the people page 2" in {
    ...
    peoplePage.withClueAndScreenShot(
      ...
      if (ids.size == 2) {
        ...
        testlog.info(
          s"Not testing the people page with results, number of matches played was not 2: ${ids.size}"
        )
      }
    }
  }
