...
object ImportPage {
  ...
  def current(implicit webDriver: WebDriver, patienceConfig: PatienceConfig, pos: Position): ImportPage = {
    new ImportPage
  }

  def goto(implicit webDriver: WebDriver, patienceConfig: PatienceConfig, pos: Position): ImportPage = {
    go to urlFor
    new ImportPage
  }
...
class ImportPage( implicit val webDriver: WebDriver, pageCreated: SourcePosition ) extends Page[ImportPage] with Popup[ImportPage] {
  ...
  def validate(implicit patienceConfig: PatienceConfig, pos: Position): ImportPage = logMethod(s"${pos.line} ${getClass.getSimpleName}.validate") { eventually {
  ...
  def validateSuccess( file: Option[File], beforeCount: Int )(implicit patienceConfig: PatienceConfig, pos: Position): (ImportPage, Option[Int]) = logMethod(s"${pos.line} ${getClass.getSimpleName}.validate") { eventually {
    currentUrl mustBe urlFor
  ...
  def checkSelectedFile( file: Option[File], beforeCount: Int )( implicit pos: Position ): (ImportPage, Option[Int]) = {
    val row = file.map { f =>
   ...
   */
  def getImportedIds(implicit patienceConfig: PatienceConfig, pos: Position): List[(String, Int)] = eventually {
  ...
  def delete( row: Int )(implicit patienceConfig: PatienceConfig, pos: Position): ImportPage = {
    clickButton( s"Delete${row}" )
    this
  }

  def importDuplicate( importId: String, row: Int )(implicit patienceConfig: PatienceConfig, pos: Position): ListDuplicatePage = {
    clickButton( s"Duplicate${row}" )
    new ListDuplicatePage( Some(importId) )
  }

  def importChicago( importId: String, row: Int )(implicit patienceConfig: PatienceConfig, pos: Position): ListPage = {
    clickButton( s"Chicago${row}" )
    new ListPage( Some(importId) )
  }

  def importRubber( importId: String, row: Int )(implicit patienceConfig: PatienceConfig, pos: Position): rubber.ListPage = {
