...
object ScoreboardPage {
  ...
  def current(implicit webDriver: WebDriver, patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    val (did, view) = findDuplicateId
    new ScoreboardPage( Option( did ), view)
  }

  def waitFor(implicit webDriver: WebDriver, patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    val (did, view) = eventually { findDuplicateId }
    new ScoreboardPage( Option( did ), view)
  }

  def goto( id: String )(implicit webDriver: WebDriver, patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    go to getUrl(id)
  ...
  def buttonIds( view: ViewType ): List[(String, String)] = view match {
...
class ScoreboardPage(
  ...
  def validate(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = logMethod(s"${pos.line} ${getClass.getSimpleName}.validate") {
  ...
  def validate(boards: List[Int])(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = logMethod(s"${pos.line} ${getClass.getSimpleName}.validate") {
    eventually {
  ...
  def clickMainMenu(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    clickButton("MainMenu")
    this
  }

  def validateMainMenu(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    eventually {
  ...
  def clickHelpMenu(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    clickButton("HelpMenu")
    this
  }

  def validateHelpMenu(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    eventually {
      findElemById("Help")
    }
    this
  ...
  def clickDirectorButton(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
  ...
  def clickScoreStyle(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    clickButton("ScoreStyle")
    this
  }

  def clickCompleteGameButton(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
  ...
  def clickAllBoards(implicit patienceConfig: PatienceConfig, pos: Position): BoardsPage = {
    clickButton("AllBoards")
    new BoardsPage
  }

  def clickEditNames(implicit patienceConfig: PatienceConfig, pos: Position): EditNamesPage = {
    clickButton("EditNames")
    new EditNamesPage( dupid.get )
  }

  def clickTableButton(table: Int)(implicit patienceConfig: PatienceConfig, pos: Position): TablePage = {
    view match {
  ...
  def clickBoardToHand( board: Int )(implicit patienceConfig: PatienceConfig, pos: Position): HandPage = {
    clickButton(s"Board_B${board}")
    new HandPage
  }

  def clickBoardToBoard( board: Int )(implicit patienceConfig: PatienceConfig, pos: Position): BoardPage = {
    clickButton(s"Board_B${board}")
    new BoardPage
  }

  def clickSummary(implicit patienceConfig: PatienceConfig, pos: Position): ListDuplicatePage = {
//    clickButton("Summary")
   ...
   * @param team
   */
  def checkTable( teams: TeamScoreboard* /* ( Team, String, List[String] )* */ )(implicit pos: Position): Unit = {
    val table = getTable
  ...
  def clickDelete(implicit patienceConfig: PatienceConfig, pos: Position): ScoreboardPage = {
    eventually {
      clickButton("Delete")
    }
    this
  ...
  def clickDeleteOk(implicit patienceConfig: PatienceConfig, pos: Position): ListDuplicatePage = {
    clickPopUpOk
    // Thread.sleep(200L)
   ...
   * @param style the input style, valid values are from ScoreStyle sealed trait
   */
  def setScoreStyle( style: ScoreStyle )( implicit webDriver: WebDriver, pos: Position ): ScoreboardPage = {
