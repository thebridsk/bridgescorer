...
class EditNamesPage(
    ...
    with ErrorMsgDiv[EditNamesPage] {
  ...
  def getCurrentPlayerNames(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): List[String] = {
    getElemsByXPath(
      """//div[contains(concat(' ', @class, ' '), ' chiDivEditNamesPage ')]/table/tbody/tr/td[1]"""
    ).map(e => e.text)
  }

  def getNewPlayerNames(implicit
      patienceConfig: PatienceConfig,
      pos: Position
  ): List[String] = {
    getElemsByXPath(
      """//div[contains(concat(' ', @class, ' '), ' chiDivEditNamesPage ')]/table/tbody/tr/td[2]/div/div/div/div/input"""
