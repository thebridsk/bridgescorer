...
object DuplicateTestPages {
  ...
  lazy val matchedNames: List[String] = allHands.teams.flatMap { t =>
    List(t.one, t.two).filter(n =>
      n.toLowerCase().startsWith(prefixThatMatchesSomeNames)
    )
  }
  ...
  lazy val allHands: AllHandsInMatch = new AllHandsInMatch(
    List(
      ...
      HandsOnBoard(
        ...
        OtherHandPlayed(2, 1, 5, 2, 0, 12, -12)
      ),
      HandsOnBoard(
        2,
        1,
      ...
      HandsOnBoard(
        ...
        OtherHandPlayed(2, 3, 10, 2, 0, 1, -1)
      ),
      HandsOnBoard(
        2,
        3,
      ...
      HandsOnBoard(
        ...
        OtherHandPlayed(2, 3, 11, 1, 1, 0, 0)
      ),
      HandsOnBoard(
        2,
        3,
      ...
      HandsOnBoard(
        ...
        OtherHandPlayed(2, 3, 12, 1, 1, 0, 0)
      ),
      HandsOnBoard(
        2,
        3,
      ...
      HandsOnBoard(
        ...
        OtherHandPlayed(2, 5, 16, 2, 0, 2, -2)
      ),
      HandsOnBoard(
        2,
        5,
      ...
      HandsOnBoard(
        ...
        OtherHandPlayed(2, 5, 17, 1, 1, 0, 0)
      ),
      HandsOnBoard(
        2,
        5,
      ...
      HandsOnBoard(
        ...
        OtherHandPlayed(2, 5, 18, 1, 1, 0, 0)
      ),
      HandsOnBoard(
        2,
        5,
  ...
  it should "allow players names to be entered at both tables" in {
    ...
    waitForFutures(
      ...
      CodeBlock {
        ...
        en =
          en.enterPlayer(South, team1original.two).enterPlayer(East, team2.two)
        en.isOKEnabled mustBe false
      ...
      CodeBlock {
        ...
        en.isOKEnabled mustBe true
        val scoreboard =
          en.clickOK.asInstanceOf[ScoreboardPage].validate(4 :: 5 :: 6 :: Nil)
...
//          hand.clickClear
          ...
          board
            .checkBoardButtons(1, true, 1)
            .checkBoardButtons(1, false, 2, 3)
            .checkBoardButtonSelected(1)
          val board1 = board.clickPlayedBoard(1).validate
          val nsRound1 = allHands.movement
            .map { mov =>
              mov.hands
                .find(hit => hit.round == 1 && hit.table === 1)
                .map { hit =>
                  hit.ns
                }
          ...
          board2
            .checkBoardButtons(2, true, 1, 2)
            .checkBoardButtons(2, false, 3)
            .checkBoardButtonSelected(2)
      ...
      CodeBlock {
        ...
        PageBrowser.withClueAndScreenShot(
          screenshotDir,
          "Round1Table2EnterHand",
          "Entering hands R1T2"
        ) {
          val hand = HandPage.current
          hand.getScore mustBe Tuple3(
            "Missing required information",
            "",
            "Enter contract tricks"
          )
          hand.isOkEnabled mustBe false
          hand.getInputStyle mustBe Some("Guide")
          val board = hand.enterHand(2, 1, 4, allHands, team3, team4)
          board
            .checkBoardButtons(4, true, 4)
            .checkBoardButtons(4, false, 5, 6)
            .checkBoardButtonSelected(4)
          val hand2 = board.clickUnplayedBoard(5).validate
          val board2 = hand2.enterHand(2, 1, 5, allHands, team3, team4)
          board2
            .checkBoardButtons(5, true, 4, 5)
            .checkBoardButtons(5, false, 6)
            .checkBoardButtonSelected(5)
  ...
  def checkPlayedBoards(
    ...
    val pr = boards.foldLeft(None: Option[BoardPage]) { (progress, board) =>
      def processBoard(bp: BoardPage) = {
        ...
        testlog.fine(
          s"table ${table}, round ${round}, current board is ${board}, checkmarks ${checkmarks}, all boards are ${boards}: board picture ${boardWithPicture}, ${nsForPicture}, ${pictureButtonShowing}"
        )
        boardWithPicture.filter(b => b == board).foreach { b =>
          nsForPicture.foreach { ns =>
            if (pictureButtonShowing) {
              testlog.fine(
                s"table ${table}, round ${round}, current board is ${board}, checkmarks ${checkmarks}, all boards are ${boards}: show picture"
              )
              ...
              testlog.fine(
                s"table ${table}, round ${round}, current board is ${board}, checkmarks ${checkmarks}, all boards are ${boards}: no picture"
              )
        ...
        testlog.fine(
          s"trying to take screen shot of board ${screenshot}, current board is ${board}, all boards are ${boards}"
        )
        screenshot
          .filter(b => b == board)
          .map { b =>
  ...
  def toOriginal(
      data: (List[TeamScoreboard], List[PlaceEntry])
  ...
  it should "show the director's scoreboard and complete scoreboard shows checkmarks for the played games" in {
    ...
    waitForFutures(
      ...
      CodeBlock {
        ...
        val (ts, pes) =
          toOriginal(allHands.getScoreToRound(1, HandDirectorView))
        sb.checkTable(ts: _*)
        ...
        checkPlayedBoards(
          ...
          1,
          false,
          boardWithPicture = Some(1),
          nsForPicture = nsForTable1Round1,
          pictureButtonShowing = true
        )
      },
      CodeBlock {
        import SessionComplete._
        ...
        val (ts, pes) =
          toOriginal(allHands.getScoreToRound(1, HandCompletedView))
        sb.checkTable(ts: _*)
        sb.checkPlaceTable(pes: _*)
        checkPlayedBoards(
          sb,
      ...
      CodeBlock {
        ...
        sb.checkTable(resultAfterOneRoundCheckMark: _*)
        val (ts, pes) = toOriginal(
          allHands.getScoreToRound(
            1,
      ...
      CodeBlock {
        ...
        val (ts, pes) = toOriginal(
          allHands.getScoreToRound(
            1,
            HandTableView(2, 1, team3.teamid, team4.teamid)
          )
        )
        sb.checkTable(ts: _*)
        sb.checkPlaceTable(pes: _*)
  ...
  def selectScorekeeper(
    ...
    val screenShotDir =
      if (takeScreenshot) Some((docsScreenshotDir, "SelectSK")) else None
  ...
  it should "allow player 1 on team 1 name to be changed" in {
    ...
    val playersBefore = (team1original :: team2 :: team3 :: team4 :: Nil)
      .flatMap(t => t.one :: t.two :: Nil)
      .grouped(2)
      .toList
    val playersAfter = (team1 :: team2 :: team3 :: team4 :: Nil)
      .flatMap(t => t.one :: t.two :: Nil)
      .grouped(2)
      .toList
  ...
  it should "allow selecting players for round 2" in {
    ...
    waitForFutures(
      ...
      CodeBlock {
        import SessionTable1._
        val sbc = ScoreboardPage.current
          .clickTableButton(1)
          .validate
          .takeScreenshot(docsScreenshotDir, "TableRound2")
          .clickCompletedScoreboard
          .validate
        val sb = selectScorekeeper(
    ...
    waitForFutures(
      ...
      CodeBlock {
        ...
        val (ts, pes) = allHands.getScoreToRound(
          2,
          HandTableView(1, 2, team1.teamid, team2.teamid)
        )
        sb.checkTable(ts: _*)
        sb.checkPlaceTable(pes: _*)
      ...
      CodeBlock {
        ...
        val (ts, pes) = allHands.getScoreToRound(
          2,
          HandTableView(2, 2, team4.teamid, team3.teamid)
        )
        sb.checkTable(ts: _*)
        sb.checkPlaceTable(pes: _*)
  ...
  def playRound(
    ...
    PageBrowser.withClueAndScreenShot(
      screenshotDir,
      s"Round${round}Table${table}EnterHand",
      s"Enter Hand R${round}T${table}"
    ) {
      val sb = selectScorekeeper(
        currentPage,
        table,
      ...
      val board = withClue(s"""board ${boards.head}""") {
        ...
        brd
          .checkBoardButtons(boards.head, true, boards.head)
          .checkBoardButtons(boards.head, false, boards.tail: _*)
          .checkBoardButtonSelected(boards.head)
      }
      ...
      while (!unplayedBoards.isEmpty) {
        ...
        val board = withClue(s"""board ${b}""") {
          ...
          currentBoard = currentBoard
            .checkBoardButtons(b, true, playedBoards: _*)
            .checkBoardButtons(b, false, unplayedBoards: _*)
            .checkBoardButtonSelected(b)
        }
  ...
  def validateRound(
    ...
    PageBrowser.withClueAndScreenShot(
      screenshotDir,
      s"Round${round}Table${table}Validate",
      "Validate R${round}T${table}"
    ) {

      val sbr = currentPage.validate
      val (ts, pes) = allHands.getScoreToRound(
        round,
        HandTableView(table, round, nsTeam.teamid, ewTeam.teamid),
        imp
      )
      sbr.checkTable(ts: _*)
    ...
    waitForFutures(
      ...
      CodeBlock {
        ...
        withClue("""On session Director""") {
          ...
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(1, 1).head,
            allHands,
            false
          )
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(1, 3).head,
            allHands,
            false
          )
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(2, 3).head,
            allHands,
            false
          )
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(1, 5).head,
            allHands,
            false
          )
      ...
      CodeBlock {
        ...
        withClue("""On session Table 2""") {
          ...
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(1, 1)(2),
            allHands,
            true
          )
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(1, 3)(2),
            allHands,
            true
          )
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(2, 3)(2),
            allHands,
            false
          ) // played on this table
          page.checkHand(
            3,
            allHands.getBoardsInTableRound(1, 5)(2),
            allHands,
            true
          )
  ...
  it should "show the people page" in {
    ...
    val peoplePage = lp.clickStatistics.validate
      .takeScreenshot(docsScreenshotDir, "Stats")
      .clickPeopleResults
    ...
    if (ids.size == 1) {
      ...
      testlog.info(
        s"Not testing the people page with results, number of matchs played is ${ids.size}"
      )
    }
  ...
  it should "have rest call and queryml call return the same match" in {
    ...
    dupid match {
      case Some(duplicateId) =>
        val url: URL =
          new URL(TestServer.hosturl + "v1/rest/duplicates/" + duplicateId)
        ...
        try {
          ...
          val (storegood, played) =
            new MatchDuplicateCacheStoreSupport(false).fromJSON(json)
          pl = Some(played)
          ...
          case x: Exception =>
            pl match {
              case Some(played) =>
                val j =
                  new MatchDuplicateCacheStoreSupport(false).toJSON(played)
                FileIO.writeFileSafe(
                  s"${screenshotDir}/DuplicateTestPages.QueryML.json",
                  j
                )
              case None =>
            }
        ...
        try {
          ...
          played.boards.foreach(b => {
            ...
            b.hands.foreach(h => {
              ...
              assert(
                b.created - 100 <= h.created && h.created <= b.updated + 100
              )
              assert(
                b.created - 100 <= h.updated && h.updated <= b.updated + 100
              )
            })
          })
...
//    HomePage.goto.validate.clickNewDuplicateButton.validate
  ...
  it should "try going to latest new match, and succeed" in {
    ...
    PageBrowser.withClueAndScreenShot(
      screenshotDir,
      "LatestNew",
      s"Going to latest looking for ${dupid2}",
      true
    ) {
      val sp = hp
        .clickLatestNewDuplicateButton(true)
        .validate
        .asInstanceOf[ScoreboardPage]
  ...
  it should "go to table 1 page and go to round 1 and test suggestions" in {
    ...
    PageBrowser.withClueAndScreenShot(
      ...
      val es =
        table.clickRound(1).asInstanceOf[TableEnterScorekeeperPage].validate
...
//    eventually {menu.findElemById("Export")}
    ...
    Using.resource(new ZipFile(f.jfile)) { zip =>
      zip.entries().asScala.map { ze => ze.getName }.toList must contain(
        s"store/MatchDuplicate.${dupid.get}.yaml"
      )
    }
  ...
  it should "import zip" in {
    ...
    val bp =
      ldpr.clickPopUpOk.validate.clickHome.validate.clickListDuplicateButton
        .validate(newId)
        .clickDuplicate(newId)
        .validate
        .clickBoardToBoard(1)
    bp.validatePictures(ids = nsForTable1Round1.toList)
    val bp1 = bp
      .clickShowPicture(nsForTable1Round1.get)
      .validatePictures(true)
      .clickOkPicture
      .validatePictures(ids = nsForTable1Round1.toList)
  ...
  it should "calculate a pairing" in {
    ...
    se.clickCancelNeverPair.validate
      .takeScreenshot(docsScreenshotDir, "Pairings")
  }
  ...
  it should "show server URLs" in {
    ...
    ldp.withClueAndScreenShot(
      screenshotDir,
      "ShowServerURLs",
      "Unable to see server URLs"
    ) {
