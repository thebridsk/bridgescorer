...
object HandPicture {
  ...
  val patternImageSrcBrowser: Regex =
    """data:image/([^;]+);base64,[a-zA-z0-9/+]+=?=?""".r
...
trait HandPicture[+T <: Page[T]] extends Popup[T] {
  ...
  def getOkButton: Element =
    findOption(cssSelector(okPicture)).getOrElse(find(cssSelector(okPicture2)))
  ...
  def validatePicture(
    ...
    eventually {
      ...
      withClue(
        s"Validating page with a picture button, showingPicture=${showingPicture}"
      ) {
  ...
  /**
    ...
    */
  def validatePictures(
      showingPicture: Boolean = false,
      ids: List[String] = Nil,
      notShowingIds: List[String] = Nil
  )(implicit patienceConfig: PatienceConfig, pos: Position): T =
    eventually {

      withClue(
        s"Validating page with potentially multiple picture buttons, showingPicture=${showingPicture}"
      ) {
        isPictureDisplayed mustBe showingPicture
        ...
        case Some(url) =>
          if (serverUrl) {
            url match {
              ...
              case _ =>
                fail(
                  s"Did not find an img src value for server, called from ${pos.line}: ${url}"
                )
            }
          } else {
            url match {
              ...
              case _ =>
                fail(
                  s"Did not find an img src value for browser, called from ${pos.line}: ${url}"
                )
            }
          }
        case None =>
      ...
      getImageUrl match {
        case Some(url) =>
          url match {
            ...
            case _ =>
              fail(
                s"Did not find an img src value, called from ${pos.line}: ${url
                  .substring(0, 50)}"
              )
          }
        case None =>
