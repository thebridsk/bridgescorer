...
//    find( xpath( """//div[@id='BridgeApp']/div[1]/div[1]/div[3]/div[1]/h1""") ).text mustBe "Simple Rotation"
  ...
  def getPairings(implicit patienceConfig: PatienceConfig, pos: Position) = {
    ...
    def getPairs( list: List[Element], index: Int, result: List[Pairings]): List[Pairings] = {
      list match {
        case Nil => result
        case one :: Nil =>
  ...
  def checkDealer( dealer: Option[String], notDealer: String*)(implicit patienceConfig: PatienceConfig, pos: Position) = {
    ...
    withClue(s"""Checking for next sitting out player, sitting out $dealer, not sitting out $notDealer""") {
      val idplayers = players.map( toPlayerButtonId(_))
      val map = findButtons(idplayers: _*)
