...
object ChicagoTestPages {
  ...
  val log: Logger = Logger[ChicagoTestPages]()
...
class ChicagoTestPages
  ...
  it should "allow us to score a Chicago match" in {
    ...
    chicagoId = withClueAndScreenShot(
      screenshotDir,
      "ValidateEnterNames",
      "Error validating EnterNamesPage"
    ) {
      Some(eventually {
  ...
  it should "allow player names to be entered when playing Chicago" in {
    ...
    withClueAndScreenShot(
      screenshotDir,
      "PlayerNamesAndVulnerability",
      "Player names and vulnerability"
    ) {
      eventually {
  ...
  it should "play a round of 4 hands" in {
    ...
    val sp2 = enterHand(
      ...
      NotVul,
      Vul,
      s"620 ${player3}-${player4}",
      player3,
      0,
      1,
      List(0, 0, 620, 620),
      List(420, 420, 620, 620),
      List(420, 420, 620, 620)
    )
    ...
    val sp3 = enterHand(
      ...
      Vul,
      NotVul,
      s"600 ${player1}-${player2}",
      player2,
      0,
      2,
      List(600, 600, 0, 0),
      List(1020, 1020, 620, 620),
      List(1020, 1020, 620, 620)
    )
    ...
    sp3.checkButtons(
      ...
      List(
        SummaryPage.buttonNewRound,
        SummaryPage.button6HandRound,
        SummaryPage.button8HandRound
      )
    )
    ...
    val sp4 = enterHand(
      ...
      Vul,
      Vul,
      s"100 ${player1}-${player2}",
      player4,
      0,
      3,
      List(100, 100, 0, 0),
      List(1120, 1120, 620, 620),
      List(1120, 1120, 620, 620)
    )
  ...
  def getChicago(chiid: String): MatchChicago = {
    ...
    o.data match {
      ...
      case None =>
        log.warning(
          s"Unable to get MatchChicago from rest API for ${chiid}: ${o}"
        )
  ...
  def postChicago(chi: MatchChicago): String = {
    ...
    o.data match {
      ...
      case None =>
        log.warning(
          s"Unable to post MatchChicago to rest API for ${chi.id}: ${o}"
        )
  ...
  it should "be able to edit the names" in {
    ...
    withClueAndScreenShot(screenshotDir, "EditNames", "Edit Names test") {
      ...
      val namesAfterChange =
        players.map(p => if (p == player1) playerOut else p)
...
//      enp2.getPlayerCombobox(iPlayer1).clear()
  ...
  it should "start playing another game using the saved game using next hand with 6 hands in round" in {
    ...
    val gp =
      SummaryPage.current(ChicagoMatchTypeFour).validate.clickQuit.validate
    tcpSleep(1)
...
//   List(100,100,0,0), List(1120,1120,620,620), List(1120,1120,620,620) )
    ...
    val sp2 = enterHand(
      ...
      0,
      List(420, 420, 0, 0),
      List(1540, 1540, 620, 620),
      List(1540, 1540, 620, 620)
    )

    sp2.checkButtons(
      List(SummaryPage.button6HandRound, SummaryPage.button8HandRound),
  ...
  it should "export zip" in {
    ...
    Using.resource(new ZipFile(f.jfile)) { zip =>
      zip.entries().asScala.map { ze => ze.getName }.toList must contain(
        s"store/MatchChicago.${chicagoId.get}.yaml"
      )
    }
