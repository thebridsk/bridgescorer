package com.github.thebridsk.bridge.rotation
...
case class Table(
  ...
  def swapLeftPartnerOf(l: Location): Table = {
    val partner =
      partnerLocOf(l).getOrElse(error("Did not find partner of " + l))
  ...
  def getSwappingPlayersInRotateSwapRightPartnerOf(
      l: Location
  ): List[(String, String)] = {
...
object Chicago5Rotation {
  ...
  def playRight(
    ...
    var s = ts.last
    var players =
      if (order.isEmpty) s.north :: s.south :: s.west :: s.east :: Nil
      else order
  ...
  def playBoth(
    ...
    val players =
      if (order.isEmpty)
        s.sittingOut :: s.west :: s.east :: s.south :: s.north :: Nil
      else order
    println("order: " + order)
    println("players: " + players)
    if (s.sittingOut != players.head)
      throw new IllegalArgumentException(
        "First player in order must be sitting out in last entry"
      )
  ...
  def playCounterClockwise(
      playerGoingOut: String,
      list: List[Table] = Table(
        north = "1",
        south = "2",
        east = "3",
  ...
  private def getLocationNotSittingOut(
      playerGoingOut: String,
      list: List[Table]
  ) = {
    val t = list.last
...
//    var ts = playBoth(players,list)
  ...
  def maneuverRotateCounterClockwise(
      playerGoingOut: String
  )(t: Table): Table = {
    val l = t
      .findNotSittingOut(playerGoingOut)
      .getOrElse(
        throw new IllegalArgumentException(
          s"Unable to find player $playerGoingOut"
        )
      )
  ...
  def maneuverRotateCounterClockwiseAlt(
      playerGoingOut: String
  )(t: Table): Table = {
    val l = t
      .findNotSittingOut(playerGoingOut)
