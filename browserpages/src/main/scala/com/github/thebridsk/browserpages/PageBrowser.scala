...
//    PageBrowser.log.fine( s"""Clicking on ${e}: text = ${e.text}""" )
  ...
  def on(query: QueryBy)(implicit
    ...
    PageBrowser.log.fine(
      s"Finding element ${query} to click: patienceConfig=${patienceConfig}, pos=${pos.line}"
    )
    val e = query.queryElement
    PageBrowser.log.fine(
      s"Clicking on element ${query} ${e}: patienceConfig=${patienceConfig}, pos=${pos.line}"
    )
    on(e)
  }
...
trait PageBrowser {
  ...
  def pressKeys(
      value: CharSequence
  )(implicit webDriver: WebDriver, pos: Position): PageBrowser = {
...
//    Thread.sleep( 100L )
  ...
  def executeScript[T](script: String, args: AnyRef*)(implicit
    ...
    webDriver match {
      case executor: JavascriptExecutor =>
        executor.executeScript(script, args: _*)
      case _ =>
        throw new UnsupportedOperationException(
          "Web driver " + webDriver.getClass.getName + " does not support javascript execution."
        )
    }
  ...
  private def newInstance[T <: Element](
      e: WebElement,
      pos: Position,
      webdriver: WebDriver,
      patienceConfig: PatienceConfig
  )(implicit con: Constructor[T]): T = {
  ...
  def takeScreenshot(
    ...
    if (webDriver.isInstanceOf[TakesScreenshot]) {
      ...
      PageBrowser.log.warning(
        s"Unable to take screenshot, called from ${pos.line}"
      )
    }
  }
  ...
  )(fun: => T)(implicit webDriver: WebDriver, pos: Position): T = {
    try {
      fun
    } catch {
      case x: Throwable =>
        ...
        if (savedom) {
          val f =
            if (filename.endsWith(".dom.html")) filename
            else filename + ".dom.html"
          val destFile = PageBrowser.getPath(directory, f)
  ...
  def withClueAndScreenShot[T](
    ...
    takeScreenshotOnError(
      directory,
      s"${filenamePrefix}_${pos.lineForFilename}",
      savedom,
      domToStdout
    ) {
  ...
  def isSoundPlaying(e: WebElement)(implicit
    ...
    PageBrowser
      .executeScript("""!arguments[0].paused || arguments[0].currentTime""", e)
    ...
    try {
      ...
      case x: Exception =>
        ...
        classtag.runtimeClass.getConstructors().foreach { c =>
          log.warning(s"""PageBrowser for ${classtag.runtimeClass
            .getName()} found constructor $c""")
        }
        throw x
    }
  }

  private[browserpages] def getPath(filename: File): Path =
    FileSystems.getDefault.getPath(filename.toString())

  private[browserpages] def getPath(filename: String): Path =
    FileSystems.getDefault.getPath(filename)

  private[browserpages] def getPath(directory: String, filename: String): Path =
    FileSystems.getDefault.getPath(directory, filename)
