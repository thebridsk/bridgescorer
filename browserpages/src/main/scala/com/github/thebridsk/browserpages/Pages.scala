...
abstract class Page[+T <: Page[T]]()(
  ...
  def findButtons(
    ...
    val buttons = webDriver
      ...
      .toMap
    withClue(s"""${pos.line} findButtons${ids
      .mkString("[", ",", "]")} only found ${buttons.keySet
      .mkString("[", ",", "]")}, page created ${pageCreated.line}""") {
  ...
  def findAllButtons(implicit pos: Position): Map[String, Element] = {
    findElements(By.cssSelector("button")).asScala
      .map(b => (b.getAttribute("id"), new Element(b)))
      .toMap
  }
...
//        if (!we.underlying.isDisplayed()) Thread.sleep(100)
      ...
      case x: Exception =>
        log.warning(
          s"""Got exception clicking button, patienceConfig = ${patienceConfig}""",
          x
        )
        throw x
    }
...
//        PageBrowser.scrollToElement(we.underlying)
  ...
  def findNumberInput(
      iname: String
  )(implicit pos: Position, patienceConfig: PatienceConfig): NumberField = {
  ...
  def findInput(iname: String, itype: String)(
    ...
    try {
      ...
      withClue(
        s"And has an input field with name ${iname}, page created ${pageCreated.line}"
      ) { input.getTagName mustBe "input" }
      withClue(
        s"And has an input field with name ${iname} and with type ${itype}, page created ${pageCreated.line}"
      ) { input.getAttribute("type") mustBe itype }
      new Element(input)
    } catch {
      case x: Throwable =>
        testlog.fine(
          s"findInput: exception ${x.toString()}, page created ${pageCreated.line}",
          x
        )
...
//        x.printStackTrace(System.out)
  ...
  def findTextInputs(
      iname: String*
  )(implicit pos: Position): Map[String, TextField] = {
  ...
  def findInputs(itype: String, iname: String*)(
    ...
    try {
      val input = findElements(By.tagName("input")).asScala.flatMap { we =>
      ...
      withClue(
        s"""${pos.line} findInputs(${itype},${iname.toList}), page created ${pageCreated.line}"""
      ) {
      ...
      case x: Throwable =>
        testlog.fine(
          s"${pos.line} findInputs(${itype},${iname.toList}): exception ${x
...
//        x.printStackTrace(System.out)
  ...
  def findElemByXPath(xp: String)(implicit pos: Position): Element = {
    try {
      ...
      case x: org.openqa.selenium.NoSuchElementException =>
        fail(
          s"""${pos.line} Did not find Element with xpath query "${xp}", page created ${pageCreated.line}"""
        )
        throw x
    }
  }
  ...
  def findElemsByXPath(xp: String)(implicit pos: Position): List[Element] = {
    findElements(By.xpath(xp)).asScala.map { e =>
      new Element(e)
    }.toList
  }
  ...
  def findDateTimePicker(
      name: String
  )(implicit pos: Position): DateTimePicker = {
    DateTimePicker.find(name)
  }

  def getDateTimePicker(
      name: String
  )(implicit patienceConfig: PatienceConfig, pos: Position): DateTimePicker = {
  ...
  def currentUrl(implicit patienceConfig: PatienceConfig, pos: Position) =
    PageBrowser.currentUrl

  def pageTitle(implicit patienceConfig: PatienceConfig, pos: Position) =
    PageBrowser.pageTitle

  def go(implicit pos: Position) = PageBrowser.go

  def pressKeys(value: String)(implicit pos: Position): PageBrowser =
    PageBrowser.pressKeys(value)

  def executeScript[T](script: String, args: AnyRef*): AnyRef =
    PageBrowser.executeScript(script, args: _*)
  ...
  /**
    ...
    * @param filename The name of the file where the screenshot is written to.  It it doesn't end in ".png", then ".png" will be appended.
    */
  def takeScreenshotOnError[T](directory: String, filename: String)(
      fun: => T
  ...
  /**
    ...
    * @param directory The directory where the screenshot is written to
    */
  def takeScreenshotOnError[T](
      directory: String
  )(fun: => T)(implicit pos: Position): T = {
    val filename = pos.lineForFilename
  ...
  /**
    ...
    */
  def withClueAndScreenShot[T](
      directory: String,
      filenamePrefix: String,
      clue: Any,
      savedom: Boolean = false,
      domToStdout: Boolean = false
  )(fun: => T)(implicit pos: Position): T = {
    PageBrowser.takeScreenshotOnError(
      directory,
      filenamePrefix,
      savedom,
      domToStdout
